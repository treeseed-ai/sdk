...
import {
	...
	type KeyAgentCommand,
} from '../../src/operations/services/configuration/key-agent.ts';
...
function parseArgs(argv: string[]) {
	...
	while (rest.length > 0) {
		...
		if (current === '--idle-timeout-ms') {
			parsed.idleTimeoutMs = Number.parseInt(rest.shift() ?? String(KEY_AGENT_IDLE_TIMEOUT_MS), 10);
			continue;
		}
...
async function runInteractiveUnlock(parsed: ReturnType<typeof parseArgs>) {
	...
	if (wrapped.exists && !wrapped.wrapped && !wrapped.migrationRequired) {
		throw new KeyAgentError('corrupt_wrapped_key', 'Unable to read the existing Treeseed machine key file.');
	}
	...
	if (needsCreation || needsMigration) {
		...
		if (!first) {
			throw new KeyAgentError('interactive_required', 'A non-empty passphrase is required.');
		}
		const second = (await prompt('Confirm passphrase: ', true)).trim();
		if (first !== second) {
			throw new KeyAgentError('unlock_failed', 'The passphrase confirmation did not match.');
		}
		passphrase = first;
	} else {
		passphrase = (await prompt('Treeseed passphrase: ', true)).trim();
		if (!passphrase) {
			throw new KeyAgentError('interactive_required', 'A passphrase is required to unlock the Treeseed machine key.');
		}
	}

	const response = await requestKeyAgent({
		command: 'unlock',
	...
	assertKeyAgentResponse(response);
	writeJson(response);
}

async function main() {
	const parsed = parseArgs(process.argv.slice(2));
	const defaults = getKeyAgentPaths();
	...
	const payload = JSON.parse(parsed.payload) as KeyAgentCommand;
	const response = await requestKeyAgent(payload);
	writeJson(response);
}

main().catch((error) => {
	const message = error instanceof Error ? error.message : String(error);
	const code = error instanceof KeyAgentError ? error.code : 'unlock_failed';
	writeJson({
		ok: false,
		code,
		message,
