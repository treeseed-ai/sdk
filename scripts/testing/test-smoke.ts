...
try {
	...
	writeFileSync(resolve(installRoot, 'package.json'), `${JSON.stringify({
		...
		type: 'module',
		scripts: {
			'verify:direct': 'tsx -e "console.log(\'treeseed-sdk-smoke-verify\')"',
			'verify:local': 'tsx -e "process.env.TREESEED_VERIFY_DRIVER=\'direct\'; console.log(\'treeseed-sdk-smoke-verify-local\')"',
			'verify:action': 'tsx -e "process.env.TREESEED_VERIFY_DRIVER=\'act\'; console.log(\'treeseed-sdk-smoke-verify-action\')"',
		},
	}, null, 2)}\n`, 'utf8');
