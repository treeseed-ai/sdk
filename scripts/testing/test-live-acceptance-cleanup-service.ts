...
import { runLiveReconcileTests } from '../../src/reconcile/support/acceptance/live-acceptance.ts';
import { provisionLocalCapacityAcceptanceProvider } from '../../src/reconcile/capacity/capacity-core/live-acceptance-capacity-context.ts';
import {
	createLocalCapacityAcceptanceScope,
	isLocalCapacityAcceptanceTeam,
} from '../../src/reconcile/capacity/capacity-core/live-acceptance-capacity-scope.ts';
...
try {
	...
	const cleanup = await runLiveReconcileTests({
		cwd: process.cwd(),
		environment: 'local',
		mode: 'cleanup',
		providers: ['local'],
