...
import { corePackageRoot, packageRoot, packageScriptPath, sdkPackageRoot } from '../../src/operations/services/agents/runtime-tools.ts';
import { listTemplateProducts, validateTemplateProduct } from '../../src/operations/services/support/template-registry.ts';
import { DEFAULT_STARTER_TEMPLATE_ID } from '../../src/entrypoints/models/sdk-types.ts';
...
async function scaffoldSite(siteRoot) {
	...
	runStep(process.execPath, [packageScriptPath('templates/scaffold-site'), siteRoot, '--template', DEFAULT_STARTER_TEMPLATE_ID, '--name', 'Smoke Site', '--site-url', 'https://smoke.example.com', '--contact-email', 'hello@example.com']);
}
