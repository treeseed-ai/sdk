...
function guaranteeId(type: string, subtype: string, journey: string, index: number) {
	return `guarantee.${type}.${subtype}.${slugifyTreeseedGuaranteeJourney(journey)}.${String(index).padStart(3, '0')}`;
}
...
const outputs = records.map((record) => {
	...
	const type = normalizeTreeseedGuaranteeTaxonomy(String(record.Type));
	const subtype = normalizeTreeseedGuaranteeTaxonomy(String(record['Sub-Type']));
	const journey = String(record.Journey);
	const owner = ownerPackage(type, subtype);
	const journeySlug = slugifyTreeseedGuaranteeJourney(journey);
	...
	const dependencyGuarantees = dependencies.map((dependency) => {
		...
		if (!dep) return null;
		return guaranteeId(normalizeTreeseedGuaranteeTaxonomy(String(dep.Type)), normalizeTreeseedGuaranteeTaxonomy(String(dep['Sub-Type'])), String(dep.Journey), dependency);
	}).filter(Boolean);
