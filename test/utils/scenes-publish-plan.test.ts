...
describe('scene publication planning', () => {
	it('builds a publish plan from an existing Phase 10 local publish manifest', async () => {
		...
		expect(generateTreeseedSceneTrainingOutputs({ projectRoot: root, scene: 'publish-plan-demo', from: run.artifacts!.runRoot }).ok).toBe(true);
		expect((await publishTreeseedSceneEvidence({ projectRoot: root, scene: 'publish-plan-demo', from: run.artifacts!.runRoot, target: 'local' })).ok).toBe(true);

		const report = await planTreeseedScenePublication({ projectRoot: root, scene: 'publish-plan-demo', from: run.artifacts!.runRoot });
	...
	it('auto-generates Phase 10 local publish when missing and supports artifact-store as plan-only', async () => {
		...
		const report = await planTreeseedScenePublication({ projectRoot: root, scene: 'publish-plan-demo', from: run.artifacts!.runRoot, targets: ['artifact-store'] });
		expect(report.ok).toBe(true);
	...
	it('reports invalid targets, missing runs, scene mismatches, and failed release-evidence plans', async () => {
		const root = workspace();
		writeScene(root);
		const missing = await planTreeseedScenePublication({ projectRoot: root, scene: 'publish-plan-demo', from: 'missing' });
		expect(missing.ok).toBe(false);
		expect(missing.diagnostics.some((entry) => entry.code === 'scene.run_not_found')).toBe(true);

		const run = await createRun(root);
		const invalid = await planTreeseedScenePublication({ projectRoot: root, scene: 'publish-plan-demo', from: run.artifacts!.runRoot, targets: ['bad-target' as never] });
		...
		const mismatch = await planTreeseedScenePublication({ projectRoot: root, scene: 'publish-plan-demo', from: run.artifacts!.runRoot, targets: ['docs'] });
		expect(mismatch.ok).toBe(false);
		expect(mismatch.diagnostics.some((entry) => entry.code === 'scene.publish_plan_scene_mismatch')).toBe(true);

		writeScene(root);
		const failedRelease = await planTreeseedScenePublication({ projectRoot: root, scene: 'publish-plan-demo', from: run.artifacts!.runRoot, targets: ['release-evidence'] });
	...
	it('exports selected redacted artifacts without raw trace, video, network, console, or app logs', async () => {
		...
		const run = await createRun(root, { failing: false });
		expect(generateTreeseedSceneTrainingOutputs({ projectRoot: root, scene: 'publish-plan-demo', from: run.artifacts!.runRoot }).ok).toBe(true);
		const report = await exportTreeseedScenePublication({ projectRoot: root, scene: 'publish-plan-demo', from: run.artifacts!.runRoot, targets: ['docs', 'training'] });
		expect(report.ok).toBe(true);
	...
	it('keeps publish-plan output ok when run.json cannot be updated', async () => {
		...
		const report = await planTreeseedScenePublication({ projectRoot: root, scene: 'publish-plan-demo', from: run.artifacts!.runRoot, targets: ['docs'] });
