...
import { TreeDbApiError } from '../../src/treedb/index.ts';
import { assertPublicHygiene } from './treedb-public-hygiene.ts';

describe('TreeDB public hygiene assertions', () => {
	it('accepts representative public payloads and error details', () => {
		...
		const error = new TreeDbApiError('Workspace policy has been revoked.', {
			status: 409,
	...
	it('detects internal paths and secret-like values', () => {
		expect(() => assertPublicHygiene({ localPath: '/tmp/treedb/repo' })).toThrow();
