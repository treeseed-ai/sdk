...
import * as treedx from '../../src/treedx/index.ts';
...
describe('TreeDX SDK contract drift checks', () => {
	...
	it('keeps TreeDX subpaths and SDK runtime exports present', () => {
		for (const subpath of ['./treedx', './treedx/client', './treedx/types', './treedx/adapters']) {
		...
		for (const name of [
			...
			expect(treedx, name).toHaveProperty(name);
		}
	});
});
