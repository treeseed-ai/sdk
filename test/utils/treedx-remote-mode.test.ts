...
function remoteSdk(payloads: unknown[]) {
	...
	const client = sdk.treeDx!.client as { fetchImpl?: typeof fetch };
...
describe('AgentSdk TreeDX remote mode', () => {
	it('constructs no-clone TreeDX mode from baseUrl token and repoId', () => {
		const { sdk } = remoteSdk([]);
		expect(sdk.treeDx?.client.baseUrl).toBe('https://treedx.example.test');
		expect(sdk.treeDx?.registry).toBeDefined();
		expect(sdk.treeDx?.federated).toBeDefined();
	...
	it('runs no-clone read search write graph context snapshot artifact and federated calls', async () => {
		const { sdk, calls } = remoteSdk([
			...
			{ ok: true, artifact: { artifactId: 'art_1', snapshotId: 'snap_1', repoId: 'repo_1', format: 'tar.zst', size: 1, checksum: 'blake3:1', uri: 'treedx://artifact/art_1' } },
		...
		expect((await sdk.treeDx!.client.exportArtifact()).artifactId).toBe('art_1');
		expect((await sdk.treeDx!.federated!.federatedSearch({ repoIds: ['repo_1'], query: 'guide' })).query).toBe('guide');
		expect((await sdk.treeDx!.federated!.federatedQuery({ repoIds: ['repo_1'], type: 'text', query: 'guide' })).type).toBe('text');
		expect(calls.every((call) => call.authorization === 'Bearer token-123')).toBe(true);
		expect(calls.map((call) => call.url)).toContain('https://treedx.example.test/api/v1/repos/repo_1/files/read');
	});
});
