...
describe('scene visual audit', () => {
	...
	it('discovers user-facing core, admin, tenant override, and content routes', () => {
		const root = projectRoot();
		const report = validateTreeseedScene({ projectRoot: root, scene: 'site-visual-audit' });
		const discovered = discoverTreeseedSceneVisualAuditRoutes({ projectRoot: root, scene: report.scene! });
	...
	it('formats a visual audit report grouped by path root', () => {
		...
		const paths: TreeseedSceneVisualAuditPaths = {
			auditRoot: root,
		...
		const markdown = formatTreeseedSceneVisualAuditMarkdown({ manifest, paths });
