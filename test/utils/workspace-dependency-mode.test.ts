...
describe('workspace dependency mode', () => {
	...
	it('detects workspace-link contamination in deployment lockfiles', () => {
		const root = createWorkspace();
		...
		expect(collectDeploymentLockfileWorkspaceIssues(root)).toHaveLength(1);
		expect(() => assertNoWorkspaceLinksInDeploymentLockfiles(root)).toThrow(/Deployment install resolved internal packages/u);
	});
});
