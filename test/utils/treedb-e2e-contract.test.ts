...
describe('TreeDB end-to-end SDK contract', () => {
	it('drives the Phase 10 route sequence with bearer auth', async () => {
		...
		expect(calls.map((call) => call.url)).toEqual([
			'https://treedb.example.test/api/v1/auth/whoami',
			'https://treedb.example.test/api/v1/policy/effective-scope?repoId=repo_1',
			'https://treedb.example.test/api/v1/registry/repos/repo_1/placement',
			...
			'https://treedb.example.test/api/v1/federation/query/plan',
			'https://treedb.example.test/api/v1/workspaces/ws_1/files?path=docs%2Freadme.md',
			'https://treedb.example.test/api/v1/workspaces/ws_1/exec',
			'https://treedb.example.test/api/v1/workspaces/ws_1/status',
			'https://treedb.example.test/api/v1/workspaces/ws_1/diff',
			'https://treedb.example.test/api/v1/workspaces/ws_1/commit',
	...
	it('maps content operations through TreeDB without a local repo root', async () => {
		const { client, calls } = clientWith([
		...
		const adapter = new TreeDbRepositoryAdapter({
			client,
			models: registry(),
	...
	it('maps graph adapter calls to remote graph and context APIs', async () => {
		const { client, calls } = clientWith([
			...
			{ ok: true, repoId: 'repo_1', graphVersion: 'graph_1', seedIds: [], nodes: [], edges: [], providerId: 'treedb-graph-mvp', diagnostics: {} },
	...
	it('preserves TreeDB authorization errors and maps hidden graph nodes to null', async () => {
		...
		const graph = new TreeDbGraphAdapter({ client: graphClient, repoId: 'repo_1' });
