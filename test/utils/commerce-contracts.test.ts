...
describe('commerce contracts', () => {
	...
	it('typechecks representative commerce contract records', () => {
		const product = {
			id: 'product_1',
			vendorId: 'vendor_1',
			sellerTeamId: 'team_seller',
			kind: 'template',
			slug: 'cooperative-starter',
			title: 'Cooperative Starter',
			summary: 'A governed starter product.',
			description: 'A starter product shaped around cooperative governance.',
			status: 'draft',
			visibility: 'public',
			catalogItemId: null,
			currentVersionId: null,
			ownershipModel: 'cooperative_owned',
			ownershipRecordId: 'ownership_1',
		...
		const ownership = {
			...
			canonicalOwnerId: 'coop_1',
			sellerTeamId: product.sellerTeamId,
			stewardTeamId: product.sellerTeamId,
			governancePolicyId: 'policy_1',
			publicSummary: 'Maintained by a cooperative steward team.',
			buyerVisible: true,
		...
		const successionEvent = {
			...
			stewardshipAssignmentId: null,
			successorType: 'team',
			successorId: 'team_successor',
			eventType: 'successor_named',
			status: 'submitted',
			reason: 'Named a successor steward.',
			evidence: {},
			effectiveAt: null,
			createdByType: 'user',
			createdById: 'user_1',
		...
		const workflow = {
			productId: product.id,
			currentOwnershipRecord: ownership,
		...
		const offer = {
			...
			termsSummary: 'Active subscribers receive updates while subscribed.',
			accessScope: {},
			supportScope: {},
			fulfillmentMode: 'automatic',
			activePriceId: null,
			stripeProductId: null,
			stripeProductStatus: 'not_synced',
			stripeProductSyncedAt: null,
			stripeProductSyncError: null,
		...
		const price = {
			...
			status: 'active',
			stripeProductId: 'prod_tree',
			stripePriceId: 'price_tree',
			stripeLookupKey: 'treeseed_test_price_1_v1',
			stripeSyncStatus: 'synced',
			stripeSyncedAt: '2026-06-14T00:00:00.000Z',
		...
		const serviceRequest = {
			...
			status: 'requested',
			requestedScope: 'Implement a governed scoped service.',
			approvedScope: null,
			accessNeeds: {},
			buyerVisibleSummary: null,
			vendorPrivateNotes: null,
			activeQuoteId: null,
		...
		const serviceQuote = {
			...
			sellerTeamId: serviceRequest.sellerTeamId,
			buyerTeamId: serviceRequest.buyerTeamId,
			buyerUserId: serviceRequest.buyerUserId,
			quoteVersion: 1,
			status: 'submitted',
			title: 'Scoped quote',
			scopeSummary: 'Scoped quote summary.',
			deliverables: [],
			assumptions: [],
			accessRequirements: {},
		...
		const capacityListing = {
			...
			supportedRegions: ['us'],
			runtimeRequirements: {},
			dataHandlingSummary: 'Buyer-provided data only.',
			buyerVisibleRiskSummary: 'Manual review before any access.',
			governanceRequirements: {},
			supportPolicy: 'Seller support policy.',
			availabilitySummary: 'Limited availability.',
			ownershipSnapshot: {},
			metadata: {},
			createdAt: '2026-06-14T00:00:00.000Z',
			updatedAt: '2026-06-14T00:00:00.000Z',
		...
		const capacityInquiry = {
			id: 'capacity_inquiry_1',
			listingId: capacityListing.id,
			productId: product.id,
			vendorId: product.vendorId,
			sellerTeamId: product.sellerTeamId,
			buyerTeamId: 'team_buyer',
			buyerUserId: null,
			status: 'requested',
			requestedServiceType: 'research_review',
			requestedScope: 'Evaluate a capacity engagement.',
			dataAccessRequested: {},
		...
		const marketplace = {
			products: [{
				...
				vendorId: product.vendorId,
				sellerTeamId: product.sellerTeamId,
				vendorDisplayName: 'Cooperative Seller',
				ownershipModel: product.ownershipModel,
				buyerVisibleOwnershipSummary: ownership.publicSummary,
				...
				offers: [{
					...
					capacityInquiryEligible: false,
					stripeSyncStatus: price.stripeSyncStatus,
				}],
				capacityListingId: capacityListing.id,
		...
		const refreshResponse = {
			paymentGroup,
			clientSecret: 'pi_test_secret_once',
		...
		const monitor = {
			...
			sellerTeamId: product.sellerTeamId,
			stripeReady: true,
			blockedStripeSyncCount: 0,
			driftedStripeSyncCount: 0,
			pendingFulfillmentCount: 0,
			failedRefundCount: 0,
			failedWebhookCount: 0,
			pendingServiceRequestCount: 0,
			pendingCapacityInquiryCount: 0,
			pendingGovernanceTransferCount: 0,
		...
		const connectedAccount = {
			...
			capabilities: {},
			onboardingStartedAt: '2026-06-14T00:00:00.000Z',
