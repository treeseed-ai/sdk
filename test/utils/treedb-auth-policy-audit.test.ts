...
describe('TreeDB auth policy and audit client methods', () => {
	it('calls auth mode, capabilities, grants, audit, and federation planner endpoints', async () => {
		...
		const client = new TreeDbClient({ baseUrl: 'https://treedb.example.test', token: 'token', fetch: fetchMock });
		...
		expect(calls.map((call) => call.url)).toContain('https://treedb.example.test/api/v1/auth/mode');
	...
	it('preserves connected auth and capability errors', async () => {
		...
		await expect(client.listCapabilities()).rejects.toMatchObject<TreeDbApiError>({
			status: 401,
