...
import {
	...
	type TreeseedSceneEnvironmentPrepareReport,
...
function okEnvironment(partial: Partial<TreeseedSceneEnvironmentPrepareReport> = {}): TreeseedSceneEnvironmentPrepareReport {
	return {
		ok: true,
		environment: 'local',
		readiness: { ok: true },
...
describe('scene Phase 3 environment integration', () => {
  ...
  dev:
    ...
    reuseExisting: true
`));
		const reused = await runTreeseedScene({
			projectRoot: root,
			scene: 'phase-three',
		...
		const started = await runTreeseedScene({
			projectRoot: root,
			scene: 'phase-three',
			browserAdapter: new FakeAdapter(),
...
`));
	...
	it('waits for explicit operations and fails timeout or failed statuses', async () => {
		const passed = await waitForTreeseedSceneOperation({
		...
		const failed = await waitForTreeseedSceneOperation({
			projectRoot: workspace(),
			scene: {} as any,
			environment: 'local',
			baseUrl: 'http://example.test',
