...
import { TREE_DB_CLIENT_OPERATION_MAP, TreeDbClient } from '../../src/treedb/index.ts';
...
describe('TreeDB SDK request contract', () => {
	it('maps public client methods to OpenAPI operation ids', () => {
		expect(TREE_DB_CLIENT_OPERATION_MAP).toMatchObject({
			whoami: 'getWhoami',
			ready: 'getReadiness',
	...
	it('constructs representative requests with documented methods, paths, auth, and bodies', async () => {
		...
		expect(calls.map((call) => [call.init.method, call.url])).toEqual([
			['GET', 'https://treedb.example.test/api/v1/auth/whoami'],
			['GET', 'https://treedb.example.test/api/v1/ready'],
			['GET', 'https://treedb.example.test/api/v1/health/deep'],
			['GET', 'https://treedb.example.test/api/v1/metrics'],
			['GET', 'https://treedb.example.test/metrics'],
			['GET', 'https://treedb.example.test/api/v1/repos/repo_1'],
			['POST', 'https://treedb.example.test/api/v1/repos/repo_1/workspaces'],
			['POST', 'https://treedb.example.test/api/v1/workspaces/ws_1/blobs/write'],
			['POST', 'https://treedb.example.test/api/v1/search'],
		]);
