...
import { TreeDbClient, TreeDbGraphAdapter } from '../../src/treedb/index.ts';
...
describe('TreeDB graph parity surface', () => {
	it('exposes refresh job metadata and adapter-compatible graph shapes', async () => {
		...
		const payloads = [
			...
			{ ok: true, repoId: 'repo_1', graphVersion: 'graph_1', seedIds: [], nodes: [{ node: { id: 'file:1', nodeType: 'File' }, score: 1, depth: 0, reasons: [] }], edges: [], providerId: 'treedb-graph-mvp' },
		...
		const adapter = new TreeDbGraphAdapter({ client, repoId: 'repo_1', defaultRef: 'refs/heads/main' });
		...
		await expect(adapter.queryGraph({ query: 'release' })).resolves.toMatchObject({ providerId: 'treedb-graph-mvp' });
		...
		expect(calls.map((call) => call.url)).toEqual([
			...
			'https://treedb.example.test/api/v1/repos/repo_1/graph/refresh-jobs/grjob_1',
			...
			'https://treedb.example.test/api/v1/repos/repo_1/graph/related',
		]);
	});
});
