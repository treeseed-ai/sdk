...
describe('TreeDbClient', () => {
	it('normalizes base URLs and sends bearer auth', async () => {
		const { client, calls } = mockClient([{ ok: true, status: 'ok', service: 'treedb-api' }]);
		expect(client.baseUrl).toBe('https://treedb.example.test');
		await client.health();
		expect(calls[0]?.url).toBe('https://treedb.example.test/api/v1/health');
	...
	it('maps core endpoint methods', async () => {
		...
		expect(calls.map((call) => call.url)).toEqual([
			...
			'https://treedb.example.test/api/v1/node',
	...
	it('constructs workspace, repository query, graph, and context routes', async () => {
		...
		expect(calls.map((call) => call.url)).toEqual([
			...
			'https://treedb.example.test/api/v1/workspaces/ws_1/tree?path=docs',
			'https://treedb.example.test/api/v1/workspaces/ws_1/files?path=docs%2Freadme.md',
			'https://treedb.example.test/api/v1/workspaces/ws_1/files?path=docs%2Freadme.md',
			'https://treedb.example.test/api/v1/repos/repo_1/query',
	...
	it('throws TreeDbApiError for API, non-json, network, and missing repo errors', async () => {
		const apiClient = new TreeDbClient({
			baseUrl: 'https://treedb.example.test',
			token: 'token',
			repoId: 'repo_1',
		...
		const badJsonClient = new TreeDbClient({
			baseUrl: 'https://treedb.example.test',
			token: 'token',
			repoId: 'repo_1',
		...
		const networkClient = new TreeDbClient({
			baseUrl: 'https://treedb.example.test',
			token: 'token',
			repoId: 'repo_1',
			fetch: (async () => {
		...
		const missingRepo = new TreeDbClient({
			baseUrl: 'https://treedb.example.test',
			token: 'token',
