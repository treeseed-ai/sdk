...
describe('TreeDxClient git remote and storage helpers', () => {
	it('pushes, fetches, checks mirror health, promotes mirror, compacts, and backs up storage', async () => {
		const { client, calls } = mockClient([
			...
			json({ ok: true, promotion: { mirrorId: 'mirror_1', repoId: 'repo_1', planOnly: true, status: 'planned' } }),
			json({ ok: true, compact: { status: 'ok', planOnly: true, backupId: null, files: [] } }),
			json({
				ok: true,
				backup: {
					backupId: 'backup_1',
		...
		await expect(
			client.push({ refspecs: ['refs/heads/main:refs/heads/main'], planOnly: true }),
		).resolves.toMatchObject({ backend: 'gix', status: 'plan' });
		await expect(client.fetchRemote({ planOnly: false })).resolves.toMatchObject({ fetch: { status: 'synced' } });
		await expect(client.checkMirrorHealth({ mirrorId: 'mirror_1' })).resolves.toMatchObject({
			health: { status: 'healthy' },
		});
		await expect(client.promoteMirror({ mirrorId: 'mirror_1', planOnly: true })).resolves.toMatchObject({
			promotion: { status: 'planned' },
		});
		await expect(client.compactStorage({ planOnly: true })).resolves.toMatchObject({ compact: { status: 'ok' } });
	...
	it('maps Git remote error envelopes to TreeDxApiError', async () => {
		...
		const error = await client
			.push({ refspecs: ['refs/heads/main:refs/heads/main'], planOnly: false })
