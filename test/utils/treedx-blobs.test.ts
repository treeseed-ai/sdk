...
describe('TreeDxClient blob helpers', () => {
	it('reads and writes JSON blob envelopes', async () => {
		...
		expect(calls.map((call) => call.url)).toEqual([
			'https://treedx.example.test/api/v1/repos/repo_1/blobs/read',
			'https://treedx.example.test/api/v1/workspaces/ws_1/blobs/write',
			'https://treedx.example.test/api/v1/workspaces/ws_1/blobs/delete',
		]);
	...
	it('downloads and uploads raw blob bytes', async () => {
		const { client, calls } = mockClient([
			binary(new Uint8Array([1, 2, 3]), {
				'x-treedx-content-hash': 'blake3:hash',
				'x-treedx-object-id': 'abc123',
				'x-treedx-source': 'workspace',
			}),
			json({
				ok: true,
				result: {
		...
		expect(calls[0]?.url).toBe(
			'https://treedx.example.test/api/v1/workspaces/ws_1/blobs/download?path=assets%2Flogo.bin',
		);
		expect(calls[1]?.url).toBe(
			'https://treedx.example.test/api/v1/workspaces/ws_1/blobs/upload?path=assets%2Fupload.bin',
		);
		expect((calls[1]?.init.headers as Record<string, string>)['x-treedx-expected-content-hash']).toBe(
			'blake3:expected',
		);
		expect(calls[1]?.init.body).toBeInstanceOf(Uint8Array);
	});

	it('maps blob error envelopes to TreeDxApiError', async () => {
	...
	it('calls multipart upload and artifact lifecycle endpoints', async () => {
		const { client, calls } = mockClient([
			...
			json({ ok: true, artifacts: [{ artifactId: 'artifact_1', snapshotId: 'snap_1', repoId: 'repo_1', format: 'tar.zst', size: 1, checksum: 'blake3:a', uri: 'treedx://artifact/snap_1' }] }),
			json({ ok: true, artifact: { artifactId: 'artifact_1', snapshotId: 'snap_1', repoId: 'repo_1', format: 'tar.zst', size: 1, checksum: 'blake3:a', uri: 'treedx://artifact/snap_1' } }),
			json({ ok: true, artifact: { artifactId: 'artifact_1', snapshotId: 'snap_1', repoId: 'repo_1', format: 'tar.zst', size: 1, checksum: 'blake3:a', uri: 'treedx://artifact/snap_1', status: 'deleted' } }),
		...
		expect(calls.map((call) => call.url)).toEqual([
			'https://treedx.example.test/api/v1/workspaces/ws_1/blobs/uploads',
			'https://treedx.example.test/api/v1/workspaces/ws_1/blobs/uploads/upload_1/parts/1',
			'https://treedx.example.test/api/v1/workspaces/ws_1/blobs/uploads/upload_1/complete',
			'https://treedx.example.test/api/v1/workspaces/ws_1/blobs/uploads/upload_2',
			'https://treedx.example.test/api/v1/repos/repo_1/artifacts',
			'https://treedx.example.test/api/v1/repos/repo_1/artifacts/artifact_1',
			'https://treedx.example.test/api/v1/repos/repo_1/artifacts/artifact_1',
			'https://treedx.example.test/api/v1/admin/artifacts/cleanup',
		]);
