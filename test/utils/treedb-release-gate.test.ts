...
describe("TreeDB release gate integration", () => {
  it("keeps package-local OpenAPI files for standalone SDK verification", () => {
    ...
    expect(
      read(resolve(sdkRoot, "scripts/generate-treedb-openapi-types.ts")),
  ...
  it("keeps package scripts wired to contract checks", () => {
    ...
    expect(pkg.scripts["treedb:check-types"]).toContain("--check");
    expect(pkg.scripts["treedb:contract"]).toContain(
      "treedb-openapi-contract.test.ts",
    );
