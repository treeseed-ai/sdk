...
import { TREE_DB_CLIENT_OPERATION_MAP } from '../../src/treedb/client.ts';

type OpenApiOperation = {
	operationId?: string;
	summary?: string;
	security?: unknown;
	'x-treedb-required-capabilities'?: unknown;
...
describe('TreeDB OpenAPI contract metadata', () => {
	it('uses unified contract language', () => {
		...
		const blocked = [`x-treedb-${rollout}`, `S${rollout.slice(1)} \\d`, `P${segment.slice(1)} \\d`, 'deferred-' + 'production'];
	...
	it('defines operation metadata and typed success schemas for every JSON route', () => {
		for (const { path, method, operation } of operations()) {
			...
			expect(operation['x-treedb-required-capabilities'], `${method.toUpperCase()} ${path}`).toBeDefined();
			...
			expect(schemaName, `${operation.operationId} must not use generic TreeDbOkEnvelope`).not.toBe('TreeDbOkEnvelope');
	...
	it('keeps SDK-mapped operations present', () => {
		...
		for (const [method, operationId] of Object.entries(TREE_DB_CLIENT_OPERATION_MAP)) {
