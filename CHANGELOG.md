...
- build(build): checkpoint release recovery fixes (96651d5c98cd)
...
- feat(source): allow retained production aliases during staging (f64ef470616d)

### Fixed

- fix(tests): prevent stale stage auto-resume after candidate heads (dffc0b2e623f)
- build(tests): deduplicate Railway API services and enforce canonical (425cd3a52533)
...
- feat(source): retain declared sibling operations runner resources (b635366c9ed8)

### Fixed

- build(build): reattach production Railway domains to exact desired (18c6a43d83a2)
...
- fix(source): Retry transient GitHub job setup failures once (d7cb7bd0fba1)
- build(workflow): Fix release readiness and merged branch cleanup (ae9d9af038ed)
- fix(tests): Require explicit workflow resume and prevent stale (589c5ef89268)

### Tests

- build(workflow): Delete merged feature branches after successful stage (98a9f62804fa)
- build(source): Permanently isolate Railway staging Git sources (14dbb439b6c0)

### Dependencies

- build(workflow): Avoid repeated fetches during merged branch cleanup (72dd77dd42a4)
...
- build(build): preserve registry dependency lock semantics (067ffb487ab7)
...
- feat(source): fix Railway Dockerfile hosted build command verification (cf8e10ae6572)

### Fixed

- fix(deps): complete starter, api guarantee, and agent live reliability (29f38990e611)

### Tests

- test(tests): complete starter, api guarantee, and agent live (355a84ee6d9f)
- build(deps): complete starter, api guarantee, and agent live (39ce9056550c)
...
- build(build): fix production source cache purge finalization (3d0b683b09da)
