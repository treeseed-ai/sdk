...
export function exportGuaranteesJson(input: { registry: GuaranteeRegistryReport; filter?: GuaranteeFilter }) {
	return {
		schemaVersion: 'treeseed.guarantees.export/v1',
		generatedAt: new Date().toISOString(),
		workspaceRoot: input.registry.workspaceRoot,
		guarantees: filterGuarantees({ guarantees: input.registry.guarantees, filter: input.filter, includeDependencies: false }).map((entry) => ({
...
export function exportGuaranteesMarkdown(input: { registry: GuaranteeRegistryReport; filter?: GuaranteeFilter }) {
	const rows = filterGuarantees({ guarantees: input.registry.guarantees, filter: input.filter, includeDependencies: false });
	return [
		'# TreeSeed Guarantees',
		'',
...
export function writeGuaranteesExport(input: { workspaceRoot: string; format: 'csv' | 'json' | 'markdown'; output: string; filter?: GuaranteeFilter }) {
	...
	const content = input.format === 'csv'
		? exportGuaranteesCsv({ guarantees: registry.guarantees, filter: input.filter })
		: input.format === 'json'
			? `${JSON.stringify(exportGuaranteesJson({ registry, filter: input.filter }), null, 2)}\n`
			: exportGuaranteesMarkdown({ registry, filter: input.filter });
	writeFileSync(outputPath, content, 'utf8');
	return { ok: registry.ok, outputPath, registry };
}

export function verifierDefinitionsByRef(registries: LoadedGuaranteeVerifierRegistry[]) {
	const definitions = new Map<string, GuaranteeVerifierResolution>();
...
export function resolveGuaranteeVerifierRefs(input: {
	refs: string[];
	verifierRegistries: LoadedGuaranteeVerifierRegistry[];
	status?: GuaranteeStatus;
	sourcePath?: string;
}): GuaranteeVerifierResolutionReport {
...
export async function writeCommandEvidence(input: {
	...
	const safeRef = slugifyGuaranteeJourney(input.ref);
	...
	try {
		...
		return {
			status: 'passed' as GuaranteeRunStatus,
		...
		return {
			status: 'failed' as GuaranteeRunStatus,
...
export function validateVitestVerifierOutput(result: { stdout: string; stderr: string }) {
