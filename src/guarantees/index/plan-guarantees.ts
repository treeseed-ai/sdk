...
export function planGuarantees(input: { workspaceRoot: string; filter?: GuaranteeFilter; environment?: string; includeDependencies?: boolean }): GuaranteePlanReport {
	...
	const selectedWithoutDeps = filterGuarantees({ guarantees: registry.guarantees, filter: input.filter, includeDependencies: false });
	const graph = buildGuaranteeDependencyGraph({ guarantees: registry.guarantees, filter: input.filter, includeDependencies: input.includeDependencies !== false });
...
export function auditGuaranteeJourneys(input: { workspaceRoot: string; filter?: GuaranteeFilter; writeReport?: string; now?: Date }): GuaranteeJourneyAudit {
	const workspaceRoot = resolve(input.workspaceRoot);
	const registry = discoverGuarantees({ workspaceRoot, filter: input.filter });
	...
	const graph = buildGuaranteeDependencyGraph({ guarantees: registry.guarantees, filter: input.filter, includeDependencies: true });
	const items: GuaranteeJourneyAuditItem[] = [];
	for (const entry of valid.filter((candidate) => selectedByFilter(candidate.manifest, input.filter))) {
		...
		const classification: GuaranteeJourneyAuditItem['classification'] = !routeExists
		...
		const requiredAction: GuaranteeJourneyAuditItem['requiredAction'] = classification === 'missing-product-route'
	...
	const audit: GuaranteeJourneyAudit = {
		schemaVersion: GUARANTEE_JOURNEY_AUDIT_SCHEMA_VERSION,
		workspaceRoot,
