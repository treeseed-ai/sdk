...
import type { TreeseedDesiredEnvironment, TreeseedDesiredResource } from './desired-state.ts';

export const TREESEED_LOCAL_CONTENT_MODES = ['auto', 'none', 'preview', 'edit'] as const;

export type TreeseedLocalContentMode = typeof TREESEED_LOCAL_CONTENT_MODES[number];

export type TreeseedLocalContentMaterializationStatus =
	| 'not_requested'
	| 'existing_path_ready'
...
function requestedMaterialization(architecture: SeedProjectArchitecture, mode: TreeseedLocalContentMode): SeedLocalContentMaterialization {
...
export function buildProjectLocalContentResources({
	...
	return seed.plan.actions.flatMap((action) => {
		...
		const materializationStatus: TreeseedLocalContentMaterializationStatus = materialization === 'none'
			? 'not_requested'
			: materialization === 'existing_path'
		...
		} satisfies TreeseedDesiredResource];
	});
}
