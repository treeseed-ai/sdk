...
export type ContentRuntimeDiagnosticStatus = 'ready' | 'missing' | 'site_not_prepared' | 'unsupported_structure';

export type ContentRuntimeDiagnostic = {
	code: string;
	status: ContentRuntimeDiagnosticStatus;
	source: ContentRuntimeMode;
	summary: string;
	relativePath?: string;
};

export type LocalContentRuntimeSummary = {
...
export type R2ContentRuntimeMetadata = {
	manifestKey?: string | null;
	overlayKey?: string | null;
	revision?: string | null;
};

export type TreeDxContentRuntimeMetadata = {
	libraryId?: string | null;
	repositoryId?: string | null;
	snapshotId?: string | null;
};

export type ContentRuntimeResolution = {
	contentRuntimeSource: SeedContentRuntimeSource;
	effectiveContentSource: EffectiveContentSource;
	...
	revision: string | null;
	snapshotId: string | null;
	diagnostics: ContentRuntimeDiagnostic[];
...
function contentRuntimeMode(source: EffectiveContentSource): ContentRuntimeMode {
...
function localContentRequested(architecture: Pick<SeedProjectArchitecture, 'contentRuntimeSource'>, local?: LocalContentRuntimeSummary | null) {
...
export function resolveContentRuntimeSource(input: {
	architecture: Pick<SeedProjectArchitecture, 'contentRuntimeSource' | 'contentPublishTarget'>;
	local?: LocalContentRuntimeSummary | null;
	r2?: R2ContentRuntimeMetadata | null;
	treeDx?: TreeDxContentRuntimeMetadata | null;
	includeLocalPath?: boolean;
}): ContentRuntimeResolution {
	const diagnostics: ContentRuntimeDiagnostic[] = [];
	...
	const source: EffectiveContentSource = localSelected ? 'local_directory' : input.architecture.contentRuntimeSource ?? 'r2_published_manifest';
...
export function inspectContentStructure(input: {
	projectRoot: string;
...
export function contentRuntimeMetadataFromTarget(target: unknown): {
	r2: R2ContentRuntimeMetadata;
	treeDx: TreeDxContentRuntimeMetadata;
} {
