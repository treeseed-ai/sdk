...
export type DesiredResourceEdge = {
	from: string;
	to: string;
	reason: 'depends-on' | 'releases' | 'hosts';
};

export type DesiredResourceGraph = {
	workspaceId: string;
	environment: DesiredEnvironment;
	packages: PackageUnit[];
	templates: TemplateUnit[];
	resources: DesiredResource[];
	edges: DesiredResourceEdge[];
...
export function reconcileIdentityForGraph(workspaceId: string, environment: DesiredEnvironment): DesiredUnit['identity'] {
	return {
		teamId: workspaceId,
...
export function environmentFromTarget(target: ReconcileTarget): DesiredEnvironment {
	if (target.kind === 'persistent') return target.scope;
	return 'staging';
}

export function packageReleaseCapability(adapter: PackageAdapter): PackageUnit['releaseCapability'] {
...
export function packageUnitFromAdapter(adapter: PackageAdapter): PackageUnit {
	return {
		id: adapter.id,
		name: adapter.name,
		kind: adapter.kind,
...
export function packageRequiredSecretsForGitHubEnvironment(adapter: PackageAdapter, environmentName: string) {
...
export function packageRequiredVariablesForGitHubEnvironment(adapter: PackageAdapter, _environmentName: string) {
...
export function packageUnitRequiredSecretsForGitHubEnvironment(pkg: PackageUnit, environmentName: string) {
	const isProduction = environmentName === 'production';
	return pkg.requiredSecrets.filter((secretName) => isProduction || !PRODUCTION_ONLY_GITHUB_SECRET_NAMES.has(secretName));
}

export function templateReleaseTag(manifest: TemplateRepositoryManifest) {
	return manifest.version ? `${manifest.release.tagPrefix}${manifest.id}/v${manifest.version}` : null;
}

export function templateUnitFromRepository(repo: ReturnType<typeof checkedOutTemplateRepositories>[number]): TemplateUnit {
...
export function packageShortSha(adapter: PackageAdapter) {
...
export function materializeDockerImageTags(tags: string[], adapter: PackageAdapter, branch: string) {
