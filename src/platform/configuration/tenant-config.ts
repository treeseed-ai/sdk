...
const manifestFieldAliases: FieldAliasRegistry = {
	siteConfigPath: { key: 'siteConfigPath', aliases: ['site_config_path'] },
};

const manifestContentFieldAliases: FieldAliasRegistry = {
...
const manifestOverrideFieldAliases: FieldAliasRegistry = {
...
export function defineTenant<T>(tenantConfig: T): T {
	return tenantConfig;
}
	...
	const tenantConfig = defineTenant({
		...
		overrides: overrides
			? {
			  ...
			  } satisfies NonNullable<TenantConfig['overrides']>
			: undefined,
	}) as TenantConfig;
...
export function getTenantContentRoot(
	...
	const root = tenantConfig.content[collectionName as keyof TenantConfig['content']];
	if (!root) {
...
export function tenantFeatureEnabled(
	tenantConfig: Pick<TenantConfig, 'features'>,
	featureName: string,
) {
	return tenantConfig.features?.[featureName] !== false;
}

const MODEL_FEATURE_MAP: Partial<Record<ContentCollection, FeatureName>> = {
	docs: 'docs',
	books: 'books',
	notes: 'notes',
	questions: 'questions',
...
export function tenantModelRendered(
	tenantConfig: Pick<TenantConfig, 'features' | 'site'>,
	modelName: ContentCollection,
) {
