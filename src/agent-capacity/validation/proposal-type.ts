...
export const PROPOSAL_TYPE_ID_PATTERN = /^[a-z][a-z0-9-]*$/u;
...
export function validateProposalTypeContract(value: unknown): ProposalTypeValidation {
	...
	if (typeof value.id === 'string' && !PROPOSAL_TYPE_ID_PATTERN.test(value.id)) diagnostics.push({ code: 'proposal_type_id_invalid', path: 'id', message: 'id must use lowercase kebab-case.' });
