...
export function validateProjectAgentClassConfiguration(value: unknown): CapacityConfigurationValidation {
	...
	const allowed = new Set(['id', 'slug', 'name', 'status', 'allowedModes', 'kernelProfile', 'kernelPolicy', 'handlerRefs', 'outputContracts', 'metadata']);
...
export type ProjectAgentClassConfiguration = Pick<ProjectAgentClass, 'id' | 'slug' | 'name' | 'status' | 'allowedModes' | 'kernelProfile' | 'kernelPolicy' | 'handlerRefs' | 'outputContracts' | 'metadata'>;
