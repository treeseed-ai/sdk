...
export const capacityLedgerEntries = pgTable('capacity_ledger_entries', {
	...
	foreignKey({ name: 'fk_capacity_ledger_execution_provider', columns: [table.capacityProviderId, table.executionProviderId], foreignColumns: [capacityExecutionProviders.capacityProviderId, capacityExecutionProviders.id] }).onDelete('restrict'),
	foreignKey({ name: 'fk_capacity_ledger_lane', columns: [table.capacityProviderId, table.laneId], foreignColumns: [capacityProviderLanes.capacityProviderId, capacityProviderLanes.id] }).onDelete('restrict'),
	...
	check('chk_capacity_ledger_lane_purpose', sql`${table.lanePurpose} IS NULL OR ${table.lanePurpose} IN ('communication','operation')`),
	check('chk_capacity_ledger_overflow', sql`${table.communicationOverflow} IN (0,1)`),
	check('chk_capacity_ledger_execution_kind', sql`${table.executionKind} IN ('workday','conversation','simulation','recovery')`),
	check('chk_capacity_ledger_trigger_kind', sql`${table.triggerKind} IN ('scheduled','manual','discussion','agent-handoff')`)
]);
...
export const capacityUsageActuals = pgTable('capacity_usage_actuals', {
	...
	sourceMessageRefsJson: text('source_message_refs_json').notNull().default('[]'),
	operationHandoffId: text('operation_handoff_id'),
...
export const capacityProviderAssignments = pgTable('capacity_provider_assignments', {
	...
	check('chk_capacity_provider_assignments_lane_purpose', sql`${table.lanePurpose} IS NULL OR ${table.lanePurpose} IN ('communication','operation')`),
	check('chk_capacity_provider_assignments_overflow', sql`${table.communicationOverflow} IN (0,1)`),
	check('chk_capacity_provider_assignments_handoff_depth', sql`${table.handoffDepth} >= 0`),
	check('chk_capacity_provider_assignments_execution_kind', sql`${table.executionKind} IN ('workday','conversation','simulation','recovery')`),
	check('chk_capacity_provider_assignments_trigger_kind', sql`${table.triggerKind} IN ('scheduled','manual','discussion','agent-handoff')`),
