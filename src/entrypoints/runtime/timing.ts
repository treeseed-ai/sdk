...
export function summarizeSlowestTimings(entries: TimingEntry[], limit = 8) {
	return flattenTimings(entries)
		.sort((left, right) => right.durationMs - left.durationMs)
		.slice(0, Math.max(0, limit));
}

export function flattenTimings(entries: TimingEntry[]): TimingEntry[] {
	const flattened: TimingEntry[] = [];
	const visit = (entry: TimingEntry) => {
...
export function formatTimingSummary(entries: TimingEntry[], { title = 'Provider deploy timing summary', limit = 12 } = {}) {
...
export function formatTimingMarkdown(entries: TimingEntry[], { title = 'Provider deploy timing summary', limit = 20 } = {}) {
	const slowest = summarizeSlowestTimings(entries, limit);
