...
function resolveCopilotAuthToken(env: NodeJS.ProcessEnv) {
	return resolveGitHubCopilotToken(env)
...
export async function runCopilotTask(input: CopilotTaskInput): Promise<CopilotTaskResult> {
	const cwd = input.cwd ?? process.cwd();
	const baseEnv = createManagedToolEnv(input.env ?? process.env);
	...
	const env = copilotAuthToken
		...
		: baseEnv;
	const cliPath = resolveToolBinary('copilot', { env });
	if (!cliPath) {
		return {
			status: 'failed',
