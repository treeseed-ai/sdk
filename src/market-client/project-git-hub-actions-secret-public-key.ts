...
export function projectGitHubActionsSecretPublicKeyMethod(this: MarketClient, projectId: string, input: {
    ...
    if (input.installationId)
        query.set('installationId', String(input.installationId));
    query.set('repository', input.repository);
    if (input.scope)
        query.set('scope', String(input.scope));
    if (input.environment)
        query.set('environment', String(input.environment));
    return this.request<{
        ok: true;
        payload: TreeseedGitHubActionsSecretPublicKeyMetadata;
    }>(`/v1/projects/${encodeURIComponent(projectId)}/secrets/github-actions/public-key?${query}`, { requireAuth: true });
}
