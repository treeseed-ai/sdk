...
export function deployProjectGitHubActionsSecretMethod(this: MarketClient, projectId: string, body: TreeseedGitHubActionsEncryptedSecretDeployment & Record<string, unknown>) {
    return this.request<{
        ok: true;
        payload: unknown;
    }>(`/v1/projects/${encodeURIComponent(projectId)}/secrets/github-actions/deploy`, { method: 'POST', body, requireAuth: true });
}
