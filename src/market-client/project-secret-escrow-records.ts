...
export function projectSecretEscrowRecordsMethod(this: MarketClient, projectId: string, filters: {
    ...
    if (filters.secretId)
        query.set('secretId', String(filters.secretId));
    if (filters.status)
    ...
    return this.request<{
        ok: true;
        payload: unknown[];
    }>(`/v1/projects/${encodeURIComponent(projectId)}/secrets/escrow${query.toString() ? `?${query}` : ''}`, { requireAuth: true });
}
