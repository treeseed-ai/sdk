...
export function cancelProjectDeploymentMethod(this: MarketClient, projectId: string, deploymentId: string, body: Record<string, unknown> = {}) {
    return this.request<{
        ok: true;
        deployment: ProjectDeployment;
        cancellation: 'completed' | 'requested' | string;
    }>(`/v1/projects/${encodeURIComponent(projectId)}/deployments/${encodeURIComponent(deploymentId)}/cancel`, { method: 'POST', body, requireAuth: true });
}
