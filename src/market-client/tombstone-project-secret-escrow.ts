...
export function tombstoneProjectSecretEscrowMethod(this: MarketClient, projectId: string, escrowId: string) {
    return this.request<{
        ok: true;
        payload: unknown;
    }>(`/v1/projects/${encodeURIComponent(projectId)}/secrets/escrow/${encodeURIComponent(escrowId)}`, { method: 'DELETE', requireAuth: true });
}
