...
export function capacityGrantsMethod(this: MarketClient, teamId: string, page: {
    limit?: number;
    cursor?: string;
} = {}) {
    ...
    }>(`/v1/teams/${encodeURIComponent(teamId)}/capacity-grants${query.size ? `?${query}` : ''}`, { requireAuth: true });
}
