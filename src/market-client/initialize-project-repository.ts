...
export function initializeProjectRepositoryMethod(this: MarketClient, projectId: string, role: string, body: Record<string, unknown> = {}) {
    return this.request<{
        ok: true;
        operation: Record<string, unknown>;
    }>(`/v1/projects/${encodeURIComponent(projectId)}/repositories/${encodeURIComponent(role)}/initialize`, { method: 'POST', body, requireAuth: true });
}
