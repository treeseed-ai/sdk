...
export function projectDeploymentMethod(this: MarketClient, projectId: string, deploymentId: string) {
    return this.request<{
        ok: true;
        payload: ProjectDeployment;
    }>(`/v1/projects/${encodeURIComponent(projectId)}/deployments/${encodeURIComponent(deploymentId)}`, { requireAuth: true });
}
