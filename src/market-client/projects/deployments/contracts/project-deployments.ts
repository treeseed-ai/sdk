...
export function projectDeploymentsMethod(this: MarketClient, projectId: string, filters: ProjectDeploymentListFilters = {}) {
    const query = new URLSearchParams();
    if (filters.environment)
        query.set('environment', String(filters.environment));
    if (filters.action)
        query.set('action', String(filters.action));
    if (filters.status)
    ...
    }>(`/v1/projects/${encodeURIComponent(projectId)}/deployments${suffix}`, { requireAuth: true });
}
