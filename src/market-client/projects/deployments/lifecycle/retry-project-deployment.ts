...
export function retryProjectDeploymentMethod(this: MarketClient, projectId: string, deploymentId: string, body: Record<string, unknown> = {}) {
    return this.request<{
        ok: true;
        originalDeployment: ProjectDeployment;
        retryDeployment: ProjectDeployment;
        operation: Record<string, unknown>;
    }>(`/v1/projects/${encodeURIComponent(projectId)}/deployments/${encodeURIComponent(deploymentId)}/retry`, { method: 'POST', body, requireAuth: true });
}
