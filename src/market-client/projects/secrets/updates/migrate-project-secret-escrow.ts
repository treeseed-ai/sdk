...
export function migrateProjectSecretEscrowMethod(this: MarketClient, projectId: string, escrowId: string, body: Record<string, unknown>) {
    return this.request<{
        ok: true;
        payload: unknown;
    }>(`/v1/projects/${encodeURIComponent(projectId)}/secrets/escrow/${encodeURIComponent(escrowId)}/migrate`, { method: 'POST', body, requireAuth: true });
}
