...
export function deleteTeamMethod(this: MarketClient, teamId: string, confirmation: string) {
    return this.request<{
        ok: boolean;
        team?: {
            id: string;
        ...
        code?: string;
        message?: string;
    }>(`/v1/teams/${encodeURIComponent(teamId)}`, { method: 'DELETE', body: { confirmation }, requireAuth: true });
}
