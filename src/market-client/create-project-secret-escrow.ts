...
export function createProjectSecretEscrowMethod(this: MarketClient, projectId: string, body: Record<string, unknown>) {
    return this.request<{
        ok: true;
        payload: unknown;
    }>(`/v1/projects/${encodeURIComponent(projectId)}/secrets/escrow`, { method: 'POST', body, requireAuth: true });
}
