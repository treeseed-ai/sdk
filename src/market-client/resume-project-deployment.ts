...
export function resumeProjectDeploymentMethod(this: MarketClient, projectId: string, deploymentId: string, body: Record<string, unknown> = {}) {
    return this.request<Record<string, unknown>>(`/v1/projects/${encodeURIComponent(projectId)}/deployments/${encodeURIComponent(deploymentId)}/resume`, { method: 'POST', body, requireAuth: true });
}
