...
export function replaceProjectHostMethod(this: MarketClient, projectId: string, requirementKey: string, body: Record<string, unknown> = {}) {
    return this.request<{
        ok: true;
        payload: Record<string, unknown>;
        operation: Record<string, unknown>;
    }>(`/v1/projects/${encodeURIComponent(projectId)}/hosts/${encodeURIComponent(requirementKey)}/replace`, { method: 'POST', body, requireAuth: true });
}
