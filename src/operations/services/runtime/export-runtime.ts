...
function resolveConfiguredBundlePaths(directory: string, tenantRoot: string, config: DeployConfig) {
...
function resolveIgnorePatterns(config: DeployConfig) {
	return [
		'.treeseed/exports',
		'.treeseed/exports/**',
		'**/.treeseed/exports',
...
export async function exportCodebase({
	...
	const tenantRoot = findNearestRoot(resolvedDirectory);
	if (!tenantRoot) {
	...
	const deployConfig = loadDeployConfigFromPath(resolveDeployConfigPathFromRoot(tenantRoot));
