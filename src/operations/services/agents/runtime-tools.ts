...
const RuntimeRoot = resolve(packageRootFromSource, 'src', 'treeseed');
...
export const runtimeRoot = RuntimeRoot;
...
export function WorkspacePackageCheckoutState(root = resolve(packageRoot, '..')) {
...
function assertUsablePackageCheckout(fallbackDirName?: string) {
	if (!fallbackDirName) {
		return;
	}
	const state = WorkspacePackageCheckoutState();
	const rootHasSubmodules = existsSync(resolve(packageRoot, '..', '..', '.gitmodules'));
	if (state.mode !== 'partial' || !rootHasSubmodules) {
		return;
	}
...
function resolveInstalledPackageRoot(packageName, exportPath?: string, fallbackDirName?: string) {
	assertUsablePackageCheckout(fallbackDirName);
	if (fallbackDirName) {
...
export const corePackageRoot = resolveInstalledPackageRoot('@treeseed/core', '@treeseed/core/config', 'core');
export const agentPackageRoot = resolveInstalledPackageRoot('@treeseed/agent', '@treeseed/agent', 'agent');
export const sdkPackageRoot = resolveInstalledPackageRoot('@treeseed/sdk', '@treeseed/sdk', 'sdk');
...
export function loadCliDeployConfig(tenantRoot) {
	...
	return loadDeployConfigFromPath(configPath);
}
