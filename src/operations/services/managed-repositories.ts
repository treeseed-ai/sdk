...
import { currentBranch, gitStatusPorcelain, originRemoteUrl, repoRoot } from './workspace-save.ts';
import { hasCompleteTreeseedPackageCheckout, sortWorkspacePackages, workspacePackages } from './workspace-tools.ts';
import { discoverTreeseedPackageAdapters } from './package-adapters.ts';

export type TreeseedManagedRepositoryKind = 'root' | 'package' | 'template' | 'fixture' | 'project';

export type TreeseedTemplateRepositoryManifest = {
	id: string;
	name: string;
	category: string;
...
export type TreeseedManagedRepository = {
	id: string;
	name: string;
	kind: TreeseedManagedRepositoryKind;
	dir: string;
	relativeDir: string;
	...
	templateManifest: TreeseedTemplateRepositoryManifest | null;
};
...
export function readTreeseedTemplateRepositoryManifest(repoDir: string): TreeseedTemplateRepositoryManifest | null {
...
function classifyManagedRepo(root: string, relativeDir: string, repoDir: string): TreeseedManagedRepositoryKind {
	if (relativeDir === '.') return 'root';
	if (/^starters\/[^/]+$/u.test(relativeDir) || readTreeseedTemplateRepositoryManifest(repoDir)) return 'template';
...
export function discoverTreeseedManagedRepositories(root: string): TreeseedManagedRepository[] {
	...
	return [...repos.entries()]
		.map(([relativeDir, dir]) => {
			...
			const templateManifest = kind === 'template' ? readTreeseedTemplateRepositoryManifest(dir) : null;
			...
			return {
				...
				templateManifest,
			} satisfies TreeseedManagedRepository;
		})
...
export function checkedOutManagedWorkflowRepos(root: string) {
	return discoverTreeseedManagedRepositories(root).filter((repo) => repo.kind !== 'root');
}

export function checkedOutTemplateRepositories(root: string) {
	return discoverTreeseedManagedRepositories(root).filter((repo) => repo.kind === 'template' && repo.templateManifest);
}
