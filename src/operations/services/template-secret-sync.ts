...
const DEFAULT_SCOPES: TreeseedConfigScope[] = ['staging', 'prod'];
...
function normalizeScopes(items: ProjectLaunchSecretDeploymentPlanItem[], requested?: TreeseedConfigScope[]) {
	const scopes = new Set<TreeseedConfigScope>(requested?.length ? requested : DEFAULT_SCOPES);
...
export function resolveProjectLaunchSecretValueOverlay(
	...
	for (const item of planItems) {
		...
		const itemScopes = (item.scopes ?? []).filter((scope): scope is TreeseedConfigScope => scope === 'local' || scope === 'staging' || scope === 'prod');
		if (targets.length === 0) {
