...
import { RepoBranchMode, RepositorySaveNode, RepositorySaveOptions, RepositorySaveReport, SaveState, emitProgress, readJson } from '../support/repo-kind.ts';
import { packageVersionEligibleForBranch, remoteBranchCommitSafe } from '../support/classify-repo-kind.ts';
import { assertTagStateMatchesHead, ensurePackageTagReady, ensureRemoteAccessBeforeVerification, finalizePackageReference, tagState } from '../support/tag-state.ts';
import { checkoutCommandFor, remoteBranchExistsSafe } from '../repositories/discover-repository-save-nodes.ts';
import { hasNpmLockfile } from '../support/has-staged-changes.ts';
import { hasScript, manifestVerifyCommand, runNpmInstallWithRetry, validateRepositoryLockfile } from '../treedx/repositories/sync-root-workspace-lockfile-metadata.ts';
