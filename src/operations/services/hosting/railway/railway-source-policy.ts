export type RailwaySourcePolicyScope = 'local' | 'staging' | 'prod';

export type RailwaySourcePolicyService = {
	key?: string | null;
...
export type RailwaySourceIdentity = RailwaySourcePolicyService & {
...
export function railwayEnvironmentQualifiedServiceName(
	serviceName: string,
	scope: RailwaySourcePolicyScope | string,
) {
...
function railwaySourceSignature(service: RailwaySourceIdentity) {
...
export function assertNoRailwaySourceIdentityCollisions(services: RailwaySourceIdentity[]) {
	const identities = new Map<string, RailwaySourceIdentity>();
...
export function isApiRailwaySourcePolicyService(service: RailwaySourcePolicyService) {
...
export function apiRailwayDefaultSourceRepo(service: RailwaySourcePolicyService) {
	const serviceName = String(service.serviceName ?? '').trim();
...
export function apiRailwayDefaultDockerfilePath(service: RailwaySourcePolicyService) {
	const serviceName = String(service.serviceName ?? '').trim();
...
export function assertApiRailwaySourcePolicy(
	scope: RailwaySourcePolicyScope | string,
	service: RailwaySourcePolicyService,
) {
