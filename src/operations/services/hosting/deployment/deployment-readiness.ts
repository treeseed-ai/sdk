...
export type DeploymentReadinessStatus = 'passed' | 'failed' | 'warning' | 'skipped';
...
function summary(checks: DeploymentReadinessCheck[]) {
	return {
...
function checkIncludes(id: string, values: unknown, required: string[], message: string, remediation: string): DeploymentReadinessCheck {
...
function hasLocalDevService(adapter: PackageAdapter, serviceId: string) {
...
function discoveredApiPackageRoot(tenantRoot: string) {
	const adapters = discoverPackageAdapters(tenantRoot);
...
export function collectDeploymentReadiness(options: DeploymentReadinessOptions): DeploymentReadinessReport {
	const tenantRoot = resolve(options.tenantRoot);
	const environment = options.environment;
	const graph = compileHostingGraph({ tenantRoot, environment, appId: options.appId });
...
export function formatReadinessReport(report: DeploymentReadinessReport) {
	const lines = [
