...
export function recoverGitLocks(cwd: string, { execute = false, all = false }: { execute?: boolean; all?: boolean } = {}): GitLockDiagnostic | GitWorkspaceLockDiagnostics {
	if (all) {
		const repositories = workspaceGitRepositories(cwd).map((repo) => ({
			...
			locks: recoverGitLocks(repo, { execute, all: false }) as GitLockDiagnostic[],
		}));
...
export function repairSafeLocks(cwd: string) {
	const diagnostics = inspectGitLocks(cwd);
	const repaired: GitLockDiagnostic[] = [];
...
export function gitLockFailure(output: GitRunnerResult) {
...
export function gitLockErrorMessage(cwd: string, args: string[], diagnostics: GitLockDiagnostic[], original: GitRunnerResult, retry?: GitRunnerResult) {
...
export function runRepositoryGit(args: string[], options: RunOptions): GitRunnerResult {
...
export function runGitText(args: string[], options: RunOptions): string {
	return runRepositoryGit(args, options).stdout.trim();
}

export function runGitOk(args: string[], options: Omit<RunOptions, 'allowFailure'>): boolean {
	return runRepositoryGit(args, { ...options, allowFailure: true }).status === 0;
}

export async function runGitBatch(
	operations: GitBatchOperation[],
	_options: { repoSerialization?: boolean } = {},
): Promise<GitRunnerResult[]> {
	const results: GitRunnerResult[] = [];
	for (const operation of operations) {
		results.push(runRepositoryGit(operation.args, operation));
	}
	return results;
}
