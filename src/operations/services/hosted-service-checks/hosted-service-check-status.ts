...
export type HostedServiceTarget = 'local' | 'staging' | 'prod';

export type HostedServiceProvider = 'cloudflare' | 'railway' | 'http' | 'dns' | 'github';

export type HostedServiceType =
	| 'web'
	| 'api'
	| 'operationsRunner'
...
export interface HostedServiceCheck {
	id: string;
	provider: HostedServiceProvider;
	serviceKey?: string;
	...
	status: HostedServiceCheckStatus;
	issues: string[];
	remediation?: string;
}

export interface HostedServiceCheckReport {
	target: HostedServiceTarget;
	tenantRoot: string;
	generatedAt: string;
	summary: {
	...
	checks: HostedServiceCheck[];
}

export interface ObservedRailwayServiceState {
...
export interface HostedServiceCheckOptions {
	tenantRoot: string;
	target?: HostedServiceTarget;
	appId?: string;
	serviceKeys?: string[];
	...
	observedRailwayServices?: Record<string, ObservedRailwayServiceState | undefined>;
...
export function check(input: Omit<HostedServiceCheck, 'issues'> & { issues?: string[] }): HostedServiceCheck {
	return {
		...input,
		issues: input.issues ?? [],
	};
...
export function variableObserved(observed: ObservedRailwayServiceState | undefined, key: string) {
	return hasProviderKey(observed?.variables, key) || hasProviderKey(observed?.secrets, key);
}

export function valuePresence(values: Record<string, string | undefined>, observed: ObservedRailwayServiceState | undefined, key: string, serviceTarget: string) {
...
export function serviceTypeFor(key: string): HostedServiceType {
...
export function observedFor(options: HostedServiceCheckOptions, serviceName: string | null | undefined) {
	return serviceName ? options.observedRailwayServices?.[serviceName] : undefined;
}

export function webCheckConfig(deployConfig: Record<string, any>, selectedApplication: ReturnType<typeof discoverApplications>[number] | null) {
...
export function httpStatus(url: string, options: HostedServiceCheckOptions) {
