...
export async function repairHostingAfterSuccessfulDeploy(options: ProjectPlatformActionOptions, systems: RunnableBootstrapSystem[]) {
	...
	const audit = await runHostingAudit({
	...
	const repaired = await runHostingAudit({
		tenantRoot: options.tenantRoot,
		environment,
		repair: true,
		env,
