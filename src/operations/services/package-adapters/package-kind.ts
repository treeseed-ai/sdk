...
export type PackageAdapter = {
	...
	verifyCommands: {
		fast: PackageCommand | null;
		local: PackageCommand | null;
		release: PackageCommand | null;
	};
	artifacts: Array<{
		provider: 'npm' | 'docker';
		name: string;
...
export type PackageImageWorkflowOptions = {
	root?: string;
	packageId: string;
	branch?: string | null;
...
export type PackageWorkflowTemplateKind = 'npm-publish' | 'docker-image' | 'release-gate';

export type PackageWorkflowSyncResult = {
	packageId: string;
	path: string;
	workflow: string;
	template: PackageWorkflowTemplateKind;
	exists: boolean;
	changed: boolean;
	written: boolean;
};
...
export function commandFromScript(dir: string, script: unknown, label: string): PackageCommand | null {
...
export function normalizePackageProjectArchitecture(value: unknown, packageId: string): SeedProjectArchitecture | null {
...
export function docsSiteReadiness(dir: string, architecture: SeedProjectArchitecture | null) {
	...
	const diagnostic = inspectContentStructure({ projectRoot: dir, architecture });
