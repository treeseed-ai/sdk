...
export function buildLocalRuntimeVars(deployConfig, state, target, env) {
	...
	return {
		...
		LOCAL_DEV_MODE: envValue(env, 'TREESEED_LOCAL_DEV_MODE') ?? 'cloudflare',
		...
		FORM_TOKEN_SECRET:
			...
			?? state.generatedSecrets?.FORM_TOKEN_SECRET
		...
		FORMS_LOCAL_BYPASS_CLOUDFLARE_GUARDS: envValue(env, 'TREESEED_FORMS_LOCAL_BYPASS_CLOUDFLARE_GUARDS') ?? '',
...
export function buildSecretMap(deployConfig, state) {
	const generatedSecret = state.generatedSecrets?.FORM_TOKEN_SECRET ?? randomBytes(24).toString('hex');
	const previewSecret = state.generatedSecrets?.TREESEED_EDITORIAL_PREVIEW_SECRET ?? randomBytes(24).toString('hex');
	return {
		FORM_TOKEN_SECRET: envOrNull('TREESEED_FORM_TOKEN_SECRET') ?? generatedSecret,
		TREESEED_EDITORIAL_PREVIEW_SECRET: envOrNull('TREESEED_EDITORIAL_PREVIEW_SECRET') ?? previewSecret,
		TURNSTILE_SECRET_KEY: state.turnstileWidgets?.formGuard?.secret ?? envOrNull('TREESEED_TURNSTILE_SECRET_KEY'),
