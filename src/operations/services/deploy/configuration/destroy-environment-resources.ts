...
export async function destroyEnvironmentResources(tenantRoot, options = {}) {
	...
	const sweepEnabled = options.sweep === true;
	...
	const cacheRules = deleteCacheRules(deployConfig, state, { env, planOnly });
	...
	const sweep = sweepEnabled
		? {
			cloudflare: sweepCloudflareResources(tenantRoot, deployConfig, state, { env, planOnly, deleteData }),
			railway: await sweepRailwayResources(deployConfig, state, { env: process.env, planOnly }),
		}
		: { cloudflare: [], railway: [] };
