...
import { LOCAL_DOCKER_RESOURCE_PATTERN, dockerAvailable, killPidFromFile, runDestroyDocker } from './delete-cache-rules.ts';
import { deleteD1Database, deleteKvNamespace, deleteTurnstileWidget, deleteWorker, resourceOperation } from '../hosting/collect-missing-deploy-inputs.ts';
import { primaryHost } from './default-compatibility-date.ts';
import { deleteCloudflareApiResource, deleteQueueByName, deleteR2Bucket } from '../hosting/delete-cloudflare-api-resource.ts';
...
export function SweepTokens(deployConfig, state) {
...
export function matchesSweep(value, tokens) {
...
export function cloudflareEntryMatches(entry, tokens) {
	return cloudflareNameCandidates(entry).some((candidate) => matchesSweep(candidate, tokens));
}
...
export function sweepCloudflareResources(tenantRoot, deployConfig, state, { env, planOnly, deleteData }) {
	const tokens = SweepTokens(deployConfig, state);
	const operations = [];
	const pagesProjects = listPagesProjects(tenantRoot, env).filter((entry) => cloudflareEntryMatches(entry, tokens));
	...
	for (const worker of listWorkers(tenantRoot, env).filter((entry) => cloudflareEntryMatches(entry, tokens))) {
	...
	for (const namespace of listKvNamespaces(tenantRoot, env).filter((entry) => cloudflareEntryMatches(entry, tokens))) {
	...
	for (const queue of listQueues(tenantRoot, env).filter((entry) => cloudflareEntryMatches(entry, tokens))) {
		operations.push({ ...deleteQueueByName(tenantRoot, queue, { env, planOnly }), sweep: true });
	}

	for (const database of listD1Databases(tenantRoot, env).filter((entry) => cloudflareEntryMatches(entry, tokens))) {
	...
	for (const bucket of listR2Buckets(tenantRoot, env).filter((entry) => cloudflareEntryMatches(entry, tokens))) {
		operations.push({ ...deleteR2Bucket(tenantRoot, bucket.name, { env, planOnly, deleteData }), sweep: true });
	}

	for (const widget of listTurnstileWidgets(tenantRoot, env).filter((entry) => cloudflareEntryMatches(entry, tokens))) {
	...
	for (const zone of listDnsZones(env)) {
		...
		for (const record of listDnsRecords(zoneId, env)) {
			...
			if (!cloudflareEntryMatches(record, tokens)) {
				continue;
			}
...
export function countMatchingCloudflareEntries(entries, tokens) {
	return entries.filter((entry) => cloudflareEntryMatches(entry, tokens)).length;
}
