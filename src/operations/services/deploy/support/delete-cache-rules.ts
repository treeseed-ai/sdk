...
export function configuredRailwayDestroyTargets(tenantRoot, deployConfig, scope) {
	...
	const Database = deployConfig.services?.treeseedDatabase;
	if (Database?.enabled !== false && Database?.provider === 'railway' && Database?.railway?.resourceType === 'postgres') {
		const baseName = typeof Database.railway?.serviceName === 'string' && Database.railway.serviceName.trim()
			? Database.railway.serviceName.trim()
