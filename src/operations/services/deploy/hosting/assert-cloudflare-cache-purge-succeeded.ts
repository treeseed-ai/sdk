...
export function resolveSourcePagePurgeUrls(deployConfig) {
	...
	const paths = resolveWebCachePolicy(deployConfig).sourcePages.paths;
...
export function resolveConfiguredPagesProjectName(deployConfig) {
	return sharedDeploymentName(resolveResourceIdentity(deployConfig, createPersistentDeployTarget('prod')));
}
...
export function resolveConfiguredContentBucketName(deployConfig) {
	return sharedDeploymentName(resolveResourceIdentity(deployConfig, createPersistentDeployTarget('prod')), 'content');
}
