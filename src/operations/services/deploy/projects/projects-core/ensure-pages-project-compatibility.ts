...
import { buildCloudflarePagesFunctionBindings, mergeCloudflarePagesDeploymentConfig } from '../../support/run-wrangler.ts';
import { DEFAULT_COMPATIBILITY_DATE, DEFAULT_COMPATIBILITY_FLAGS, resolveConfiguredSurfaceDomain, resolveResourceIdentity } from '../../support/default-compatibility-date.ts';
import { deployTargetLabel, targetWorkerName, targetWorkersDevUrl } from '../../hosting/configured-surface-hosts.ts';
import { resolveConfiguredCloudflareAccountId, resolveConfiguredContentPublicBaseUrl } from '../../hosting/assert-cloudflare-cache-purge-succeeded.ts';
...
export function buildProvisioningSummary(deployConfig, state, target) {
	...
	const identity = state.identity ?? resolveResourceIdentity(deployConfig, target);
