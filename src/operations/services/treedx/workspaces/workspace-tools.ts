...
function packageSortWeight(pkg) {
	...
	const index = WORKSPACE_PACKAGE_DIRS.indexOf(dirName);
...
export function workspaceRoot(startCwd = process.cwd()) {
	return findNearestWorkspaceRoot(startCwd) ?? resolve(startCwd);
}

export function findNearestRoot(startCwd = process.cwd()) {
...
export function isWorkspaceRoot(root = process.cwd()) {
	try {
		return workspacePatterns(root).length > 0 || hasCompletePackageCheckout(root);
	} catch {
		return false;
	}
}

export function hasCompletePackageCheckout(root = process.cwd()) {
	return WORKSPACE_PACKAGE_DIRS.every((dirName) => {
...
export function findNearestWorkspaceRoot(startCwd = process.cwd()) {
...
export function workspacePatterns(root = workspaceRoot()) {
	if (!existsSync(resolve(root, 'package.json'))) {
		return hasCompletePackageCheckout(root) ? ['packages/*'] : [];
	}
...
function canResolveGitRef(baseRef, cwd = workspaceRoot()) {
	const result = runRepositoryGit(['rev-parse', '--verify', baseRef], {
		cwd,
		mode: 'read',
		allowFailure: true,
	});
