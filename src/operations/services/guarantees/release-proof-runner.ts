...
export type ProofRunResult = {
	...
	records: ProofRecord[];
	reused: ProofRecord[];
	failures: ProofRecord[];
	ok: boolean;
};

function workflowRecordFromInspection(input: {
	root: string;
	subject: ProofPlan['subjects'][number];
...
async function runHostedProof(root: string, subject: ProofPlan['subjects'][number]) {
	...
	const status: ProofRecord['status'] = workflow?.state === 'success'
		? 'passed'
		: workflow?.state === 'pending' ? 'pending' : 'failed';
	return workflowRecordFromInspection({ root, subject, workflow, startedAt, status });
}

function advisorySkippedProof(subject: ProofPlan['subjects'][number]) {
...
function runLocalProof(subject: ProofPlan['subjects'][number]) {
	const startedAt = new Date().toISOString();
