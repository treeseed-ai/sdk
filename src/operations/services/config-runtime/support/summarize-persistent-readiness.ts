...
export async function summarizePersistentReadiness(
	...
	const reconcile = await collectReconcileStatus({
		tenantRoot,
...
export function formatConfigValidationFailure(
	validations: Record<ConfigScope, ReturnType<typeof validateEnvironmentValues>>,
	scopes: ConfigScope[],
) {
...
export function createConfigReadiness(values, validation) {
	...
	return {
		github: {
			configured: Boolean(resolveGitHubToken(values)) && providerIssues('github').length === 0,
		},
		cloudflare: {
