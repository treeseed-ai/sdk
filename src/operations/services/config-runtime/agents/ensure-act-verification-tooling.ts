...
import { checkCommand, commandAvailable, toolStatus } from '../configuration/resolve-launch-environment.ts';
import { CLI_CHECK_TIMEOUT_MS } from '../configuration/machine-config-relative-path.ts';

export function ensureActVerificationTooling({ tenantRoot = process.cwd(), installIfMissing = true, env = process.env, write } = {}) {
	const managedEnv = createManagedToolEnv(env);
	const ghBinary = resolveToolBinary('gh', { env: managedEnv });
	...
	const railwayCommand = resolveToolCommand('railway', { env });
...
export function checkGitHubConnection({ tenantRoot, env }) {
	...
	const credential = repository
		...
		: null;
	const token = credential?.token ?? resolveGitHubToken(env);
	if (!token) {
	...
	const owner = typeof env.TREESEED_HOSTED_HUBS_GITHUB_OWNER === 'string'
		...
		: '';
	const toolEnv = createManagedToolEnv({ ...process.env, ...env, TREESEED_GITHUB_TOKEN: token });
