...
import { githubConfigSyncUnitId, runBounded, withGitHubEnvironmentCredentialContext } from '../hosting/check-railway-connection.ts';
import { collectEnvironmentContext, nonEmptyEnvironmentValues, resolveMachineEnvironmentValues } from '../support/resolve-entry-value-from-buckets.ts';
...
export function createGitHubConfigSyncUnits({
	...
	const bindingUnits = items.flatMap((item): DesiredUnit[] => {
		if (item.kind === 'secret') {
			return [{
...
export async function syncGitHubEnvironment({
	tenantRoot,
	scope = 'prod',
	planOnly = false,
