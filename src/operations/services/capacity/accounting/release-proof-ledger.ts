...
function isProofRecord(value: unknown): value is ProofRecord {
	return Boolean(value && typeof value === 'object'
		&& (value as ProofRecord).schemaVersion === 1
		&& typeof (value as ProofRecord).proofId === 'string'
		&& typeof (value as ProofRecord).status === 'string');
}

export function readProofLedger(root: string): ProofRecord[] {
...
export function writeProofRecord(root: string, record: ProofRecord): void {
...
export function findReusableProof(
	root: string,
	input: ProofInput & { requiredDriver?: ProofDriver },
): ProofRecord | null {
...
export function invalidateProofs(root: string, predicate: (record: ProofRecord) => boolean): void {
