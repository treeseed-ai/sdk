...
export function buildPackageArtifact(input: { packageRoot: string; outputDir: string }) {
	...
	const sourceSha = runGitText(['rev-parse', 'HEAD'], { cwd: packageRoot, mode: 'read' }).trim();
...
export function verifyPackageArtifact(input: { manifestPath: string; artifactPath?: string }) {
	const manifestPath = resolve(input.manifestPath);
	const manifest = JSON.parse(readFileSync(manifestPath, 'utf8')) as PackageArtifactManifest;
...
export function hydratePackageArtifacts(input: { artifactsRoot: string; projectRoot: string }) {
	...
	const manifests = readdirSync(artifactsRoot, { withFileTypes: true })
		...
		.map((entry) => verifyPackageArtifact({ manifestPath: resolve(artifactsRoot, entry.name, 'manifest.json') }))
