export type WorkflowPhaseStatus = 'passed' | 'failed' | 'skipped';

export type WorkflowTimingPhase = {
	id: string;
	label: string;
	startedAt: string;
	finishedAt: string;
...
export function createWorkflowTimer(startedAt = nowIso()): WorkflowTimer {
	const startMs = Date.parse(startedAt);
	const phases: WorkflowTimingPhase[] = [];

	function record(id: string, label: string, started: string, startedMs: number, status: WorkflowPhaseStatus) {
...
export function slowestWorkflowPhases(timing: WorkflowTiming, limit = 5) {
