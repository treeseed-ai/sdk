...
export class DefaultOperationsProvider implements OperationProvider {
	...
	constructor() {
		this.operations = [
			...
			new ScriptOperation('preview', 'tenant/tenant-astro-command'),
			new ScriptOperation('lint', 'treedx/workspaces/workspace-lint'),
			new ScriptOperation('test', 'treedx/workspaces/workspace-test'),
			new ScriptOperation('test:unit', 'treedx/workspaces/workspace-test-unit'),
			new PreflightOperation('preflight', false),
			new PreflightOperation('auth:check', true),
			new ScriptOperation('test:e2e', 'treedx/workspaces/workspace-command-e2e'),
			new ScriptOperation('test:e2e:local', 'treedx/workspaces/workspace-command-e2e', ['--mode=local']),
			new ScriptOperation('test:e2e:staging', 'treedx/workspaces/workspace-command-e2e', ['--mode=staging']),
			new ScriptOperation('test:e2e:full', 'treedx/workspaces/workspace-command-e2e', ['--mode=full']),
			new ScriptOperation('test:release', 'treedx/workspaces/workspace-release-verify'),
			new ScriptOperation('test:release:full', 'treedx/workspaces/workspace-release-verify', ['--full-smoke']),
			new ScriptOperation('release:publish:changed', 'treedx/workspaces/workspace-publish-changed-packages'),
			new ScriptOperation('astro', 'tenant/tenant-astro-command'),
			new ScriptOperation('d1:migrate:local', 'persistence/tenant-d1-migrate-local'),
