...
export function resolveContentDir(
	...
	options: Pick<TreeDxRepositoryAdapterOptions, 'contentPathMap' | 'repoRoot'>,
) {
...
function treeDxField(field: string) {
...
export class TreeDxRepositoryAdapter {
	constructor(private readonly options: TreeDxRepositoryAdapterOptions) {}
	...
	async create(request: SdkMutationRequest): Promise<TreeDxContentMutationResult> {
		const definition = resolveModelDefinition(request.model, this.options.models);
	...
	async update(request: SdkUpdateRequest): Promise<TreeDxContentMutationResult> {
		const definition = resolveModelDefinition(request.model, this.options.models);
