...
import { currentBranch, hasMeaningfulChanges, repoRoot } from "../../../../operations/services/treedx/workspaces/workspace-save.ts";
...
export async function connectMarketProject(
	helpers: WorkflowOperationHelpers,
	tenantRoot: string,
	input: ConfigInput,
	context: {
		scopes: ReturnType<typeof normalizeConfigScopes>;
		sync: ConfigInput['sync'];
		repairs: unknown[];
		preflight: ReturnType<typeof collectCliPreflight>;
		toolHealth: ReturnType<typeof ensureActVerificationTooling>;
	},
) {
	const machineConfig = loadMachineConfig(tenantRoot) as Record<string, any>;
	...
	const activeRemoteSession = resolveRemoteSession(tenantRoot, hostId)
		?? resolveRemoteSession(tenantRoot, remoteSettings.activeHostId)
		?? resolveRemoteSession(tenantRoot, 'official');
	...
	machineConfig.settings.market = {
		...
		runnerHostId,
		runnerReady: Boolean(connectionResult.runnerToken || resolveRemoteSession(tenantRoot, runnerHostId)?.accessToken),
