...
export async function workflowProof(helpers: WorkflowOperationHelpers, input: ProofInput = {}) {
	try {
		return await withContextEnv(helpers.context.env, async () => {
			...
			if (action === 'run') {
				const result = await timer.phaseAsync('proof-run', 'Run release proof subjects', () => runProof({
				...
				const ledger = timer.phase('proof-failures', 'Inspect failed proof records', () => summarizeProofLedger(root));
				payload = { action, target, failures: ledger.failures, summary: ledger.summary };
			} else if (action === 'explain') {
				const ledger = timer.phase('proof-explain', 'Explain proof duration and reuse', () => summarizeProofLedger(root));
				payload = {
				...
				const plan = timer.phase('proof-plan', 'Plan release proof subjects', () => buildProofPlan({
...
export function normalizeReleaseCandidatePackages(value: ReleaseCandidateInput['package']) {
	if (Array.isArray(value)) return value.map(String).filter(Boolean);
	if (typeof value === 'string' && value.trim()) return [value.trim()];
	return [];
}

export async function workflowReleaseCandidate(helpers: WorkflowOperationHelpers, input: ReleaseCandidateInput = {}) {
	try {
		return await withContextEnv(helpers.context.env, async () => {
			const timer = createWorkflowTimer();
			...
			const mode = (input.mode ?? 'strict') as WorkflowReleaseCandidateMode;
			const driver: ProofDriver = input.verifyDriver === 'local'
				? 'local'
			...
			const plan = timer.phase('proof-plan', 'Plan release-candidate proof subjects', () => buildProofPlan({
			...
			const proof = await timer.phaseAsync('proof-run', 'Run release-candidate proof nodes', () => runProof({
...
export function buildReleasePlanSnapshot(input: {
	...
	const publishablePackageNames = new Set(
		discoverPackageAdapters(input.root)
	...
	if (input.repairVersionLine !== true) {
		for (const adapter of discoverPackageAdapters(input.root)) {
