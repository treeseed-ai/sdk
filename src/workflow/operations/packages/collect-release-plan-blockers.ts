...
export function assertReleaseGitHubAutomationReady(root: string, selectedPackageNames: Set<string>, ciMode: WorkflowCiMode) {
	if (ciMode === 'off') {
		return;
	}
	const values = resolveMachineEnvironmentValues(root, 'prod');
