...
import { gitWorkflowRoot, headCommit, listTaskBranches, PRODUCTION_BRANCH, STAGING_BRANCH } from "../../operations/services/git-workflow.ts";
import { resolveGitHubRepositorySlug } from "../../operations/services/github-automation.ts";
import { inspectGitHubActionsVerification, type GitHubActionsVerificationTarget } from "../../operations/services/github-actions-verification.ts";
import { hostedWorkflowForPackage } from "../../operations/services/release-proof-planner.ts";
import { renderAdministrativeCommitMessage, type ReleaseHistoryCommit, type ReleaseHistorySummary } from "../../operations/services/release-history.ts";
...
import { checkedOutWorkspacePackageRepos, resolveTreeseedWorkflowSession, type TreeseedWorkflowSession } from ".././session.ts";
import type { TreeseedCiInput, TreeseedTaskBranchMetadata, TreeseedWorkflowNextStep, TreeseedWorkflowResult } from "../../workflow.ts";
import { stringRecord } from './gates-for-saved-repository-reports.ts';
import { ageDays, renderWorkflowStep } from './run-release-production-guarantees.ts';
import { buildWorkflowResult, submodulePointerForRef } from './create-repo-report.ts';
...
export function createNextSteps(steps: TreeseedWorkflowNextStep[]) {
	return steps.map(renderWorkflowStep);
}

export function createStatusResult(cwd: string, options: TreeseedWorkflowStatusOptions = {}): TreeseedWorkflowResult<ReturnType<typeof resolveTreeseedWorkflowState>> {
...
export function normalizeCiScope(value: TreeseedCiInput['scope']): 'workspace' | 'root' | 'packages' {
	return value === 'root' || value === 'packages' ? value : 'workspace';
}

export function normalizeCiLogLines(value: TreeseedCiInput['logLines']) {
...
export function normalizeCiWorkflows(input: TreeseedCiInput) {
...
export function ciTargetsForSession(session: TreeseedWorkflowSession, input: TreeseedCiInput) {
...
export async function createCiResult(cwd: string, input: TreeseedCiInput): Promise<TreeseedWorkflowResult<TreeseedCiResult>> {
...
export function createTasksResult(cwd: string): TreeseedWorkflowResult<{ tasks: TreeseedTaskBranchMetadata[]; workstreams: Array<{
	id: string;
	title: string;
