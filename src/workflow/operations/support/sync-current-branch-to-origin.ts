...
export function syncCurrentBranchToOrigin(operation: WorkflowOperationId, repoDir: string, branch: string) {
	try {
		...
		throw new WorkflowError(operation, 'merge_conflict', formatMergeConflictReport(report, repoDir, branch), {
...
export function createSaveFailure(
	...
	const exitCode = error instanceof WorkflowError ? error.exitCode : undefined;
	throw new WorkflowError('save', code, `${message}\n${rendered}`, {
		details: {
			partialFailure: {
...
export async function workflowStatus(helpers: WorkflowOperationHelpers, input: WorkflowStatusOptions = {}) {
