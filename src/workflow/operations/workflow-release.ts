...
import { PRODUCTION_BRANCH, pushBranch, STAGING_BRANCH } from "../../operations/services/git-workflow.ts";
...
import { checkedOutWorkspacePackageRepos, resolveTreeseedWorkflowSession } from ".././session.ts";
import type { TreeseedReleaseInput } from "../../workflow.ts";
import { WorkflowOperationHelpers, ensureWorkflowWorkspaceLinks, maybeRunLocalWorkflowCleanup, normalizeCiMode, normalizeSceneArtifactsMode, unlinkWorkflowWorkspaceLinks } from './workflow-write.ts';
import { resolveProjectRootOrThrow, withContextEnv, workflowError } from './run-release-production-guarantees.ts';
import { buildWorkflowResult, createWorkspacePackageReports, createWorkspaceRootRepoReport, normalizeExecutionMode, singleSelectedWorkflowAppId } from './create-repo-report.ts';
import { checkedOutReleaseHelperRepos, readJsonFile, stageCandidateAttestationBlockers, syncAllCheckedOutReleaseHelperRepos } from './staging-candidate-workflow-gates.ts';
import { stringRecord } from './gates-for-saved-repository-reports.ts';
import { acquireWorkflowRun, completeWorkflowRun, executeJournalStep, findAutoResumableReleaseRun, prepareFreshReleaseRun, skipJournalStep } from './prepare-fresh-release-run.ts';
import { buildReleasePlanSnapshot } from './workflow-proof.ts';
import { backMergeProductionIntoStaging, backMergeRootProductionIntoStaging, collectReleaseHelperRepoBlockers, releaseHelperRepoToProduction, releasePlanPackageSelection, releasePlanStableDependencyVersionMap, releasePlanVersionMap } from './back-merge-production-into-staging.ts';
import { collectReleasePlanBlockers } from './collect-release-plan-blockers.ts';
import { persistProductionReleaseImageRefs, productionReleaseImageRefEnv, productionReleaseImageRefVersions, verifyReleaseApiEnvironmentIsolation } from './reconcile-save-hosted-environment.ts';
import { hostedDeployGate, waitForWorkflowGates, worktreePayload } from './normalize-release-candidate-mode.ts';
import { runReleaseGateReconcileFacade } from './run-release-gate-reconcile-facade.ts';
import { adoptPublishedPackageRelease, applyStableWorkspaceVersionChanges, commitAllIfChanged, ensureReleaseTag, promoteCommitToProductionBranch, updatePackageLockRootVersion, updateReleaseChangelog, versionLines } from './plan-root-package-version.ts';
import { failWorkflowRun, prepareAdapterReleaseMetadata, productionPackageDeployGates, releaseWorkflowForPackage, runReleaseNpmInstall, tagCommitSha } from './fail-workflow-run.ts';
import { createNextSteps, releaseAdminMessage } from './release-admin-message.ts';
	...
	try {
		return await withContextEnv(helpers.context.env, async () => {
			...
			const explicitResumeRunId = helpers.context.workflow?.resumeRunId
				?? (input as TreeseedReleaseInput & { resumeRunId?: string }).resumeRunId
				?? null;
			...
			const effectiveInput = autoResumeRun
				? {
					...(autoResumeRun.input as unknown as TreeseedReleaseInput), 					ciMode: input.ciMode ?? (autoResumeRun.input as unknown as TreeseedReleaseInput).ciMode,
				}
					: input;
