...
import { collectTreeseedEnvironmentContext, collectTreeseedConfigSeedValues, setTreeseedMachineEnvironmentValue } from "../../operations/services/config-runtime.ts";
import { createPersistentDeployTarget, purgeSourcePageCaches } from "../../operations/services/deploy.ts";
import { highestStableGitTagOnLine } from "../../operations/services/workspace-save.ts";
...
import { configuredRailwayServices, waitForRailwayManagedDeploymentsSettled } from "../../operations/services/railway-deploy.ts";
...
import { selectorFromWorkflowHostingGraph } from './normalize-release-candidate-mode.ts';
...
export async function verifyReleaseApiEnvironmentIsolation(
	...
	for (const environment of ['prod', 'staging'] as const) {
		const env = {
			...helpers.context.env, 			...collectTreeseedConfigSeedValues(root, environment, helpers.context.env),
