...
export async function workflowConfig(helpers: WorkflowOperationHelpers, input: ConfigInput = {}) {
	try {
		return await withContextEnv(helpers.context.env, async () => {
			...
			const repairs = input.repair === false ? [] : (resolveWorkflowState(tenantRoot).deployConfigPresent ? applySafeRepairs(tenantRoot) : []);
			const toolHealth = ensureActVerificationTooling({
			...
			const contextSnapshot = collectConfigContext({
			...
			if (rotateMachineKeyFlag) {
				const result = rotateMachineKey(tenantRoot);
				return buildWorkflowResult(
					'config', 					tenantRoot,
					{
			...
			if (bootstrapPreflight) {
				...
				const plansByScope = await Promise.all(scopes
					...
					.map(async (scope) => {
						...
						const selectedUnits = filterDesiredUnitsByBootstrapSystems(
							derived.units, 							selection.runnable.filter((system) => system !== 'github'),
						);
						const registry = createReconcileRegistry(derived.deployConfig);
						...
						const planned = await planReconciliation({
			...
			const reports = printEnv
				? await Promise.all(scopes.map(async (scope) => ({
					...
					provider: finalizeResult.connectionChecks.find((report) => report.scope === scope) ?? await checkProviderConnections({ tenantRoot, scope, env: helpers.context.env }),
				})))
				: [];
			const { configPath, keyPath } = getMachineConfigPaths(tenantRoot);
...
export async function workflowExport(helpers: WorkflowOperationHelpers, input: ExportInput = {}) {
	return await withContextEnv(helpers.context.env, async () => {
		const directory = resolve(helpers.context.cwd ?? helpers.cwd(), input.directory ?? '.');
		const exported = await exportCodebase({ directory });
