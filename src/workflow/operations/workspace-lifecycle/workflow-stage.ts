	...
	try {
		return await withContextEnv(helpers.context.env, async () => {
			...
			const explicitResumeRunId = helpers.context.workflow?.resumeRunId
				?? (input as StageInput & { resumeRunId?: string }).resumeRunId
				?? null;
			...
			const effectiveInput = autoResumeRun
				? (autoResumeRun.input as unknown as StageInput)
				: input;
			...
			const workflowRun = acquireWorkflowRun('stage', resolveWorkflowSession(root), {
