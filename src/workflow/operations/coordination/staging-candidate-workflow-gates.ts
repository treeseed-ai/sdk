...
export function normalizeStageCiMode(input: StageInput): StageCiMode {
...
export function normalizeStageCleanupMode(input: StageInput): StageCleanupMode {
...
export function dedupeManagedReposByRemote(repos: ManagedRepository[]) {
...
export function stageConflictError(message: string, details: Record<string, unknown>) {
	return new WorkflowError('stage', 'merge_conflict', message, {
		details,
		exitCode: 12,
	});
}
