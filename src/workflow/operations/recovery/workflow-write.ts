...
export type WorkflowStatePayload = ReturnType<typeof resolveWorkflowState>;

export type ReleaseCandidateMode = WorkflowReleaseCandidateMode;
...
export type WorkflowOperationHelpers = {
	...
	cwd(): string;
	write: WorkflowWrite;
	runStatus(): Promise<WorkflowResult<ReturnType<typeof resolveWorkflowState>>>;
	runTasks(): Promise<WorkflowResult<{ tasks: TaskBranchMetadata[] }>>;
};
...
export function normalizeCiMode(mode: WorkflowCiMode | undefined, operation: 'save' | 'release') {
	if (mode === 'hosted' || mode === 'off') return mode;
	return operation === 'save' ? 'off' : 'hosted';
}

export function normalizeSaveLane(lane: SaveInput['lane'] | undefined) {
...
export function normalizeSaveVerifyMode(mode: SaveInput['verifyMode'] | undefined): SaveVerifyMode {
	switch (mode) {
		case 'skip':
		case 'fast':
		case undefined:
