import { checkoutBranch, headCommit, PRODUCTION_BRANCH, pushBranch, remoteHeadCommit, remoteBranchExists, STAGING_BRANCH, syncBranchWithOrigin } from "../../operations/services/git-workflow.ts";
...
import { type ReleaseHistorySummary } from "../../operations/services/release-history.ts";
import { collectMergeConflictReport, currentBranch, formatMergeConflictReport, hasMeaningfulChanges, highestStableGitTagOnLine, originRemoteUrl, repoRoot } from "../../operations/services/workspace-save.ts";
import { workspacePackages } from "../../operations/services/workspace-tools.ts";
import { type TreeseedManagedRepository } from "../../operations/services/managed-repositories.ts";
import type { TreeseedWorkflowOperationId } from "../../workflow.ts";
import { ReleaseCandidateMode, TreeseedWorkflowError, runGit } from './workflow-write.ts';
import { commitAllIfChanged, promoteCommitToProductionBranch, releaseHistoryCommits, versionLines } from './plan-root-package-version.ts';
import { releaseAdminMessage } from './release-admin-message.ts';
import { syncAllCheckedOutPackageRepos } from './sync-current-branch-to-origin.ts';
import { checkedOutReleaseHelperRepos } from './staging-candidate-workflow-gates.ts';
import { normalizeReleaseCandidateMode } from './normalize-release-candidate-mode.ts';
...
export function releaseHelperRepoToProduction(repo: TreeseedManagedRepository) {
...
export function releaseCandidateProofDriver(mode: ReleaseCandidateMode, lane: 'fast' | 'promotion' = 'fast'): TreeseedProofDriver {
