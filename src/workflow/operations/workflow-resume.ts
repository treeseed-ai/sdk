...
import { currentBranch } from "../../operations/services/workspace-save.ts";
...
import type { TreeseedCloseInput, TreeseedDestroyInput, TreeseedReleaseInput, TreeseedRecoverInput, TreeseedResumeInput, TreeseedSaveInput, TreeseedStageInput, TreeseedSwitchInput, TreeseedUpdateInput } from "../../workflow.ts";
...
import { buildWorkflowResult, createWorkspacePackageReports, createWorkspaceRootRepoReport } from './create-repo-report.ts';
import { acceptedPublishedReleaseHeads } from './prepare-fresh-release-run.ts';
import { helpersForCwd } from './normalize-release-candidate-mode.ts';
import { workflowSwitch } from './workflow-switch.ts';
import { workflowSave } from './workflow-save.ts';
import { workflowUpdate } from './workflow-update.ts';
import { workflowClose } from './workflow-close.ts';
import { workflowStage } from './workflow-stage.ts';
import { workflowRelease } from './workflow-release.ts';
import { workflowDestroy } from './workflow-destroy.ts';
import { nextPendingJournalStep, toError } from './connect-treeseed-market-project.ts';
			...
			switch (journal.command) {
				...
				case 'release':
					return workflowRelease(resumedHelpers, {
						...(journal.input as unknown as TreeseedReleaseInput), 						resumeRunId: runId,
					} as TreeseedReleaseInput & { resumeRunId: string });
				case 'destroy':
