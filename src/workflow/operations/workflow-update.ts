...
import { applyTreeseedEnvironmentToProcess, assertTreeseedCommandEnvironment, resolveTreeseedLaunchEnvironment } from "../../operations/services/config-runtime.ts";
...
import type { TreeseedUpdateInput, TreeseedWorkflowDevInput } from "../../workflow.ts";
...
import { TreeseedUpdateConflict, TreeseedUpdateRepoResult, normalizeUpdateSource, normalizeUpdateStrategy } from './workflow-switch.ts';
import { buildWorkflowResult, ensureLocalReadinessOrThrow, normalizeExecutionMode } from './create-repo-report.ts';
import { commitRootUpdateIfNeeded, ensureUpdateRepoReady, mergeUpdateRepo, planUpdateRepo } from './update-ahead-behind.ts';
...
export async function workflowDev(helpers: WorkflowOperationHelpers, input: TreeseedWorkflowDevInput = {}) {
	try {
		return withContextEnv(helpers.context.env, async () => {
			...
			const runtime = {
				mode: process.env.TREESEED_LOCAL_DEV_MODE ?? 'cloudflare', 				apiBaseUrl: process.env.TREESEED_API_BASE_URL ?? 'http://127.0.0.1:3000', 				webUrl: 'http://127.0.0.1:8787',
			};
			if (input.plan) {
