...
import type { RailwayServiceSummary } from '../operations/services/railway-api.ts';
import type {
...
import {
	...
	type TreeseedRailwayIacService,
...
function runDockerImagePrepareCommand(input: TreeseedReconcileAdapterInput) {
...
function ensureLocalComposeDataDir(input: TreeseedReconcileAdapterInput) {
...
function observedOrResolvedComposeFiles(input: TreeseedReconcileAdapterInput) {
...
function localComposeProfiles(input: TreeseedReconcileAdapterInput) {
...
function localComposeBuildPolicy(input: TreeseedReconcileAdapterInput): 'never' | 'missing' | 'always' {
...
interface LocalTreeDxContentProject {
	projectKey?: string;
	slug: string;
...
function localTreeDxProjects(value: unknown): LocalTreeDxContentProject[] {
...
function railwayServiceRootDirectory(
	tenantRoot: string,
...
function configuredMarketDatabaseService(tenantRoot: string, deployConfig: TreeseedReconcileAdapterInput['context']['deployConfig']) {
...
function treeseedDatabaseDescriptor(service: Record<string, any>, slug?: string | null) {
	return {
		service,
...
function railwayDriftSessionKey(scope: string) {
	return `railway:provider-drift:${scope}`;
}

function recordRailwayProviderDrift(input: TreeseedReconcileAdapterInput, scope: string, drift: Record<string, unknown>) {
...
function railwayProviderDrift(input: TreeseedReconcileAdapterInput, scope: string) {
...
function isRailwayCapacityProviderService(service: ReturnType<typeof configuredRailwayServices>[number]) {
	return String(service.key ?? '').startsWith('capacityProvider');
}

function railwayServiceMatchesKey(service: ReturnType<typeof configuredRailwayServices>[number], key: string) {
	return service.key === key || service.instanceKey === key || service.serviceName === key || service.serviceId === key;
}

function configuredRailwayProjectSyncGroups(
	input: TreeseedReconcileAdapterInput,
	scope: 'local' | 'staging' | 'prod',
	serviceKeys?: string[],
) {
...
function configuredRailwayIacDatabase(
	input: TreeseedReconcileAdapterInput,
...
async function reconcileStaleOperationsRunnerResourcesForProject(
	input: TreeseedReconcileAdapterInput,
	{
		scope,
		env,
...
async function observeRailwayUnit(input: TreeseedReconcileAdapterInput, {
	refresh = false,
...
function railwayProjectServiceKeys(
	input: TreeseedReconcileAdapterInput,
...
function findRailwayTopologyEntry(
	topology: Awaited<ReturnType<typeof resolveRailwayTopologyForScope>>,
	identity: string,
) {
...
async function resolveRailwayUnitTopology(
	input: TreeseedReconcileAdapterInput,
	scope: TreeseedEnvironmentScope,
...
async function buildRailwayDiff(input: TreeseedReconcileAdapterInput, observed: TreeseedObservedUnitState): Promise<TreeseedReconcileUnitDiff> {
	const scope = input.context.target.kind === 'persistent' ? input.context.target.scope : 'staging';
	const serviceKey = String(input.unit.metadata.serviceKey ?? '').trim();
	const configuredService = configuredRailwayServiceForUnit(input, scope);
