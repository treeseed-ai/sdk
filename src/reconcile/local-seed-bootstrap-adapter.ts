...
async function loadSeedModule(input: TreeseedReconcileAdapterInput): Promise<LocalSeedModule> {
...
function seedInput(input: TreeseedReconcileAdapterInput) {
	return {
...
async function observe(input: TreeseedReconcileAdapterInput): Promise<TreeseedObservedUnitState> {
	try {
...
function result(
	input: TreeseedReconcileAdapterInput & { observed: TreeseedObservedUnitState; diff: TreeseedUnitDiff },
...
export function createLocalSeedBootstrapAdapter(): TreeseedReconcileAdapter {
	return {
		providerId: 'local',
		...
		async verify(input): Promise<TreeseedUnitVerificationResult> {
			const pending = Number(input.observed.live.pendingMutations ?? 0);
