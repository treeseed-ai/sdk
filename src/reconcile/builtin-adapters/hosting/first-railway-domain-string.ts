...
export async function observeRailwayCustomDomainLive(input: ReconcileAdapterInput, domain: string) {
	if (!domain) {
		return null;
	}
...
export function collectCloudflareEnvironmentSync(input: ReconcileAdapterInput) {
	...
	if (generatedTurnstileSiteKey) {
		vars.PUBLIC_TURNSTILE_SITE_KEY = generatedTurnstileSiteKey;
...
export function verificationCheck(
	key: string,
	description: string,
	source: UnitVerificationCheck['source'],
	options: {
		exists: boolean;
		configured?: boolean;
		ready?: boolean;
