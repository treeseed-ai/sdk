import { queueId, queueName } from "../../operations/services/deploy.ts";
import type { TreeseedObservedUnitState, TreeseedReconcileAdapterInput } from ".././contracts.ts";
import { cloudflareObservationSnapshot, findTurnstileWidget } from './normalize-turnstile-domains.ts';
import { hasLiveResourceId } from './build-workflow-meta-adapter.ts';
import { noopObservedState } from './to-deploy-target.ts';

export function observeCloudflareUnit(input: TreeseedReconcileAdapterInput): TreeseedObservedUnitState {
