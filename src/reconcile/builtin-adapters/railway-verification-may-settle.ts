import { deleteRailwayService, deleteRailwayVolume, listRailwayVolumes } from "../../operations/services/railway-api.ts";
import type { TreeseedObservedUnitState, TreeseedReconcileAdapterInput, TreeseedReconcileResult, TreeseedReconcileUnitDiff, TreeseedUnitVerificationResult } from ".././contracts.ts";
...
import { buildRailwayEnv, providerCache } from './build-workflow-meta-adapter.ts';
...
export function railwayVerificationMaySettle(verification: TreeseedUnitVerificationResult) {
...
export function railwayUnitServiceIdentity(input: TreeseedReconcileAdapterInput) {
...
export async function buildRailwayDiff(input: TreeseedReconcileAdapterInput, observed: TreeseedObservedUnitState): Promise<TreeseedReconcileUnitDiff> {
...
export async function reconcileRailwayUnit(input: TreeseedReconcileAdapterInput, diff: TreeseedReconcileUnitDiff): Promise<TreeseedReconcileResult> {
	const scope = input.context.target.kind === 'persistent' ? input.context.target.scope : 'staging';
	const serviceKey = String(input.unit.metadata.serviceKey ?? '').trim();
	const configuredService = configuredRailwayServiceForUnit(input, scope);
...
export async function destroyRailwayUnit(input: TreeseedReconcileAdapterInput): Promise<TreeseedReconcileResult> {
	const scope = input.context.target.kind === 'persistent' ? input.context.target.scope : 'staging';
	const serviceKey = String(input.unit.metadata.serviceKey ?? '').trim();
