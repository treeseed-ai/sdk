...
export function cloudflareObservationSnapshot(input: ReconcileAdapterInput, forceRefresh = false) {
...
export function storeCustomDomainState(input: ReconcileAdapterInput, provider: string, domain: string, value: Record<string, unknown>) {
	input.context.session.set(customDomainStateKey(provider, domain), value);
}

export function getCustomDomainState(input: ReconcileAdapterInput, provider: string, domain: string) {
	return input.context.session.get(customDomainStateKey(provider, domain)) as Record<string, unknown> | undefined;
}

export function getPersistedCustomDomainState(input: ReconcileAdapterInput, provider: string, domain: string) {
	if (!domain) {
		return null;
	}
	if (provider === 'railway') {
		try {
			const state = loadReconcileState(input.context.tenantRoot, input.context.target, input.context.launchEnv);
			const unitId = createReconcileUnitId('custom-domain:api', domain);
...
export function dnsRecordsFromCurrentResult(input: ReconcileAdapterInput & { result?: ReconcileResult | null }) {
