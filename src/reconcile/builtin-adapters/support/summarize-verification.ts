...
export function summarizeVerification(unitId: string, checks: UnitVerificationCheck[], warnings: string[] = []): UnitVerificationResult {
...
export function unsupportedVerification(unitId: string, message: string): UnitVerificationResult {
	return {
		unitId,
		supported: false,
		exists: false,
...
export function syncPagesEnvironmentVariablesForTarget(input: ReconcileAdapterInput, { planOnly = false } = {}) {
