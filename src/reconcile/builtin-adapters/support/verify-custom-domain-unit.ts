...
export function verifyDnsRecordUnit(input: ReconcileAdapterInput): UnitVerificationResult {
	const env = buildCloudflareEnv(input);
	const domain = String(input.unit.spec.domain ?? '').trim();
...
export async function reconcileCustomDomainUnit(input: ReconcileAdapterInput, diff: ReconcileUnitDiff): Promise<ReconcileResult> {
	switch (input.unit.unitType) {
		case 'custom-domain:web': {
			const env = buildCloudflareEnv(input);
			const domain = String(input.unit.spec.domain ?? '').trim();
...
export function reconcileDnsRecordUnit(input: ReconcileAdapterInput, diff: ReconcileUnitDiff): ReconcileResult {
	let attempt = 0;
	for (;;) {
		try {
			const env = buildCloudflareEnv(input);
