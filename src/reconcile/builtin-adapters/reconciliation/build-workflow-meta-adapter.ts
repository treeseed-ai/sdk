...
export function providerCache<T>(input: ReconcileAdapterInput, key: string, loader: () => T, forceRefresh = false): T {
	if (forceRefresh) {
...
export function resolveReconcileEnvironmentValues(
	...
	if (scope === 'local') {
		return normalizeEnvironmentValues(resolveMachineEnvironmentValues(input.context.tenantRoot, scope));
	}
...
export function buildCloudflareEnv(input: ReconcileAdapterInput) {
	...
	if (scope !== 'local' && (!accountId || !token)) {
		const configuredValues = normalizeEnvironmentValues(resolveMachineEnvironmentValues(input.context.tenantRoot, scope));
		accountId = accountId || ([
...
export function buildRailwayEnv(input: ReconcileAdapterInput, scope: 'local' | 'staging' | 'prod') {
	const values = resolveReconcileEnvironmentValues(input, scope);
	let token = [
