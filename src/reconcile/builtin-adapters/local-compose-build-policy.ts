...
import { genericObservedState } from './to-deploy-target.ts';

export function localComposeBuildPolicy(input: TreeseedReconcileAdapterInput): 'never' | 'missing' | 'always' {
	return input.unit.spec.buildPolicy === 'never' || input.unit.spec.buildPolicy === 'always'
		? input.unit.spec.buildPolicy
		: 'missing';
}

export function runLocalComposePrepareCommand(input: TreeseedReconcileAdapterInput) {
...
export function buildLocalComposeLaunchEnv(input: TreeseedReconcileAdapterInput) {
	return {
...
export function localContentSpecString(input: TreeseedReconcileAdapterInput, key: string) {
	const value = input.unit.spec[key];
	return typeof value === 'string' && value.trim() ? value.trim() : null;
}

export function localContentSpecRecord(input: TreeseedReconcileAdapterInput, key: string) {
	const value = input.unit.spec[key];
	return value && typeof value === 'object' && !Array.isArray(value) ? value as Record<string, unknown> : {};
}

export function localContentGitEnvironment(input: TreeseedReconcileAdapterInput) {
...
export function localContentPathInsideTenant(input: TreeseedReconcileAdapterInput, targetPath: string | null) {
...
export function localContentObservedState(input: TreeseedReconcileAdapterInput): TreeseedObservedUnitState {
...
export function expectedLocalContentOrigin(input: TreeseedReconcileAdapterInput) {
	const repo = localContentSpecRecord(input, 'contentRepository');
	return typeof repo.gitUrl === 'string' ? repo.gitUrl : null;
}

export function originMatches(observed: TreeseedObservedUnitState, expected: string | null) {
...
export function runLocalContentClone(input: TreeseedReconcileAdapterInput, targetPath: string, gitUrl: string, branch: string | null) {
