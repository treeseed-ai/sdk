...
import { destroyCloudflareResources } from "../../operations/services/deploy.ts";
import { configuredRailwayServices } from "../../operations/services/railway-deploy.ts";
import { ensureRailwayEnvironment, ensureRailwayProject, listRailwayEnvironments } from "../../operations/services/railway-api.ts";
import type { TreeseedObservedUnitState, TreeseedReconcileAdapter, TreeseedReconcileAdapterInput, TreeseedReconcileResult, TreeseedReconcileUnitDiff, TreeseedReconcileUnitType } from ".././contracts.ts";
import { discoverTreeseedApplications } from "../../hosting/apps.ts";
import { verifyCloudflareUnit } from './verify-cloudflare-unit-once.ts';
import { providerCache } from './build-workflow-meta-adapter.ts';
import { reconcileCloudflareTarget, syncCloudflareSecretsForTarget } from './reconcile-cloudflare-target.ts';
import { syncPagesEnvironmentVariablesForTarget } from './summarize-verification.ts';
...
export function buildCloudflareDiff(input: TreeseedReconcileAdapterInput, observed: TreeseedObservedUnitState): TreeseedReconcileUnitDiff {
	if (!observed.exists) {
		return {
			action: 'create',
...
export function reconcileCloudflareUnit(input: TreeseedReconcileAdapterInput, diff: TreeseedReconcileUnitDiff): TreeseedReconcileResult {
...
export function buildCloudflareAdapter(unitType: TreeseedReconcileUnitType): TreeseedReconcileAdapter {
	return {
...
export function configuredMarketDatabaseService(tenantRoot: string, deployConfig: TreeseedReconcileAdapterInput['context']['deployConfig']) {
...
export function treeseedDatabaseDescriptor(service: Record<string, any>, slug?: string | null) {
	return {
		service,
