...
import type { TreeseedObservedUnitState, TreeseedReconcileAdapterInput, TreeseedReconcileUnitDiff } from ".././contracts.ts";
import { discoverTreeseedApplications } from "../../hosting/apps.ts";
import { capacityProviderRoleForService } from './observe-railway-unit.ts';
import { dnsRecordMatches, getCloudflarePagesDomain, getCustomDomainState, getPersistedCustomDomainState, listCloudflareDnsRecords, normalizeRailwayDomainDnsRecord, storeCustomDomainState } from './normalize-turnstile-domains.ts';
import { buildCloudflareEnv } from './build-workflow-meta-adapter.ts';
import { observeRailwayCustomDomainLive } from './first-railway-domain-string.ts';
...
export function resolveHostedApiBaseUrl(input: TreeseedReconcileAdapterInput, scope: string) {
	const environment = scope === 'prod' ? 'prod' : scope;
...
export function buildAttachmentDiff(input: TreeseedReconcileAdapterInput, observed: TreeseedObservedUnitState): TreeseedReconcileUnitDiff {
	if (!observed.exists) {
		return {
			action: 'create',
...
export function resolveDesiredDnsRecords(input: TreeseedReconcileAdapterInput) {
...
export async function observeCustomDomainUnit(input: TreeseedReconcileAdapterInput): Promise<TreeseedObservedUnitState> {
...
export function observeDnsRecordUnit(input: TreeseedReconcileAdapterInput): TreeseedObservedUnitState {
	const env = buildCloudflareEnv(input);
	const domain = String(input.unit.spec.domain ?? '').trim();
