...
function emptyPersistedUnitState(unit: DesiredUnit): UnitPersistedState {
	return {
		unitId: unit.unitId,
...
export function migrateLegacyDeployStateUnits(legacyState: Record<string, any>, target: ReconcileTarget) {
	const identity = legacyState.identity ?? resolveResourceIdentity({
	...
	const units: Record<string, UnitPersistedState> = {};
...
export function loadReconcileState(
	tenantRoot: string,
	...
	const persistedUnits = legacyState.units && typeof legacyState.units === 'object'
		? legacyState.units as Record<string, UnitPersistedState>
...
export function writeReconcileState(
	tenantRoot: string,
	reconcileState: ReconcileStateRecord,
	env: NodeJS.ProcessEnv | Record<string, string | undefined> = process.env,
) {
	const deployConfig = loadPlatformConfig({ tenantRoot, environment: reconcileState.target.kind === 'persistent' ? reconcileState.target.scope : 'staging', env }).deployConfig;
...
export function ensurePersistedUnitState(
	reconcileState: ReconcileStateRecord,
	unit: DesiredUnit,
) {
...
export function updatePersistedUnitState(
	reconcileState: ReconcileStateRecord,
	state: UnitPersistedState,
) {
	reconcileState.units[state.unitId] = state;
}

export function desiredUnitSpecHash(unit: DesiredUnit) {
