...
export function deriveDesiredUnits({
	tenantRoot,
	target,
	env = process.env,
}: {
	...
	const deployConfig = loadPlatformConfig({ tenantRoot, environment: target.kind === 'persistent' ? target.scope : 'staging', env }).deployConfig;
	const legacyState = loadDeployState(tenantRoot, deployConfig, { target });
	const identity = legacyState.identity ?? resolveResourceIdentity(deployConfig, target);
	const units: DesiredUnit[] = [];
	const add = (unit: DesiredUnit) => {
		units.push(unit);
		return unit.unitId;
	};

	const databaseId = add({
		unitId: createReconcileUnitId('database', legacyState.d1Databases.SITE_DATA_DB.databaseName),
		unitType: 'database',
		provider: 'cloudflare',
		identity,
		target,
	...
	const formGuardKvId = add({
		unitId: createReconcileUnitId('kv-form-guard', legacyState.kvNamespaces.FORM_GUARD_KV.name),
	...
	const turnstileWidgetId = legacyState.turnstileWidgets?.formGuard?.name && deployConfig.turnstile?.enabled === true
		? add({
			unitId: createReconcileUnitId('turnstile-widget', legacyState.turnstileWidgets.formGuard.name),
	...
	const contentStoreId = add({
		unitId: createReconcileUnitId('content-store', legacyState.content.bucketName ?? deployConfig.slug),
	...
	const pagesProjectId = add({
		unitId: createReconcileUnitId('pages-project', legacyState.pages.projectName),
	...
	const edgeWorkerId = add({
		unitId: createReconcileUnitId('edge-worker', legacyState.workerName),
	...
	if (deployConfig.surfaces?.web?.enabled !== false) {
		...
		const webDomainUnitId = webDomain
			? add({
				unitId: createReconcileUnitId('custom-domain:web', webDomain),
		...
		const webDnsUnitId = webDomain
			? add({
				unitId: createReconcileUnitId('dns-record', `web:${webDomain}`),
				unitType: 'dns-record',
			...
			: null;
		add({
			unitId: createReconcileUnitId('web-ui', 'web-ui'),
			unitType: 'web-ui',
			provider: 'treeseed',
			identity,
			target,
	...
	for (const configuredService of configuredRailwayServices(tenantRoot, scope, env)) {
		...
		const concreteId = add({
			unitId: createReconcileUnitId(concreteType, desiredServiceName),
		...
		const apiCustomDomainId = apiDomain
			? add({
				unitId: createReconcileUnitId('custom-domain:api', apiDomain),
		...
		const apiDnsUnitId = apiDomain
			? add({
				unitId: createReconcileUnitId('dns-record', `api:${apiDomain}`),
				unitType: 'dns-record',
				provider: deployConfig.providers?.dns ?? 'cloudflare-dns',
				identity,
				target,
		...
		const runtimeInstanceKey = serviceKey === 'operationsRunner'
			...
			: serviceKey;
		add({
			unitId: createReconcileUnitId(runtimeUnitType, runtimeInstanceKey),
