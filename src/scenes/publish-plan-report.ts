...
export function formatTreeseedScenePublishPlanMarkdownReport(manifest: TreeseedScenePublishPlanManifest) {
	const lines = [
...
export function writeTreeseedScenePublishPlanReport(input: {
	manifest: TreeseedScenePublishPlanManifest;
	paths: TreeseedScenePublishPlanPaths;
}) {
	writeJson(input.paths.manifestPath, input.manifest);
	writeText(input.paths.reportPath, formatTreeseedScenePublishPlanMarkdownReport(input.manifest));
}

export function appendTreeseedScenePublishPlanPaths(input: {
	runPath: string;
	paths: TreeseedScenePublishPlanPaths;
}): TreeseedSceneDiagnostic[] {
	try {
		...
		const run = JSON.parse(readFileSync(input.runPath, 'utf8')) as TreeseedSceneRunReport & {
			publishPlanPaths?: TreeseedScenePublishPlanPaths;
