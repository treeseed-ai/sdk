...
function normalizePlaywrightError(error: unknown) {
	...
	const tagged = new Error(message) as Error & { treeseedSceneCode?: string };
	tagged.treeseedSceneCode = missingBrowser ? 'scene.playwright_browser_missing' : 'scene.playwright_unavailable';
	return tagged;
}

export function createPlaywrightTreeseedSceneBrowserAdapter(): TreeseedSceneBrowserAdapter {
	return {
