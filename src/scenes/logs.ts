...
import { readTreeseedDevLogs } from '../local-dev/managed-dev.ts';
import { sceneWarningDiagnostic } from './diagnostics.ts';
import type { TreeseedSceneLogCollectOptions, TreeseedSceneLogReport } from './types.ts';
...
export function collectTreeseedSceneLogs(input: TreeseedSceneLogCollectOptions): TreeseedSceneLogReport {
	const diagnostics = [];
	...
	try {
		const devLogs = readTreeseedDevLogs({ cwd: input.projectRoot, surfaces: 'web,api,operations-runner' }).logs;
