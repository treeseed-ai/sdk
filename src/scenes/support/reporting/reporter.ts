...
import type { SceneRunReport } from '../../types.ts';
import { formatSceneDiagnostics } from './diagnostics.ts';

function row(values: string[]) {
	return `| ${values.map((value) => value.replace(/\|/gu, '\\|')).join(' | ')} |`;
}

export function formatSceneMarkdownReport(report: SceneRunReport) {
	const lines = [
	...
	if (report.diagnostics.length > 0) {
		lines.push('## Diagnostics', '', ...formatSceneDiagnostics(report.diagnostics).map((line) => `- ${line}`), '');
	}
...
function setupSummary(report: SceneRunReport) {
...
export function writeSceneMarkdownReport(report: SceneRunReport) {
	if (!report.artifacts) return;
	writeFileSync(report.artifacts.markdownReportPath, formatSceneMarkdownReport(report), 'utf8');
}
