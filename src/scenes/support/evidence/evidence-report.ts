...
export function formatSceneEvidenceMarkdownReport(manifest: SceneEvidenceManifest) {
	const lines = [
...
export function writeSceneEvidenceReport(input: {
	manifest: SceneEvidenceManifest;
	paths: SceneEvidencePaths;
}) {
	writeJson(input.paths.manifestPath, input.manifest);
	writeText(input.paths.reportPath, formatSceneEvidenceMarkdownReport(input.manifest));
}

export function appendSceneEvidencePaths(input: {
	runPath: string;
	paths: SceneEvidencePaths;
}): SceneDiagnostic[] {
	try {
		...
		const run = JSON.parse(readFileSync(input.runPath, 'utf8')) as SceneRunReport & {
			evidencePaths?: SceneEvidencePaths;
