...
function artifactId(kind: SceneEvidenceArtifactKind, relativePath: string) {
...
function createArtifact(input: {
	...
	redactionStatus?: SceneEvidenceArtifact['redactionStatus'];
}): SceneEvidenceArtifact | null {
...
function pushArtifact(artifacts: SceneEvidenceArtifact[], artifact: SceneEvidenceArtifact | null) {
	if (!artifact) return;
...
function buildRecommendations(input: {
	...
	const recommendations: SceneEvidenceRecommendation[] = [];
...
export function buildSceneEvidenceManifest(input: {
	scene: SceneManifest;
	run: SceneRunReport;
...
export function writeSceneEvidence(input: {
	...
	writeSceneEvidenceReport({ manifest: input.manifest, paths });
	if (input.manifest.bundlePolicy === 'sanitized') writeSceneEvidenceBundle({ manifest: input.manifest, paths });
	return paths;
}
...
export function generateSceneEvidence(input: SceneEvidenceOptions): SceneEvidenceReport {
	...
	try {
		...
		const paths = writeSceneEvidence({ runRoot: resolved.runRoot, manifest });
		const updateWarnings = appendSceneEvidencePaths({ runPath, paths });
		return {
			ok: true,
			phase: 9,
