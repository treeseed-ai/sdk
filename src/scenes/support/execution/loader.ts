...
export function resolveScenePath(projectRoot: string, sceneNameOrPath: string) {
...
export function loadSceneDocument(projectRoot: string, sceneNameOrPath: string): LoadedSceneDocument {
	const diagnostics: SceneDiagnostic[] = [];
	const trimmed = sceneNameOrPath.trim();
	const path = resolveScenePath(projectRoot, trimmed);
