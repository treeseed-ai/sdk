...
function blocked(input: SceneResumeOptions, diagnostics: ReturnType<typeof sceneErrorDiagnostic>[]): SceneRunReport {
...
export async function resumeScene(input: SceneResumeOptions): Promise<SceneRunReport> {
	...
	const checkpoint = readJson<SceneCheckpoint>(checkpointPath);
