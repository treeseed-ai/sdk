...
function addRoute(
	routes: Map<string, SceneVisualAuditRoute>,
	input: {
		path: string;
...
function discoverCoreRoutes(projectRoot: string, routes: Map<string, SceneVisualAuditRoute>, diagnostics: SceneDiagnostic[]) {
	const candidates = [
		resolve(projectRoot, 'packages/core/src/support/routes.ts'),
		resolve(projectRoot, 'packages/core/src/site.ts'),
	];
	const routesPath = candidates.find((path) => existsSync(path));
	if (!routesPath) {
		diagnostics.push(sceneWarningDiagnostic('scene.visual_audit_route_discovery_failed', `Core route registry not found at ${candidates[0]}.`, 'visualAudit.routeDiscovery.core'));
		return;
	}
	for (const pattern of extractRoutePatterns(readComposedModule(routesPath))) {
...
function discoverAdminRoutes(projectRoot: string, routes: Map<string, SceneVisualAuditRoute>, diagnostics: SceneDiagnostic[]) {
...
function discoverTenantOverrides(projectRoot: string, routes: Map<string, SceneVisualAuditRoute>) {
...
function discoverContentRoutes(projectRoot: string, routes: Map<string, SceneVisualAuditRoute>) {
...
export function discoverSceneVisualAuditRoutes(input: {
	projectRoot: string;
	scene: SceneManifest;
	pathRoots?: string[];
	pathGlobs?: string[];
	...
	const routes = new Map<string, SceneVisualAuditRoute>();
...
export function SceneVisualAuditPathRoot(path: string) {
	return pathRootFor(path);
}

export function SceneVisualAuditRouteFilename(path: string) {
