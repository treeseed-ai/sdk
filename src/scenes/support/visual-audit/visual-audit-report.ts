...
function firstDiagnostic(capture: SceneVisualAuditCapture) {
	return capture.diagnostics[0] ?? null;
}

export function formatSceneVisualAuditMarkdown(input: {
...
export function writeSceneVisualAuditReport(input: {
	manifest: SceneVisualAuditManifest;
	paths: SceneVisualAuditPaths;
	review?: SceneVisualAuditReview | null;
}) {
	...
	writeFileSync(input.paths.reportPath, formatSceneVisualAuditMarkdown(input), 'utf8');
}
