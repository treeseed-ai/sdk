...
function hasError(diagnostics: SceneDiagnostic[]) {
	return diagnostics.some((entry) => entry.severity === 'error');
}

function defaultRenderMode(scene: SceneManifest, run: SceneRunReport): SceneRenderMode {
...
export function defaultSceneRemotionComposition(mode: SceneRenderMode) {
...
function renderSettings(scene: SceneManifest, run: SceneRunReport, mode: SceneRenderMode, composition: string | undefined, format: SceneRenderFormat | undefined, durationMs: number) {
	...
	return {
		mode,
		composition: composition ?? remotion?.composition ?? defaultSceneRemotionComposition(mode),
		fps,
		width,
		height,
...
function renderSettingsWithLayout(
	...
	renderDiagrams: ReturnType<typeof buildSceneRenderDiagrams>['diagrams'],
) {
...
function captureViewportForRender(scene: SceneManifest, run: SceneRunReport) {
...
function durationMsFor(input: { run: SceneRunReport; timeline: SceneTimelineEvent[]; chapterId?: string }) {
	if (input.chapterId) {
...
function readCheckpoints(run: SceneRunReport): SceneCheckpoint[] {
	const root = run.artifacts?.checkpointsRoot;
	if (!root || !existsSync(root)) return run.checkpoints ?? [];
	return readdirSync(root)
		.filter((entry) => entry.endsWith('.json'))
		.sort()
		.map((entry) => readJson<SceneCheckpoint>(join(root, entry)));
}

function readSegmentPaths(run: SceneRunReport) {
...
function screenshotsFromRun(run: SceneRunReport, timeline: SceneTimelineEvent[]) {
	const timelineByStep = new Map<string, SceneTimelineEvent>();
...
function videoRefFromPath(path: string): NonNullable<SceneRenderInput['media']['videoRefs']>[number] | null {
...
function legacyFullPageScreenshotsFromArtifacts(run: SceneRunReport, timeline: SceneTimelineEvent[]) {
...
function filterForChapter(run: SceneRunReport, timeline: SceneTimelineEvent[], chapterId: string | undefined) {
...
function filterDiagramsForChapter(diagrams: ReturnType<typeof buildSceneRenderDiagrams>['diagrams'], run: SceneRunReport, chapterId: string | undefined) {
...
export function loadSceneRenderInput(input: {
	projectRoot: string;
	scene: string;
	from: string;
	mode?: SceneRenderMode;
	...
	const scene = readJson<SceneManifest>(scenePath);
	...
	if (input.device && !run.device?.id) {
		const resolvedDevice = resolveSceneDeviceProfile({ scene, device: input.device });
	...
	if (renderer === 'remotion' && !listSceneRemotionCompositions().some((composition) => composition.id === settings.composition)) {
	...
	const renderDiagramReport = buildSceneRenderDiagrams({
		scene,
		run,
	...
	const videoRefs = sourceVideos.map(videoRefFromPath).filter((path): path is NonNullable<SceneRenderInput['media']['videoRefs']>[number] => Boolean(path));
	...
	const trainingOutputs = buildSceneTrainingOutputs({ scene, run, timeline: filtered?.timeline ?? timeline });

	const renderInput: SceneRenderInput = {
		...
		segments: filtered?.segments as SceneRunSegmentReport[] ?? run.segments ?? [],
