...
function pointStyle(point?: SceneVisualPoint): React.CSSProperties {
	if (!point) return {};
	return { left: unitValue(point.x, point.unit), top: unitValue(point.y, point.unit) };
}

function sizeStyle(size?: SceneVisualSize): React.CSSProperties {
	if (!size) return {};
	return { width: unitValue(size.width, size.unit), height: unitValue(size.height, size.unit) };
}

export function resolveVisualRegion(region: SceneVisualRegion | undefined, margin = 48): React.CSSProperties {
...
function toneColors(style?: SceneVisualStyle) {
	if (style?.tone === 'success') return { accent: SceneVideoTheme.success, background: 'rgba(20,83,45,0.88)' };
	if (style?.tone === 'warning') return { accent: SceneVideoTheme.warning, background: 'rgba(120,53,15,0.9)' };
	if (style?.tone === 'danger') return { accent: SceneVideoTheme.danger, background: 'rgba(127,29,29,0.9)' };
	if (style?.tone === 'brand') return { accent: SceneVideoTheme.brand, background: 'rgba(19,78,74,0.9)' };
	if (style?.tone === 'info') return { accent: SceneVideoTheme.accent, background: 'rgba(12,74,110,0.9)' };
	return { accent: SceneVideoTheme.accent, background: SceneVideoTheme.panel };
}

export function visualStyle(style?: SceneVisualStyle): React.CSSProperties {
	...
	return {
		...
		color: style?.color ?? SceneVideoTheme.text,
...
export function renderVisualObject(input: {
	object: SceneVisualObject;
	frame: number;
	fps: number;
	durationSeconds: number;
	...
	if (object.type === 'line' || object.type === 'arrow') {
		...
		return React.createElement('div', {
			...
			style: {
				...
				background: object.style?.color ?? object.style?.borderColor ?? SceneVideoTheme.accent,
			...
			style: {
				...
				borderLeft: `14px solid ${object.style?.color ?? object.style?.borderColor ?? SceneVideoTheme.accent}`,
			},
		}) : null);
	}
	if (object.type === 'cursor') {
		return React.createElement('div', {
			...
			style: {
				...
				border: 0,
				boxShadow: 'none',
				color: object.style?.color ?? SceneVideoTheme.text,
				fontSize: 30,
			},
		}, '▸');
	}
