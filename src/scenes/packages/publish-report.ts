...
export function formatScenePublishMarkdownReport(manifest: ScenePublishManifest) {
...
export function writeScenePublishReport(input: {
	manifest: ScenePublishManifest;
	paths: ScenePublishPaths;
}) {
	writeJson(input.paths.manifestPath, input.manifest);
	writeText(input.paths.reportPath, formatScenePublishMarkdownReport(input.manifest));
}

export function appendScenePublishPaths(input: {
	runPath: string;
	paths: ScenePublishPaths;
}): SceneDiagnostic[] {
	try {
		...
		const run = JSON.parse(readFileSync(input.runPath, 'utf8')) as SceneRunReport & {
			publishPaths?: ScenePublishPaths;
