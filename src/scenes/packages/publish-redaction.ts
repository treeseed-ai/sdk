...
const RUN_STATUSES = new Set<SceneRunStatus>(['passed', 'failed', 'blocked', 'skipped']);
const PUBLISH_TARGETS = new Set<ScenePublishTarget>(['local', 'release']);
...
export function createDefaultSceneRedactionPolicy(target: ScenePublishTarget): SceneRedactionPolicy {
	void target;
	const include = (artifactKind: SceneEvidenceArtifactKind, reason: string): SceneRedactionRule => ({
...
export function validateSceneRedactionPolicy(input: {
	policy: unknown;
	path?: string;
}): SceneDiagnostic[] {
	...
	for (const [index, rawRule] of policy.rules.entries()) {
		...
		if (typeof rawRule.artifactKind !== 'string' || !EVIDENCE_ARTIFACT_KINDS.has(rawRule.artifactKind as SceneEvidenceArtifactKind)) {
		...
		if (rawRule.allowWhen !== undefined) {
			if (!isRecord(rawRule.allowWhen)) {
				...
				if (targets !== undefined) {
					...
					if (!values || values.some((value) => !PUBLISH_TARGETS.has(value as ScenePublishTarget))) {
				...
				if (statuses !== undefined) {
					...
					if (!values || values.some((value) => !RUN_STATUSES.has(value as SceneRunStatus))) {
...
export function readSceneRedactionPolicyFile(path: string): {
	policy: SceneRedactionPolicy | null;
	diagnostics: SceneDiagnostic[];
} {
	try {
		const raw = readFileSync(path, 'utf8');
		const parsed = extname(path).toLowerCase() === '.json' ? JSON.parse(raw) : parseYaml(raw);
		const diagnostics = validateSceneRedactionPolicy({ policy: parsed, path });
		return { policy: diagnostics.length > 0 ? null : parsed as SceneRedactionPolicy, diagnostics };
	} catch (error) {
		return {
			policy: null,
...
export function resolveSceneRedactionRule(input: {
	policy: SceneRedactionPolicy;
	artifactKind: SceneEvidenceArtifactKind;
	target: ScenePublishTarget;
	workflowStatus: SceneRunStatus;
}): SceneRedactionRule | null {
