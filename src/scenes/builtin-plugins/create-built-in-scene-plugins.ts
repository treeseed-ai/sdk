...
export function createBuiltInScenePlugins(): ScenePlugin[] {
	return [
		...
		},
		{
			...
			environment: {
				prepare: prepareSceneEnvironment,
				resolveAuth: resolveSceneAuth,
				prepareSeed: planOrApplySceneSeed,
				resolveBaseUrl: resolveSceneBaseUrl,
			},
		},
		{
		...
		},
		{
			...
			diagrams: {
				'treeseed-remotion-diagrams': createBuiltInSceneDiagramProvider(),
			},
		},
		{
			...
			narration: {
				'deterministic-narration': {
					...
					generate({ scene, run, transcript, style }) {
						return buildSceneNarrationEntries({ scene, run, transcript, style });
					},
				},
			},
