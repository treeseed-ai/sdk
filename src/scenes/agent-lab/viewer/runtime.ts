export function simulationViewerRuntime() {
	function boot() {
		...
		const formatAgentTime = (seconds: any) => { const value = Math.max(0,num(seconds)); return value >= 3600 ? (value/3600).toFixed(value % 3600 ? 1 : 0) + 'h' : value >= 60 ? Math.round(value/60) + 'm' : value + 's'; };
		const capacityStrip = (capacity: any = {}) => {
			const values = [['Requested', capacity.requestedSeconds], ['Reserved', capacity.reservedSeconds], ['Active', capacity.activeSeconds], ['Elapsed', capacity.elapsedSeconds], ['Released', capacity.releasedSeconds], ['Overrun', capacity.overrunSeconds]];
			return '<div class="time-strip" aria-label="Assignment time accounting">' + values.map(([label, value]) => '<div><span>' + label + '</span><strong>' + (value === null || value === undefined ? 'not reported' : esc(formatAgentTime(value))) + '</strong></div>').join('') + '</div>';
		};
		...
		const entityDetail = (kind: string, id: string) => {
			...
			if (kind === 'agent') { const item = (data.agents || []).find((entry: any) => String(entry.id) === id), runs = executions().filter((run: any) => run.agentId === id), profiles = item?.activityProfiles || []; return item ? '<section class="detail-lead"><h3>' + esc(item.title) + '</h3><p>' + esc(item.description) + '</p></section>' + capacityStrip({ requestedSeconds: runs.reduce((sum: number,run: any) => sum + num(run.capacity?.requestedSeconds),0), reservedSeconds: runs.reduce((sum: number,run: any) => sum + num(run.capacity?.reservedSeconds),0), activeSeconds: runs.reduce((sum: number,run: any) => sum + num(run.capacity?.activeSeconds),0), elapsedSeconds: runs.reduce((sum: number,run: any) => sum + num(run.capacity?.elapsedSeconds),0) }) + structured({ identity: item.identity, class: item.classTitle || item.classId, capabilities: item.capabilities, enabledHandlers: profiles.filter((profile: any) => profile.enabled).map((profile: any) => ({ activityProfile: profile.activityType, handler: profile.handlerId })), activityProfiles: profiles, executions: runs.length, tokenUsage: runs.map((run: any) => run.capacity) }) : '<div class="empty">Agent unavailable.</div>'; }
