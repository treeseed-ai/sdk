...
export async function runScene(input: SceneRunOptions): Promise<SceneRunReport> {
	...
	const environmentAdapter = input.environmentAdapter ?? environmentProvider?.prepare ?? prepareSceneEnvironment;
	...
	const setup: SceneRunSetupReport = { environment: environmentReport, auth: authReport, seed: seedReport };
	...
	if (setupDiagnostics.some((entry) => entry.severity === 'error')) {
		...
		appendBlockedSceneLogs(report, input, artifacts, environmentReport);
	...
	const chapters = createSceneChapterReports(scene);
	...
	try {
		...
		for (const step of scene.workflow) {
			...
			if (chapter && chapter.id !== currentChapterId) {
				if (currentSegment) {
					finishSceneSegment(currentSegment, currentSegment.status);
				...
				currentSegment = createSceneSegment({ segmentsRoot: paths.segmentsRoot, chapterId: chapter.id, index: nextIndex });
			...
			let stepDiagnostic: SceneDiagnostic | undefined;
			let assertionResults: SceneAssertionRunReport[] = [];
			...
			const stepReport: SceneRunStepReport = {
				id: step.id,
				title: step.title,
				actionKind,
			...
			if (status === 'passed') {
				...
				if (runtime.checkpoints.enabled && runtime.checkpoints.everyStep) {
					...
					if (checkpoint.resumable && currentSegment) {
						...
						currentSegment = createSceneSegment({ segmentsRoot: paths.segmentsRoot, chapterId: checkpoint.chapterId, index: nextIndex });
						segments.push(currentSegment);
						chapterReport?.segmentIds.push(currentSegment.id);
