...
import type { TreeseedScenePublishManifest } from './types.ts';

function safePathPart(value: string | null) {
	return (value ?? 'unknown').toLowerCase().replace(/[^a-z0-9._-]/gu, '-').replace(/^-+|-+$/gu, '') || 'unknown';
}

export function treeseedSceneReleaseEvidenceRecordPath(input: {
	projectRoot: string;
	sceneId: string | null;
	runId: string | null;
}) {
...
export function writeTreeseedSceneReleaseEvidenceRecord(input: {
	projectRoot: string;
	...
	const path = treeseedSceneReleaseEvidenceRecordPath({
