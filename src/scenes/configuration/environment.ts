...
export async function prepareSceneEnvironment(input: SceneEnvironmentPrepareOptions): Promise<SceneEnvironmentPrepareReport> {
	...
	try {
		readiness = collectDeploymentReadiness({ tenantRoot: input.projectRoot, environment: input.environment });
	...
	if (requested) {
		...
		if (running(existing) && healthUrl(existing) && running(existingApi) && healthUrl(existingApi)) {
			...
			try {
				const result = await startManagedDev({ cwd: input.projectRoot, surfaces: 'web,api', webRuntime: 'local', env: input.env });
