...
import { MarketClient, resolveMarketProfile, resolveMarketSession } from '../market-client.ts';
...
import type { TreeseedSceneSeedOptions, TreeseedSceneSeedReport } from './types.ts';
...
export async function planOrApplyTreeseedSceneSeed(input: TreeseedSceneSeedOptions): Promise<TreeseedSceneSeedReport> {
	...
	try {
		if (environments.some((environment) => environment !== 'local')) {
			...
			const authRoot = findNearestTreeseedRoot(input.projectRoot) ?? input.projectRoot;
