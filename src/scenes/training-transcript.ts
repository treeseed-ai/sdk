...
function chapterForStep(run: TreeseedSceneRunReport, stepId: string) {
...
export function buildTreeseedSceneTranscriptEntries(input: {
	scene: TreeseedSceneManifest;
	run: TreeseedSceneRunReport;
	timeline: TreeseedSceneTimelineEvent[];
}): TreeseedSceneTranscriptEntry[] {
	const entries: TreeseedSceneTranscriptEntry[] = [{
		id: 'scene',
		timestampMs: 0,
		type: 'scene',
...
function scriptFor(entry: TreeseedSceneTranscriptEntry, style: TreeseedSceneTrainingNarrationStyle) {
...
export function buildTreeseedSceneNarrationEntries(input: {
	scene: TreeseedSceneManifest;
	run: TreeseedSceneRunReport;
...
export function buildTreeseedSceneGlossary(input: {
	scene: TreeseedSceneManifest;
	transcript: TreeseedSceneTranscriptEntry[];
}): TreeseedSceneGlossaryTerm[] {
	const explicit = new Map<string, TreeseedSceneGlossaryTerm>();
	...
	const terms = new Map<string, TreeseedSceneGlossaryTerm>();
...
export function buildTreeseedSceneChapterClips(input: {
	scene: TreeseedSceneManifest;
	run: TreeseedSceneRunReport;
	timeline: TreeseedSceneTimelineEvent[];
}): TreeseedSceneChapterClipManifest[] {
...
export function formatTreeseedSceneTranscriptMarkdown(entries: TreeseedSceneTranscriptEntry[]) {
...
export function formatTreeseedSceneNarrationMarkdown(entries: TreeseedSceneNarrationScriptEntry[]) {
...
export function formatTreeseedSceneGlossaryMarkdown(terms: TreeseedSceneGlossaryTerm[]) {
