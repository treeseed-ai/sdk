...
import { renderTreeseedSceneDiagram } from './remotion-diagrams.ts';
import { interpolateSceneMotion, renderTreeseedVisualObject, resolveVisualRegion, treeseedSceneVideoTheme, visualStyle } from './remotion-visuals.ts';
import type { TreeseedSceneRenderInput } from './types.ts';

function titleCard(input: TreeseedSceneRenderInput, subtitle: string) {
...
export function treeseedSceneRenderLayout(input: TreeseedSceneRenderInput) {
...
export function resolveEvidenceViewport(input: TreeseedSceneRenderInput): {
	width: number;
	height: number;
	scale: number;
	left: number;
...
function eventStepOffset(input: TreeseedSceneRenderInput, stepId: string, type: 'step.start' | 'step.end') {
	return input.timeline.find((event) => event.type === type && event.stepId === stepId)?.offsetMs ?? null;
}

function stepAtEvidenceMs(input: TreeseedSceneRenderInput, evidenceMs: number) {
...
function stepStartMs(input: TreeseedSceneRenderInput, stepId: string) {
...
function evidenceTimelineOffsetMs(input: TreeseedSceneRenderInput) {
...
function evidenceSourceMs(input: TreeseedSceneRenderInput, frame: number) {
	const evidenceMs = Math.floor((frame / Math.max(1, input.render.fps)) * 1000);
	return evidenceMs + evidenceTimelineOffsetMs(input);
}

function activeOverlayText(input: TreeseedSceneRenderInput, evidenceMs: number) {
...
function activeOverlayDiagram(input: TreeseedSceneRenderInput, evidenceMs: number) {
...
function activeInterstitialDiagram(input: TreeseedSceneRenderInput, frame: number) {
	let cursor = 0;
...
function EvidenceFrame({ input, durationFrames }: { input: TreeseedSceneRenderInput; durationFrames: number }) {
...
function BrowserFrame({ input }: { input: TreeseedSceneRenderInput }) {
...
function LowerThird({ input }: { input: TreeseedSceneRenderInput }) {
	const frame = useCurrentFrame();
	const evidenceMs = evidenceSourceMs(input, frame);
...
function CaptionOverlay({ input }: { input: TreeseedSceneRenderInput }) {
	const frame = useCurrentFrame();
...
function Callouts({ input }: { input: TreeseedSceneRenderInput }) {
	const frame = useCurrentFrame();
	const evidenceMs = evidenceSourceMs(input, frame);
...
function OverlayDiagram({ input }: { input: TreeseedSceneRenderInput }) {
	const frame = useCurrentFrame();
	const evidenceMs = evidenceSourceMs(input, frame);
...
function ChapterBanner({ input }: { input: TreeseedSceneRenderInput }) {
	const frame = useCurrentFrame();
...
function renderShell(input: TreeseedSceneRenderInput, subtitle: string) {
...
function InterstitialDiagrams({ input, width, height }: { input: TreeseedSceneRenderInput; width: number; height: number }) {
	const frame = useCurrentFrame();
...
function DiagramOnly(input: TreeseedSceneRenderInput) {
	const frame = useCurrentFrame();
	const config = useVideoConfig();
...
export function TreeseedSceneRemotionRoot() {
	const defaultProps = {
	...
	} as unknown as TreeseedSceneRenderInput;
	const compositions = [
	...
	return React.createElement(React.Fragment, null, ...compositions.map(([id, component]) => React.createElement(Composition, {
		...
		defaultProps,
		calculateMetadata: ({ props }: { props: TreeseedSceneRenderInput }) => ({
