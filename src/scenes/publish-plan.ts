...
import {
	DEFAULT_TREESEED_SCENE_PUBLICATION_TARGETS,
	createTreeseedScenePublishDestinations,
	destinationIdsForTreeseedScenePublishedArtifact,
	isTreeseedSceneExternalPublishTarget,
...
import { publishTreeseedSceneEvidence } from './publish.ts';
import { appendTreeseedScenePublishPlanPaths, writeTreeseedScenePublishPlanReport } from './publish-plan-report.ts';
import type {
...
function validateTargets(targets: TreeseedSceneExternalPublishTarget[]): TreeseedSceneDiagnostic[] {
	return targets
		.filter((target) => !isTreeseedSceneExternalPublishTarget(target))
...
export function buildTreeseedScenePublishPlanManifest(input: {
	publish: TreeseedScenePublishManifest;
	...
	const validTargets = input.targets.filter(isTreeseedSceneExternalPublishTarget);
	const destinations = createTreeseedScenePublishDestinations({ runRoot: input.runRoot, targets: validTargets });
	const artifacts: TreeseedScenePublishPlanArtifact[] = input.publish.artifacts.map((artifact) => ({
		id: artifact.id,
		kind: artifact.kind,
		sourcePath: artifact.publishedPath ?? artifact.sourcePath,
		relativePath: artifact.relativePath,
		destinationIds: destinationIdsForTreeseedScenePublishedArtifact({ artifact, targets: validTargets }),
...
export function writeTreeseedScenePublishPlan(input: {
	runRoot: string;
	manifest: TreeseedScenePublishPlanManifest;
}): TreeseedScenePublishPlanPaths {
	...
	const paths: TreeseedScenePublishPlanPaths = {
		publishPlanRoot,
	...
	writeTreeseedScenePublishPlanReport({ manifest: input.manifest, paths });
	return paths;
}
...
async function loadOrGenerateLocalPublish(input: {
	...
	publish: TreeseedScenePublishManifest | null;
	publishManifestPath: string;
	diagnostics: TreeseedSceneDiagnostic[];
	warnings: TreeseedSceneDiagnostic[];
}> {
	...
	if (existsSync(publishManifestPath)) {
		return { publish: readJson<TreeseedScenePublishManifest>(publishManifestPath), publishManifestPath, diagnostics: [], warnings: [] };
	}
...
async function runPublicationPlan(input: TreeseedScenePublishPlanOptions & { mode: TreeseedScenePublishPlanMode }): Promise<TreeseedScenePublishPlanReport> {
	...
	const targets = input.targets ?? DEFAULT_TREESEED_SCENE_PUBLICATION_TARGETS;
	...
	const manifest = buildTreeseedScenePublishPlanManifest({
	...
	try {
		const paths = writeTreeseedScenePublishPlan({ runRoot: resolved.runRoot, manifest });
		const updateWarnings = appendTreeseedScenePublishPlanPaths({ runPath, paths });
		return {
			ok: true,
			phase: 11,
			sceneId: sourceScene.id,
...
export function planTreeseedScenePublication(input: TreeseedScenePublishPlanOptions): Promise<TreeseedScenePublishPlanReport> {
	return runPublicationPlan({ ...input, mode: input.mode ?? 'plan' });
}

export function exportTreeseedScenePublication(input: Omit<TreeseedScenePublishPlanOptions, 'mode'>): Promise<TreeseedScenePublishPlanReport> {
