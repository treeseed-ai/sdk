...
export function resolveTreeseedSceneRunRoot(projectRoot: string, run: string) {
		...
		for (const sceneId of readdirSync(runsRoot)) {
			...
			for (const runDir of readdirSync(sceneRoot)) {
				...
				if (basename(runRoot).endsWith(`-${run}`)) matches.push(runRoot);
					...
					try {
						const report = readJson<TreeseedSceneRunReport>(join(runRoot, 'run.json'));
...
export function inspectTreeseedSceneRun(input: TreeseedSceneInspectOptions): TreeseedSceneInspectReport {
	...
	const timeline = existsSync(run.timelinePath ?? '') ? readJson<TreeseedSceneTimelineEvent[]>(run.timelinePath!) : [];
	const checkpointsRoot = run.artifacts?.checkpointsRoot;
	const checkpoints: TreeseedSceneCheckpoint[] = checkpointsRoot && existsSync(checkpointsRoot)
		? readdirSync(checkpointsRoot).filter((entry) => entry.endsWith('.json')).map((entry) => readJson<TreeseedSceneCheckpoint>(join(checkpointsRoot, entry)))
		: run.checkpoints ?? [];
