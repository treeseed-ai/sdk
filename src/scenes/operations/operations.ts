...
function operationIdFrom(input: SceneOperationWaitOptions) {
...
async function defaultFetchEvents(input: SceneOperationWaitOptions, operationId: string) {
	if (input.fetchEvents) return input.fetchEvents(operationId);
	return [];
}

export async function waitForSceneOperation(input: SceneOperationWaitOptions): Promise<SceneOperationWaitReport> {
...
export function extractSceneOperationIds(value: unknown): string[] {
