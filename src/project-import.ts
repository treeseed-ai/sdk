...
export { githubRepositoryCredentialEnvName } from './operations/services/github-credentials.ts';

export type TreeseedRepositoryImportProvider = 'github';
export type TreeseedRepositoryImportVisibility = 'public' | 'private';

export interface TreeseedGitHubRepositoryObservation {
	provider?: TreeseedRepositoryImportProvider | string;
	owner?: string;
	name?: string;
	repository?: string;
	defaultBranch?: string;
	visibility?: TreeseedRepositoryImportVisibility | string;
	htmlUrl?: string;
	cloneUrl?: string;
	files?: string[];
	directories?: string[];
...
export interface TreeseedRepositoryImportInput {
	team: string;
	repository: string;
	observation?: TreeseedGitHubRepositoryObservation;
	rootPath?: string;
	sitePath?: string;
...
export interface TreeseedRepositoryImportDiagnostic {
...
export interface TreeseedRepositoryImportPathCandidates {
	rootPath: string[];
	sitePath: string[];
	contentPath: string[];
	manifests: string[];
...
export interface TreeseedRepositoryImportPlan {
	ok: boolean;
	provider: TreeseedRepositoryImportProvider;
	team: string;
	repository: {
		owner: string;
		name: string;
		slug: string;
		defaultBranch: string;
		visibility: TreeseedRepositoryImportVisibility;
		htmlUrl: string;
		cloneUrl: string;
	};
	credentialRef: string;
	...
	candidates: TreeseedRepositoryImportPathCandidates;
	diagnostics: TreeseedRepositoryImportDiagnostic[];
	plannedRecords: {
		project: {
			slug: string;
			name: string;
...
function selectSitePath(files: Set<string>, directories: Set<string>, override: string, diagnostics: TreeseedRepositoryImportDiagnostic[]) {
...
function selectContentPath(files: Set<string>, directories: Set<string>, override: string, sitePath: string, diagnostics: TreeseedRepositoryImportDiagnostic[]) {
	if (override) return override;
...
function discoverCandidates(files: Set<string>, directories: Set<string>): TreeseedRepositoryImportPathCandidates {
	return {
		rootPath: ['.'],
...
function normalizeCredentialRef(input: TreeseedRepositoryImportInput, repositorySlug: string) {
	...
	const envName = resolution.source === 'repository' ? resolution.envName : TREESEED_GITHUB_TOKEN_ENV;
	return {
...
export function planTreeseedRepositoryImport(input: TreeseedRepositoryImportInput): TreeseedRepositoryImportPlan {
	...
	const diagnostics: TreeseedRepositoryImportDiagnostic[] = [];
	...
	if (!credential.credential.configured) {
		diagnostics.push({
			...
			message: `No configured GitHub token was observed for ${credential.credential.repositoryScopedRef} or env:${TREESEED_GITHUB_TOKEN_ENV}. The import plan is safe, but execute may require credentials.`,
		});
	}
