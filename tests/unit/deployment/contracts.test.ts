...
describe('component-owned PostgreSQL requirements', () => {
	...
	it('declares application requirements without provisioning another server', () => {
		...
		const value = packageRuntimeSchema.parse({ ...base, services: [...base.services, { id: 'migration', composeService: 'migration', endpoints: [] }], postgresRequirements: [requirement],
			postgresLifecycle: [{ requirementId: 'api', credentialOwner: { uid: 65532, gid: 65532 }, migration: { composeService: 'migration', completion: 'exit-zero', timeoutSeconds: 300 }, runtimeServices: ['service'] }] });
...
describe('deployment contracts', () => {
	it('binds host database allocations to enabled components and OS custody', () => {
		...
		selected.runtime.postgresLifecycle = [{ requirementId: 'api', credentialOwner: { uid: 65532, gid: 65532 }, migration: { composeService: 'migration', completion: 'exit-zero', timeoutSeconds: 300 }, runtimeServices: ['service'] }];
