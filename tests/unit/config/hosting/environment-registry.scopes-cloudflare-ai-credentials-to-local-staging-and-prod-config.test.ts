...
it('scopes Cloudflare AI credentials to local, staging, and prod config', async () => {
		...
		expect(isEnvironmentEntryRelevant(cloudflareToken!, registry.context, 'local', 'config')).toBe(true);
		expect(isEnvironmentEntryRequired(cloudflareToken!, registry.context, 'local', 'config')).toBe(true);
