...
it('gates hosted smtp and turnstile entries on deploy config enablement', async () => {
		...
		const disabledRegistry = resolveEnvironmentRegistry({
			deployConfig,
			plugins: [],
		});
		...
		expect(isEnvironmentEntryRelevant(smtpHost!, disabledRegistry.context, 'staging', 'config')).toBe(false);
		expect(isEnvironmentEntryRelevant(turnstileSecret!, disabledRegistry.context, 'prod', 'config')).toBe(false);
		...
		expect(isEnvironmentEntryRelevant(enabledSmtpHost!, enabledRegistry.context, 'staging', 'config')).toBe(true);
		expect(isEnvironmentEntryRelevant(enabledTurnstileSecret!, enabledRegistry.context, 'prod', 'config')).toBe(false);
		expect(isEnvironmentEntryRelevant(enabledTurnstileSecret!, enabledRegistry.context, 'prod', 'deploy')).toBe(true);
		...
		expect(isEnvironmentEntryRelevant(enabledTurnstileSecret!, enabledRegistry.context, 'local', 'config')).toBe(false);
		expect(isEnvironmentEntryRequired(enabledTurnstileSecret!, enabledRegistry.context, 'staging', 'config')).toBe(false);
