...
describe('TreeAI SDK adoption', () => {
	it('maps every authoritative operation exactly once', () => {
		...
		expect(TREEAI_UPSTREAM_OPERATIONS).toHaveLength(96);
		expect(TREEAI_UPSTREAM_OPERATIONS.some(operation => String(operation.service) === 'qualification')).toBe(false);
	...
	it('emits an immutable adoption receipt', async () => {
		...
		expect(receipt).toMatchObject({ schema: 'treeseed.treeai-service-contract/v1', release: '0.11.0-rc22', openapiVersion: '3.1.1' });
