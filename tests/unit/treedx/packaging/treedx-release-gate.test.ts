...
function createTreeDxAdapterFixture() {
  ...
  return { root, adapter: findPackageAdapter(root, "treedx") };
}
...
describe("TreeDX release gate integration", () => {
  it("keeps package-local OpenAPI files for standalone SDK verification", () => {
    ...
    expect(
      read(resolve(sdkRoot, 'scripts/treedx/repositories/generate-treedx-openapi-types.ts')),
  ...
  it("does not use skipped Vitest tests in the SDK package", () => {
    const sources = listTests(resolve(sdkRoot, "tests"));
  ...
  it("renders TreeDX workflows without Node package install assumptions", () => {
    const adapter =
      findPackageAdapter(workspaceRoot, "treedx") ??
      createTreeDxAdapterFixture().adapter;
    expect(adapter).toBeTruthy();

    const releaseGate = renderPackageWorkflow(adapter!, "release-gate");
    const publish = renderPackageWorkflow(adapter!, "docker-image");
    ...
    expect(releaseGate).toContain('bash scripts/release/release-gate.sh');
  ...
  it("keeps TreeDX releases on the restored package-owned release gate", () => {
    ...
    const adapter = findPackageAdapter(workspaceRoot, "treedx") ?? fixture.adapter;
    ...
    expect(syncPackageWorkflows({ root, packageId: "treedx" })).toEqual([]);
  });
