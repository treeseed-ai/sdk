...
} from '../../../../../src/operations/services/treedx/workspaces/workspace-dependency-mode.ts';
...
function createWorkspace() {
	...
	writeJson(resolve(root, 'packages/sdk/package.json'), {
		...
		version: '0.1.0',
		bin: {
			'treeseed-sdk-verify': './dist/scripts/verification/verify-driver.js',
		},
	});
	mkdirSync(resolve(root, 'packages/sdk/dist/scripts/verification'), { recursive: true });
	writeFileSync(resolve(root, 'packages/sdk/dist/scripts/verification/verify-driver.js'), '#!/usr/bin/env node\n', 'utf8');
...
describe('workspace dependency mode', () => {
	...
	it('repairs stale package bin links when restoring workspace links', () => {
		...
		expect(readlinkSync(binLink)).toContain('../@treeseed/sdk/dist/scripts/verification/verify-driver.js');
	});
	...
	it('allows exact internal dev git lockfile refs for matching root semver ranges', () => {
		const root = createWorkspace();
		writeJson(resolve(root, 'package.json'), {
			name: '@treeseed/market',
			version: '1.0.0',
			workspaces: ['packages/*'],
			dependencies: {
				'@treeseed/core': '^0.1.0-dev.staging.20260427T000000Z',
				'@treeseed/cli': '^0.1.0-dev.staging.20260427T000000Z',
			},
		});
		writeJson(resolve(root, 'package-lock.json'), {
			name: '@treeseed/market',
