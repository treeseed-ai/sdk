...
import {
	...
	importKnowledgePack,
} from '../../../src/operations/services/market-packaging.ts';

function createProjectRoot() {
	const root = mkdtempSync(join(tmpdir(), 'treeseed-market-packaging-'));
	...
	writeFileSync(resolve(root, 'package.json'), `${JSON.stringify({ name: 'sample-project', version: '1.2.3' }, null, 2)}\n`, 'utf8');
	writeFileSync(resolve(root, 'treeseed.site.yaml'), 'name: Sample Project\nslug: sample-project\n', 'utf8');
