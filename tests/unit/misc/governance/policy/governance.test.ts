...
} from '../../../../../src/governance/policy/governance.ts';
...
async function snapshot(provider: typeof BicameralProvider, config: Record<string, unknown> = {}): Promise<GovernanceElectorateSnapshot> {
...
describe('governance voting providers', () => {
	...
	it('requires both bicameral chambers to pass', async () => {
		const electorate = await snapshot(BicameralProvider, {
		...
		const outcome = BicameralProvider.evaluate({
			electorate,
			votes: [vote('u1', 'support'), vote('u2', 'support')],
		});
