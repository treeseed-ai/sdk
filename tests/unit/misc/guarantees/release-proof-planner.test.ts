...
import { buildProofPlan, hostedWorkflowForPackage } from '../../../../src/operations/services/guarantees/release-proof-planner.ts';
import { discoverPackageAdapters } from '../../../../src/operations/services/reconciliation/package-adapters.ts';
...
describe('release proof planner', () => {
	it('plans authoritative GitHub-hosted package proof from package manifests', () => {
		...
		const plan = buildProofPlan({ root, target: 'staging', driver: 'github-hosted' });
	...
	it('marks act proof as advisory', () => {
		const root = makeWorkspace();
		const plan = buildProofPlan({ root, target: 'staging', driver: 'act', subject: 'package:@treeseed/sdk' });
