...
import {
	assertSecretsCapabilityRegistry,
	assertClientEncryptedEscrowMetadata,
	assertGitHubActionsEncryptedSecretDeployment,
	assertWritableSecretMetadata,
	buildClientEncryptedEscrowEnvelope,
	TREEDX_CREDENTIAL_BRIDGE_OPERATIONS,
	isApiServiceDecryptableCustomerSecret,
	isSecretCustodyMode,
	isV1RepositoryCredentialProvider,
	requiresClientSideSecretMaterial,
	summarizeClientEncryptedEscrowStatus,
	summarizeWorkflowCloudAccessDiagnostics,
	validateClientEncryptedEscrowMetadata,
	validateGitHubActionsEncryptedSecretDeployment,
	validateSecretsCapabilityRegistry,
	validateWritableSecretMetadata,
	type SecretsCapabilityRegistry,
} from '../../../../src/configuration/secrets-capability.ts';
import packageJson from '../../../../package.json' with { type: 'json' };

function validRegistry(): SecretsCapabilityRegistry {
	return {
...
describe('secrets capability contracts', () => {
	it('accepts the v1 GitHub App and GitHub Actions secret enclave registry shape', () => {
		...
		const validation = validateSecretsCapabilityRegistry(registry);

		expect(validation.ok).toBe(true);
		expect(validation.problems).toEqual([]);
		expect(assertSecretsCapabilityRegistry(registry)).toBe(registry);
		expect(packageJson.exports).toHaveProperty('./secrets-capability');
		expect(isSecretCustodyMode('github_actions_secret_enclave')).toBe(true);
		expect(isV1RepositoryCredentialProvider('github-app')).toBe(true);
		expect(requiresClientSideSecretMaterial('client_encrypted_escrow')).toBe(true);
		expect(TREEDX_CREDENTIAL_BRIDGE_OPERATIONS).toEqual(expect.arrayContaining([
			'fetch',
			'push',
			'pull_request',
			'repository_update',
		]));
		expect(assertWritableSecretMetadata(registry.secrets![0])).toBe(registry.secrets![0]);
		expect(assertClientEncryptedEscrowMetadata(registry.clientEncryptedEscrow!.metadataRecords![0]))
	...
	it('accepts encrypted GitHub Actions secret deployment metadata without plaintext', () => {
		...
		expect(validateGitHubActionsEncryptedSecretDeployment(deployment)).toEqual([]);
		expect(assertGitHubActionsEncryptedSecretDeployment(deployment)).toBe(deployment);
	});

	it('rejects plaintext GitHub Actions secret deployment payloads', () => {
		expect(validateGitHubActionsEncryptedSecretDeployment({
			repository: 'treeseed-ai/project',
			scope: 'environment',
			environment: 'production',
			secretName: 'TREESEED_REPOSITORY_WRITE',
	...
	it('rejects non-GitHub repository credential providers in v1', () => {
		const validation = validateSecretsCapabilityRegistry({
			...validRegistry(),
			repositoryCredentialProviders: {
		...
		expect(isV1RepositoryCredentialProvider('gitlab')).toBe(false);
	});
	...
	it('rejects customer project secrets that are API-service-decryptable', () => {
		...
		expect(validateWritableSecretMetadata(secret)).toEqual(expect.arrayContaining([
	...
	it('keeps client-encrypted escrow metadata ciphertext-only', () => {
		...
		expect(validation.ok).toBe(false);
		expect(validateClientEncryptedEscrowMetadata({
			id: 'escrow-1',
			secretId: 'secret-1',
			ciphertextRef: 'local://draft-host/secret-1',
			algorithm: 'xchacha20-poly1305',
	...
	it('accepts client-encrypted escrow envelopes with KDF and cipher metadata', () => {
		...
		expect(validateClientEncryptedEscrowMetadata(envelope)).toEqual([]);
		expect(buildClientEncryptedEscrowEnvelope(envelope)).toStrictEqual(envelope);
	});
	...
	it('summarizes escrow status without revealing ciphertext', () => {
		expect(summarizeClientEncryptedEscrowStatus({
			status: 'active',
	...
	it('reports OIDC-preferred cloud access diagnostics without failing otherwise valid workflow policy', () => {
		...
		const diagnostics = summarizeWorkflowCloudAccessDiagnostics({
			...operation!,
			trustPolicy: {
