...
describe('scene plugin registry', () => {
	it('includes built-in plugin categories and deterministic summaries', () => {
		const summaries = listBuiltInScenePlugins();
	...
	it('registers built-in action and assertion handlers', () => {
		...
		expect([...registry.actions.keys()].sort()).toEqual(['apiRequest', 'browserHistory', 'click', 'clickVisibleSequence', 'fill', 'goto', 'keyboard', 'mailpitConfirmLatest', 'pause', 'select', 'waitForOperation']);
		expect([...registry.assertions.keys()].sort()).toEqual(['focused', 'notText', 'notVisible', 'operation', 'text', 'urlIncludes', 'visible']);
		expect(registry.actions.get('clickVisibleSequence')?.status).toBe('available');
	...
	it('registers Remotion as an available renderer plugin without loading Remotion components', () => {
		const registry = createBuiltInPluginRegistryForTest();
	...
	it('combines explicit plugins and reports duplicate ids', () => {
		...
		const resolved = resolveScenePlugins({ plugins: [duplicate] });
	...
	it('reports invalid plugin metadata', () => {
		...
		const resolved = resolveScenePlugins({ plugins: [invalid] });
		expect(resolved.ok).toBe(false);
...
function createBuiltInPluginRegistryForTest() {
	return createBuiltInScenePluginRegistry();
}
