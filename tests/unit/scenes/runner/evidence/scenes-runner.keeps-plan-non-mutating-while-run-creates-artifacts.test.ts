...
it('keeps plan non-mutating while run creates artifacts', async () => {
		...
		const plan = planScene({ projectRoot: root, scene: 'browser-smoke', timestamp: '20260614T120000Z', runId: 'run123' });
		expect(plan.artifactPaths?.runRoot).toBeTruthy();
		expect(existsSync(plan.artifactPaths!.runRoot)).toBe(false);
		const report = await runScene({ projectRoot: root, scene: 'browser-smoke', browserAdapter: new FakeAdapter(), timestamp: '20260614T120000Z', runId: 'run123' });
