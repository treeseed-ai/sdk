...
it('reports browser launch failures with Playwright remediation diagnostics', async () => {
		...
		Object.assign(error, { sceneCode: 'scene.playwright_browser_missing' });
		const report = await runScene({ projectRoot: root, scene: 'browser-smoke', browserAdapter: new FailingAdapter(error) });
