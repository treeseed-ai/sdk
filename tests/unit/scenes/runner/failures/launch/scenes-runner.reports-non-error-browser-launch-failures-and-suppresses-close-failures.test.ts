...
it('reports non-Error browser launch failures and suppresses close failures', async () => {
		...
		const launchReport = await runScene({ projectRoot: root, scene: 'browser-smoke', browserAdapter: new FailingAdapter('adapter offline') });
		...
		const nullLaunchReport = await runScene({ projectRoot: root, scene: 'browser-smoke', browserAdapter: new FailingAdapter(null) });
		expect(nullLaunchReport.diagnostics.some((entry) => entry.message.includes('Playwright unavailable.'))).toBe(true);

		const closeAdapter: SceneBrowserAdapter = {
			async launch(input) {
				return {
					page: new FakePage(),
		...
		const closeReport = await runScene({ projectRoot: root, scene: 'browser-smoke', browserAdapter: closeAdapter, record: true });
