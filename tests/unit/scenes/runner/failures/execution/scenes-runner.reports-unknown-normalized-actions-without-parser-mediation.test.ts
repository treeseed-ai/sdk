...
it('reports unknown normalized actions without parser mediation', async () => {
		...
		const normalized = validateScene({ projectRoot: root, scene: 'browser-smoke' }).scene!;
		const unknownAction = {
			...normalized,
			workflow: [{ ...normalized.workflow[0]!, action: { unknownAction: {} } as never }],
		};
		const actionReport = await runScene({ projectRoot: root, scene: unknownAction, browserAdapter: new FakeAdapter() });
