...
it('fails unresolved operation actions and assertions with clear diagnostics', async () => {
    ...
    expect:
      text: Done
`));
		const actionReport = await runScene({ projectRoot: root, scene: 'unsupported-action', browserAdapter: new FakeAdapter() });
    ...
    expect:
      operation:
        ...
        status:
          - completed
`);
		const assertionReport = await runScene({ projectRoot: root, scene: 'unsupported-assertion', browserAdapter: new FakeAdapter() });
