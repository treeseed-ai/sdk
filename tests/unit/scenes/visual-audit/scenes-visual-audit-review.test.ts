...
function capture(overrides: Partial<SceneVisualAuditCapture>): SceneVisualAuditCapture {
...
function manifest(captures: SceneVisualAuditCapture[]): SceneVisualAuditManifest {
	return {
...
describe('scene visual audit review', () => {
	it('builds client, functional, display, and architecture findings', () => {
		...
		const review = buildSceneVisualAuditReview({ manifest: manifest(captures), paths: paths(), detail: 'full', maxFindings: 100 });
	...
	it('writes review JSON, Markdown, agent brief, client errors, routes, and contact sheets', () => {
		...
		const review = buildSceneVisualAuditReview({ manifest: m, paths: p });

		writeSceneVisualAuditReview({ manifest: m, review, paths: p });
