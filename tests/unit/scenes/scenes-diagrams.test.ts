...
import {
	...
	type TreeseedSceneRunReport,
	type TreeseedSceneTimelineEvent,
} from '../../../src/scenes/index.ts';

function scene(diagrams: TreeseedSceneManifest['diagrams']): TreeseedSceneManifest {
	return {
...
const run = {
	...
	progressPath: null,
	resumedFrom: null,
} as TreeseedSceneRunReport;

const timeline: TreeseedSceneTimelineEvent[] = [
...
describe('scene diagram providers', () => {
	...
	it('validates operation lifecycle props', () => {
		...
		expect(validateTreeseedSceneDiagrams({ scene: valid }).filter((entry) => entry.severity === 'error')).toEqual([]);
		const missing = scene([{ id: 'op', renderer: 'remotion', at: 'open', component: 'OperationLifecycleDiagram', placement: 'interstitial', props: {} }]);
		expect(validateTreeseedSceneDiagrams({ scene: missing }).some((entry) => entry.code === 'scene.diagram_invalid_props')).toBe(true);
		...
		expect(validateTreeseedSceneDiagrams({ scene: invalidActive }).some((entry) => entry.code === 'scene.diagram_invalid_props')).toBe(true);
	});
	...
	it('normalizes reconciliation and scene timeline diagram props', () => {
		...
		const report = buildTreeseedSceneRenderDiagrams({ scene: current, run, timeline, registry });
	...
	it('rejects invalid topology links and unknown components', () => {
		...
		expect(validateTreeseedSceneDiagrams({ scene: invalidLink }).some((entry) => entry.code === 'scene.diagram_invalid_props')).toBe(true);
		const unknown = scene([{ id: 'missing', renderer: 'remotion', at: 'open', component: 'MissingDiagram', placement: 'interstitial', props: {} }]);
		expect(validateTreeseedSceneDiagrams({ scene: unknown }).some((entry) => entry.code === 'scene.diagram_unknown_component')).toBe(true);
	});
	...
	it('warns unknown props and rejects renderer mismatches', () => {
		...
		const diagnostics = validateTreeseedSceneDiagrams({ scene: current });
