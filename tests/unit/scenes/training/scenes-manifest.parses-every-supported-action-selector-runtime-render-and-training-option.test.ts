...
training:
  ...
  chapterClips:
    enabled: false
`));
		const outputsDisabled = planScene({
			projectRoot: root,
...
`, 'utf8');
		...
		const scene = validateScene({ projectRoot: root, scene: 'hosted-base-url-demo' }).scene;

		const report = resolveSceneBaseUrl({
			projectRoot: root,
			scene: scene!,
			environment: 'staging',
		});
