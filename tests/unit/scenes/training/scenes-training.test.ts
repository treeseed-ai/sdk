...
describe('scene training outputs', () => {
	it('builds deterministic captions, transcript, narration, glossary, and chapter clip manifests', async () => {
		...
		const outputs = buildSceneTrainingOutputs({ scene, run: runJson, timeline });
	...
	it('formats captions and Markdown outputs deterministically', async () => {
		const root = workspace();
		const run = await createRun(root);
		const report = generateSceneTrainingOutputs({ projectRoot: root, scene: 'training-demo', from: run.artifacts!.runRoot });
		...
		expect(formatSceneCaptionsVtt(report.outputs!.captions)).toBe(vtt);
		expect(formatSceneCaptionsSrt(report.outputs!.captions)).toBe(srt);
		expect(formatSceneTranscriptMarkdown(report.outputs!.transcript)).toContain('# Training Demo');
		expect(formatSceneNarrationMarkdown(report.outputs!.narration)).toContain('# Treeseed Scene Narration Script');
	});
	...
	it('can filter output formats', async () => {
		const root = workspace();
		const run = await createRun(root);
		const report = generateSceneTrainingOutputs({ projectRoot: root, scene: 'training-demo', from: run.artifacts!.runRoot, formats: ['vtt'] });
		expect(report.ok).toBe(true);
	...
	it('reports missing run, missing timeline, and scene mismatch diagnostics', async () => {
		const root = workspace();
		writeScene(root, sceneYaml());
		const missing = generateSceneTrainingOutputs({ projectRoot: root, scene: 'training-demo', from: 'missing' });
		...
		const missingTimeline = generateSceneTrainingOutputs({ projectRoot: root, scene: 'training-demo', from: run.artifacts!.runRoot });
		...
		const mismatch = generateSceneTrainingOutputs({ projectRoot: root, scene: 'training-demo', from: mismatchRun.artifacts!.runRoot });
