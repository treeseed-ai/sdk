...
import { resolveEvidenceViewport } from '../../../../src/scenes/support/rendering/remotion-compositions.ts';
...
class FakeRendererAdapter implements SceneRendererAdapter {
	id = 'fake';
	...
	async render(input: Parameters<SceneRendererAdapter['render']>[0]) {
		this.calls.push({
...
` : '';
	...
	const { runScene } = await import('../../../../src/scenes/index.ts');
...
describe('scene Remotion render MVP', () => {
	it('loads render input from existing run artifacts and falls back to screenshots without video', async () => {
		...
		const loaded = loadSceneRenderInput({ projectRoot: root, scene: 'render-demo', from: run.artifacts!.runRoot });
	...
	it('uses Playwright video refs as primary media and stages local video for Remotion', async () => {
		...
		const report = await renderScene({ projectRoot: root, scene: 'render-demo', from: run.artifacts!.runRoot, rendererAdapter: adapter });
  ...
  profiles:
    - id: tablet
      ...
      hasTouch: true
`));
		const { runScene: runMismatchScene } = await import('../../../../src/scenes/index.ts');
		...
		const mismatch = loadSceneRenderInput({ projectRoot: mismatchRoot, scene: 'render-demo', from: mismatchRun.artifacts!.runRoot });
...
`;
		...
		const { runScene } = await import('../../../../src/scenes/index.ts');
		const matchedRun = await runScene({
		...
		const matched = loadSceneRenderInput({ projectRoot: matchedRoot, scene: 'render-demo', from: matchedRun.artifacts!.runRoot });
	...
	it('uses recorded device metadata for output resolution and render device checks', async () => {
		...
		const matching = await renderScene({ projectRoot: root, scene: 'render-demo', from: run.artifacts!.runRoot, device: 'mobile', rendererAdapter: new FakeRendererAdapter() });
		expect(matching.ok).toBe(true);
		expect(matching.device?.id).toBe('mobile');
		const mismatch = await renderScene({ projectRoot: root, scene: 'render-demo', from: run.artifacts!.runRoot, device: 'desktop', rendererAdapter: new FakeRendererAdapter() });
	...
	it('defaults modes and compositions for training and failure-review renders', async () => {
		...
		const trainingReport = await renderScene({ projectRoot: root, scene: 'render-demo', from: training.artifacts!.runRoot, rendererAdapter: new FakeRendererAdapter() });
		...
		const failureReport = await renderScene({ projectRoot: root, scene: 'render-demo', from: failed.artifacts!.runRoot, rendererAdapter: new FakeRendererAdapter() });
	...
	it('supports chapter rendering and reports invalid render requests', async () => {
		const root = workspace();
		const run = await createRun(root);
		const chapter = await renderScene({ projectRoot: root, scene: 'render-demo', from: run.artifacts!.runRoot, mode: 'chapter', chapterId: 'intro', rendererAdapter: new FakeRendererAdapter() });
		expect(chapter.ok).toBe(true);
		expect(chapter.mode).toBe('chapter');
		const missingChapter = await renderScene({ projectRoot: root, scene: 'render-demo', from: run.artifacts!.runRoot, mode: 'chapter', chapterId: 'missing', rendererAdapter: new FakeRendererAdapter() });
		...
		const badRenderer = await renderScene({ projectRoot: root, scene: 'render-demo', from: run.artifacts!.runRoot, renderer: 'bad', rendererAdapter: new FakeRendererAdapter() });
		expect(badRenderer.diagnostics[0]?.code).toBe('scene.renderer_unknown');
		const badFormat = await renderScene({ projectRoot: root, scene: 'render-demo', from: run.artifacts!.runRoot, format: 'webm' as 'mp4', rendererAdapter: new FakeRendererAdapter() });
	...
	it('renders typed diagrams and supports diagram-only mode without browser media', async () => {
		...
		const diagramReport = await renderScene({ projectRoot: root, scene: 'render-demo', from: run.artifacts!.runRoot, rendererAdapter: new FakeRendererAdapter() });
		...
		const adapter = new FakeRendererAdapter();
		const diagramOnly = await renderScene({ projectRoot: root, scene: 'render-demo', from: run.artifacts!.runRoot, mode: 'diagram-only', rendererAdapter: adapter });
		...
		const missing = await renderScene({ projectRoot: root, scene: 'render-demo', from: run.artifacts!.runRoot, rendererAdapter: new FakeRendererAdapter() });
	...
	it('blocks diagram-only rendering when no diagrams exist', async () => {
		const root = workspace();
		const run = await createRun(root);
		const report = await renderScene({ projectRoot: root, scene: 'render-demo', from: run.artifacts!.runRoot, mode: 'diagram-only', rendererAdapter: new FakeRendererAdapter() });
