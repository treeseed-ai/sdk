...
import { readTreeseedDevInstance } from '../../../src/local-dev/managed-dev.ts';
...
describe('scene managed base URL resolution', () => {
	it('resolves managed local web and API health URLs with host normalization', () => {
		vi.mocked(readTreeseedDevInstance).mockImplementation(({ surface }) => surface === 'web'
		...
		expect(resolveTreeseedSceneBaseUrl({ projectRoot: process.cwd(), scene: scene(), environment: 'local' })).toMatchObject({
			ok: true,
			baseUrl: 'http://0.0.0.0:4321/healthz',
		});
		expect(resolveTreeseedSceneApiBaseUrl({ projectRoot: process.cwd(), environment: 'local', webBaseUrl: 'http://web.test' })).toBe('http://127.0.0.1:3000');
	});
	...
	it('falls back when managed instances are missing or expose non-URL health strings', () => {
		...
		const web = resolveTreeseedSceneBaseUrl({ projectRoot: process.cwd(), scene: scene(), environment: 'local' });
		expect(web.ok).toBe(false);
		expect(web.diagnostics.map((entry) => entry.code)).toContain('scene.local_dev_not_running');
		expect(resolveTreeseedSceneApiBaseUrl({ projectRoot: process.cwd(), environment: 'local', webBaseUrl: 'http://web.test' })).toBe('api.local');

		vi.mocked(readTreeseedDevInstance).mockReturnValue({ running: false, health: [{ kind: 'http', url: 'http://127.0.0.1:4321' }] } as never);
		expect(resolveTreeseedSceneBaseUrl({ projectRoot: process.cwd(), scene: scene(), environment: 'local' }).ok).toBe(false);
	});
...
`);
		expect(resolveTreeseedSceneBaseUrl({ projectRoot: root, scene: scene(), environment: 'staging' })).toMatchObject({
			ok: true,
			baseUrl: 'https://web-service.example.test/app/',
		});
		expect(resolveTreeseedSceneBaseUrl({ projectRoot: root, scene: scene(), environment: 'prod' })).toMatchObject({
			ok: true,
			baseUrl: 'https://prod-web.example.test',
		});
		expect(resolveTreeseedSceneApiBaseUrl({ projectRoot: root, environment: 'staging', webBaseUrl: 'https://web.example.test' })).toBe('https://api-connection.example.test');
		expect(resolveTreeseedSceneApiBaseUrl({ projectRoot: root, environment: 'prod', webBaseUrl: 'https://web.example.test' })).toBe('https://api-service-prod.example.test');
	});
	...
	it('falls through malformed hosted config, package API config, env API fallback, and empty domain records', () => {
		...
		expect(resolveTreeseedSceneBaseUrl({ projectRoot: malformedRoot, scene: scene(), environment: 'staging' })).toMatchObject({
			ok: true,
...
`);
		expect(resolveTreeseedSceneApiBaseUrl({ projectRoot: apiRoot, environment: 'staging', webBaseUrl: 'https://web.example.test' })).toBe('https://surface-api.example.test');
		expect(resolveTreeseedSceneApiBaseUrl({ projectRoot: apiRoot, environment: 'prod', webBaseUrl: 'https://web.example.test' })).toBe('https://prod-surface-api.example.test');

		vi.mocked(readTreeseedDevInstance).mockReturnValue(null);
		process.env.TREESEED_API_BASE_URL = '   ';
		process.env.TREESEED_MARKET_API_BASE_URL = ' http://127.0.0.1:3999/api/ ';
		expect(resolveTreeseedSceneApiBaseUrl({ projectRoot: apiRoot, environment: 'local', webBaseUrl: 'http://web.test' })).toBe('http://127.0.0.1:3999/api');
