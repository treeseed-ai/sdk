...
describe('scene evidence publishing', () => {
	it('validates deny-by-default redaction policies', () => {
		const policy = createDefaultTreeseedSceneRedactionPolicy('local');
		expect(policy.mode).toBe('deny-by-default');
		expect(policy.rules.some((rule) => rule.artifactKind === 'run-report' && rule.include)).toBe(true);
		expect(validateTreeseedSceneRedactionPolicy({ policy })).toEqual([]);
		expect(validateTreeseedSceneRedactionPolicy({ policy: { ...policy, schemaVersion: 'wrong' } }).some((entry) => entry.code === 'scene.publish_redaction_policy_invalid')).toBe(true);
		expect(validateTreeseedSceneRedactionPolicy({
			policy: {
				...policy,
				rules: [
	...
	it('publishes from existing evidence with a redacted local bundle and run update', async () => {
		...
		expect(generateTreeseedSceneTrainingOutputs({ projectRoot: root, scene: 'publish-demo', from: run.artifacts!.runRoot }).ok).toBe(true);
		expect(generateTreeseedSceneEvidence({ projectRoot: root, scene: 'publish-demo', from: run.artifacts!.runRoot }).ok).toBe(true);

		const report = await publishTreeseedSceneEvidence({ projectRoot: root, scene: 'publish-demo', from: run.artifacts!.runRoot, target: 'local' });
	...
	it('auto-generates evidence when needed and blocks failed release publish', async () => {
		...
		const local = await publishTreeseedSceneEvidence({ projectRoot: root, scene: 'publish-demo', from: run.artifacts!.runRoot, target: 'local' });
		expect(local.ok).toBe(true);
		expect(local.warnings.some((entry) => entry.code === 'scene.publish_generated_evidence')).toBe(true);

		const release = await publishTreeseedSceneEvidence({ projectRoot: root, scene: 'publish-demo', from: run.artifacts!.runRoot, target: 'release' });
	...
	it('reports missing run, scene mismatch, invalid policy, and no included artifacts', async () => {
		const root = workspace();
		writeScene(root);
		const missing = await publishTreeseedSceneEvidence({ projectRoot: root, scene: 'publish-demo', from: 'missing' });
		...
		const mismatch = await publishTreeseedSceneEvidence({ projectRoot: root, scene: 'publish-demo', from: run.artifacts!.runRoot });
		...
		const invalidPolicy = await publishTreeseedSceneEvidence({ projectRoot: root, scene: 'publish-demo', from: run.artifacts!.runRoot, redactionPolicyPath: invalidPolicyPath });
...
rules: []
`);
		const noArtifacts = await publishTreeseedSceneEvidence({ projectRoot: root, scene: 'publish-demo', from: run.artifacts!.runRoot, redactionPolicyPath: emptyPolicyPath });
	...
	it('keeps publish output ok when run.json cannot be updated', async () => {
		...
		const report = await publishTreeseedSceneEvidence({ projectRoot: root, scene: 'publish-demo', from: run.artifacts!.runRoot });
