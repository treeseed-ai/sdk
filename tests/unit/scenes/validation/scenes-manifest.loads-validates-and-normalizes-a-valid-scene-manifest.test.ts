...
it('loads, validates, and normalizes a valid scene manifest', () => {
		...
		const loaded = loadSceneDocument(root, 'market-project-deploy-demo');
		expect(loaded.diagnostics).toEqual([]);
		const report = validateScene({ projectRoot: root, scene: 'market-project-deploy-demo' });
...
`));
		...
		const report = validateScene({ projectRoot: root, scene: 'visual-audit-demo' });
...
visualAudit:
  ...
  review:
    ...
    maxFindings: 0
`));
		const invalid = validateScene({ projectRoot: root, scene: 'invalid-visual-audit-demo' });
...
it('reports dense schema diagnostics for malformed journey, workflow, devices, visuals, and training config', () => {
		...
		const scene = parseSceneManifest({
