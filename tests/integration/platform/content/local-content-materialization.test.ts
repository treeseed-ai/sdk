...
import { runRepositoryGit } from '../../../../src/operations/services/operations/git-runner.ts';
import { planReconciliation, reconcileTarget } from '../../../../src/reconcile/index.ts';
import type { DesiredUnit } from '../../../../src/reconcile/support/contracts/contracts.ts';
...
function createSourceRepo() {
	...
	runRepositoryGit(['init', '--initial-branch=main'], { cwd: repoRoot, mode: 'mutate' });
	runRepositoryGit(['config', 'user.email', 'test@example.com'], { cwd: repoRoot, mode: 'mutate' });
	runRepositoryGit(['config', 'user.name', 'Treeseed Test'], { cwd: repoRoot, mode: 'mutate' });
	writeFileSync(resolve(repoRoot, 'README.md'), '# Content\n');
	runRepositoryGit(['add', 'README.md'], { cwd: repoRoot, mode: 'mutate' });
	runRepositoryGit(['commit', '-m', 'seed content'], { cwd: repoRoot, mode: 'mutate' });
	return repoRoot;
}

function localContentUnit(tenantRoot: string, sourceRepo: string, targetPath: string): DesiredUnit {
	return {
