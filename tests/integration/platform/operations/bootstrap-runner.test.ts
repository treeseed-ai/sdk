...
import { runPrefixedCommand, runBootstrapDag } from '../../../../src/operations/services/operations/bootstrap-runner.ts';
...
describe('bootstrap DAG runner', () => {
	...
	it('records node timings and emits progress without changing results', async () => {
		...
		const writes: string[] = [];
		const result = await runBootstrapDag<string>({
			nodes: [
