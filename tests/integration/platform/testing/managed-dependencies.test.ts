...
} from '../../../../src/entrypoints/runtime/managed-dependencies.ts';
...
describe('managed dependencies', () => {
	...
	it('uses TREESEED_TOOLS_HOME for managed dependency status', async () => {
		...
		const result = collectDependencyStatus({
	...
	it('resolves npm-backed tool binaries from the SDK dependency graph', () => {
		expect(resolveToolBinary('wrangler')).toContain('wrangler');
		expect(resolveToolBinary('railway')).toBeNull();
		expect(resolveToolBinary('copilot')).toContain('@github');
		expect(resolveToolBinary('copilot-sdk')).toBeNull();
	});

	it('reports the Copilot TypeScript SDK as a managed npm package', () => {
		const result = collectDependencyStatus({ spawn: spawnMock() });
