...
import type { ControlPlaneReporter } from '../../../../../src/entrypoints/clients/control-plane.ts';

import {
	monitorProjectPlatform,
	publishProjectContent,
	resolveRailwayServiceDeployDependencies,
} from '../../../../../src/operations/services/projects/projects-core/project-platform.ts';
...
it('provisions configured provider resources before hosted deploy steps', () => {
		...
		expect(publishSource).toContain('resolveResourceIdentity(siteConfig, target).teamId');
...
it('writes provider timing summaries into deploy workflow GitHub step summaries', () => {
		const workflowPaths = [
			...
			new URL('../../../../../../../packages/core/templates/github/deploy-web.workflow.yml', import.meta.url),
...
it('reconciles Railway Postgres volume naming through the IaC project graph', () => {
		const source = readSourceModule(new URL('../../../../../src/reconcile/reconciliation/builtin-adapters.ts', import.meta.url));
...
it('does not expose a Railway CLI volume reconciliation fallback', () => {
		const source = readSourceModule(new URL('../../../../../src/operations/services/hosting/railway/railway-deploy.ts', import.meta.url));
