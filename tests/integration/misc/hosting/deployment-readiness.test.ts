...
import { collectDeploymentReadiness } from '../../../../src/operations/services/hosting/deployment/deployment-readiness.ts';
...
function byId(report: ReturnType<typeof collectDeploymentReadiness>, id: string) {
...
`),
	...
	it('uses a discovered embedded API package root when validating hosting readiness', () => {
		...
		const report = collectDeploymentReadiness({ tenantRoot: root, environment: 'staging' });
		expect(report.ok).toBe(true);
	...
	it('passes current API package deployment shape', () => {
		const report = collectDeploymentReadiness({ tenantRoot: rootWith(config()), environment: 'staging' });
		expect(report.ok).toBe(true);
		expect(byId(report, 'railway-config:api:rootDirectory')).toMatchObject({ status: 'passed' });
	...
	it('requires environment-qualified Railway identities with production image deployment readiness', () => {
		const previous = {
		...
		try {
			const report = collectDeploymentReadiness({ tenantRoot: rootWith(config()), environment: 'prod' });
	...
	it('requires package-local API and runner Dockerfile build contracts for local readiness', () => {
		const report = collectDeploymentReadiness({ tenantRoot: rootWith(config()), environment: 'local' });
		expect(report.ok).toBe(true);
