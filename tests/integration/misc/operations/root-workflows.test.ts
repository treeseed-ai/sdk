...
import { PUBLIC_RELEASE_PACKAGE_NAMES } from '../../../../src/operations/services/treedx/workspaces/workspace-save.ts';
import { planGuarantees } from '../../../../src/guarantees/index.ts';
...
describe('root workflow bootstrap selection', () => {
	it('selects exact API/Agent and scene-backed release-gate collections', () => {
		...
		const apiAgent = planGuarantees({ workspaceRoot, filter: { ownerPackages: ['@treeseed/api', '@treeseed/agent'] }, includeDependencies: false });
		const ui = planGuarantees({ workspaceRoot, filter: { sceneBacked: true }, includeDependencies: false });
	...
	it('keeps hosted deployment suspended while retaining verification and manual guarantees', () => {
		...
		const operations = readSourceModule(resolve(sdkRoot, 'src/workflow/operations.ts'));
		expect(operations).toContain('function stagingCandidateWorkflowGates');
		expect(operations).toContain("adapter ? hostedWorkflowForPackage(adapter) : 'verify.yml'");
...
describe('admin package workflow integration', () => {
	it('keeps admin in public release order and web app staging selection', () => {
		const operationsSource = readSourceModule(resolve(sdkRoot, 'src', 'workflow', 'operations.ts'));

		if (!integratedWorkspaceAvailable) {
			expect(PUBLIC_RELEASE_PACKAGE_NAMES).toContain('@treeseed/admin');
			expect(operationsSource).toContain("{ name: '@treeseed/admin', dir: 'packages/admin', artifacts: ['dist/plugin.js'] }");
			return;
		}

		const workspaceBootstrapSource = readFileSync(resolve(workspaceRoot, 'packages', 'core', 'scripts', 'treedx', 'workspaces', 'workspace-bootstrap.ts'), 'utf8');
		...
		expect(PUBLIC_RELEASE_PACKAGE_NAMES).toEqual([
			'@treeseed/sdk',
			'@treeseed/ui',
			'@treeseed/core',
			'@treeseed/admin',
