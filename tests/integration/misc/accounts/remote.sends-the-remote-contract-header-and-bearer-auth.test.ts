...
it('sends the remote contract header and bearer auth', async () => {
		...
		const authClient = new RemoteAuthClient(client);
		...
		expect(calls[0]?.headers[REMOTE_CONTRACT_HEADER]).toBe(String(REMOTE_CONTRACT_VERSION));
	});
...
it('persists encrypted remote auth state and resolves configured hosts', () => {
		...
		const config = resolveRemoteConfig(tenantRoot, {});
