...
import { exportCodebase } from '../../../../src/operations/services/runtime/export-runtime.ts';
...
describe('export runtime', () => {
	it('exports markdown snapshots with ignored export artifacts and configured ignore patterns', async () => {
		...
		const exported = await exportCodebase({
...
export:
  bundledPaths:
    - "packages/missing"
`, 'utf8');
		const exportedMissing = await exportCodebase({
			directory: resolve(root, 'src'),
		});
