...
import {
	runLiveReconcileTests,
	LiveReconcileResourcePrefix,
...
describe('live reconciliation acceptance harness', () => {
	it('keeps smoke mode read-only and canonical', async () => {
		const result = await runLiveReconcileTests({
			cwd: process.cwd(),
			environment: 'staging',
			mode: 'smoke',
			providers: ['local'],
	...
	it('uses one deterministic Railway project identity for the provider run', async () => {
		...
		const prefix = LiveReconcileResourcePrefix('staging', 'railway', runId);
		const result = await runLiveReconcileTests({
			cwd: process.cwd(),
			environment: 'staging',
			mode: 'acceptance',
	...
	it('runs local capacity assignment proof through the public market clients when configured', async () => {
		const runId = '20260608120000';
		const prefix = LiveReconcileResourcePrefix('local', 'local', runId);
