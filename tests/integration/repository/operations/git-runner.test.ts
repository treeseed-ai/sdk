...
import {
	...
	runRepositoryGit,
	GitCommandUsesRemote,
} from '../../../../src/operations/services/operations/git-runner.ts';
...
describe('Treeseed Git runner locks', () => {
	it('bounds remote commands without changing local command timeouts', () => {
		expect(GitCommandUsesRemote(['ls-remote', 'origin'])).toBe(true);
		expect(GitCommandUsesRemote(['status', '--porcelain'])).toBe(false);
		expect(resolveGitCommandTimeoutMs(['fetch', 'origin'])).toBe(60_000);
		expect(resolveGitCommandTimeoutMs(['push', 'origin'], 12_345)).toBe(12_345);
		expect(resolveGitCommandTimeoutMs(['status'])).toBeUndefined();
	});
	...
	it('detects stale remote ref locks', () => {
		...
		const locks = inspectGitLockSet(root).filter((entry) => entry.exists);
	...
	it('recovers stale ref locks through the public recovery helper', () => {
		...
		const diagnostics = recoverGitLocks(root, { execute: true });
	...
	it('removes safe stale locks before mutating Git commands', () => {
		...
		runRepositoryGit(['status', '--porcelain'], {
			cwd: root,
			mode: 'mutate',
		});
