...
it('ensures Railway deploy ignore entries for local workspace artifacts', () => {
		...
		const railwayIgnorePath = ensureRailwayIgnoreEntries(tenantRoot);
...
it('builds launch env from machine config without recreating deprecated env files', () => {
		...
		setMachineEnvironmentValue(tenantRoot, 'local', {
			id: 'SHARED_VALUE',
			sensitivity: 'plain',
			storage: 'shared',
		} as any, 'from-machine');
		...
		warnDeprecatedLocalEnvFiles(tenantRoot, (line) => warnings.push(line));
