...
const {
	...
	writeMachineConfig,
} = await import('../../../../src/operations/services/configuration/config-runtime.ts');
...
function setConfigValue(
	...
	setMachineEnvironmentValue(tenantRoot, scope, {
		id,
		sensitivity,
...
function hasConfigEntry(tenantRoot: string, id: string) {
	return collectConfigContext({ tenantRoot, scopes: ['staging'], env: {} })
...
describe('config GitHub environment sync reconciliation', () => {
	...
	it('syncs GitHub environment values through reconciler-owned binding units', async () => {
		...
		const result = await syncGitHubEnvironment({ tenantRoot, scope: 'staging', entryIds: ['TREESEED_GITHUB_TOKEN'] });
	...
	it('keeps GitHub environment plan reporting non-mutating', async () => {
		...
		const result = await syncGitHubEnvironment({ tenantRoot, scope: 'staging', planOnly: true });
	...
	it('finalize github sync applies provider values through reconciliation', async () => {
		...
		const result = await finalizeConfig({
			tenantRoot,
			scopes: ['staging'],
			sync: 'github',
