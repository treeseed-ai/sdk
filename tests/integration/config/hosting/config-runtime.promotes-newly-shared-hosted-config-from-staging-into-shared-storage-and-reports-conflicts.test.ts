...
it('promotes newly shared hosted config from staging into shared storage and reports conflicts', () => {
		...
		setMachineEnvironmentValue(tenantRoot, 'prod', {
			id: 'SHARED_VALUE',
			label: 'Shared value',
			sensitivity: 'plain',
			storage: 'scoped',
		...
		const result = applyConfigValues({
			tenantRoot,
			updates: [],
		...
		expect(resolveMachineEnvironmentValues(tenantRoot, 'prod').SHARED_VALUE).toBe('staging-value');
		const persisted = loadMachineConfig(tenantRoot);
