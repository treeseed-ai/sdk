...
vi.mock('../../../../src/operations/services/configuration/config-runtime.ts', () => ({
	...
	resolveMachineEnvironmentValues: resolveMachineEnvironmentValuesMock,
}));
vi.mock('../../../../src/operations/services/hosting/deployment/deploy.ts', async () => {
	...
	return {
		...
		buildSecretMap: vi.fn(() => ({
			FORM_TOKEN_SECRET: 'generated-form-secret',
			TREESEED_EDITORIAL_PREVIEW_SECRET: 'generated-preview-secret',
			TURNSTILE_SECRET_KEY: deployState.turnstileWidgets.formGuard.secret,
		})),
...
vi.mock('../../../../src/operations/services/hosting/railway/railway-deploy.ts', () => ({
...
beforeEach(() => {
	...
	cloudflareApiRequestMock.mockReset();
		...
		resolveMachineEnvironmentValuesMock.mockReset();
		resolveMachineEnvironmentValuesMock.mockImplementation(() => {
...
it('creates live KV and D1 resources when deploy state still has placeholder ids and syncs Pages env vars', async () => {
		const { createCloudflareReconcileAdapters } = await import('../../../../src/reconcile/reconciliation/builtin-adapters.ts');
			...
			deployment_configs: {
				preview: {
					env_vars: {
						...
						PUBLIC_TURNSTILE_SITE_KEY: { type: 'plain_text', value: 'managed-site-key' },
		...
		expect(resolveMachineEnvironmentValuesMock).not.toHaveBeenCalled();
	});
});
