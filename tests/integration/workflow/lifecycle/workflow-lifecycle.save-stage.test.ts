...
it('loads existing machine config secrets before evaluating status readiness', async () => {
		...
		const statusEnv = {
			...process.env,
			[MACHINE_KEY_PASSPHRASE_ENV]: 'status-passphrase',
		};
		...
		setMachineEnvironmentValue(work, 'staging', {
		...
		lockSecretSession(work);
		const workflow = new WorkflowSdk({ cwd: work, env: statusEnv, write: () => {} });
	...
	it('reconciles staging hosting when deployed resource verification has no pushed save', () => {
		...
		const hostedCiSource = sourceFunctionBody(source, 'reconcileSaveHostedEnvironment');

		expect(hostedCiSource).toContain('compileHostingGraph');
		expect(hostedCiSource).toContain('selectorFromWorkflowHostingGraph');
		expect(hostedCiSource).toContain('reconcileTarget');
		expect(hostedCiSource).toContain('collectReconcileStatus');
		expect(hostedCiSource).toContain('collectLiveHostedServiceChecks');
...
it('resolves status from nested directories against the tenant root', () => {
		...
		const result = resolveWorkflowPaths(nested);
