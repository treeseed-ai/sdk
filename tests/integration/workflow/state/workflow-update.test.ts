...
async function runUpdate(cwd: string, input: Parameters<WorkflowSdk['update']>[0]) {
...
describe('workflow update', () => {
	...
	it('stops on the first merge conflict and reports conflicted files', async () => {
		...
		expect((error as WorkflowError).code).toBe('merge_conflict');
		expect((error as WorkflowError).details?.files).toContain('conflict.txt');
