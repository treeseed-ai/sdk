...
import { capObsoleteWorkflowRuns, capWorkflowRunHistory, recommendNextSteps, type WorkflowState } from '../../../../src/operations/workflow-state.ts';
import {
	acquireWorkflowLock,
...
function stagingState(unreleasedStagingCommits: number): WorkflowState {
	return {
		workspaceRoot: true,
		deployConfigPresent: true,
		files: { machineConfig: true, machineKey: true, Config: true },
		secrets: {
...
describe('workflow state recommendations', () => {
	it('does not recommend release when staging is only ahead by release sync commits', () => {
		const recommendations = recommendNextSteps(stagingState(0));
	...
	it('recommends release when staging has unreleased commits', () => {
		const recommendations = recommendNextSteps(stagingState(2));
	...
	it('marks failed save journals stale after a newer save advances the recorded heads', () => {
		const journal: WorkflowRunJournal = {
			schemaVersion: 1,
