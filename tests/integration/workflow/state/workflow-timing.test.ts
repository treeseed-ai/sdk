...
import { createWorkflowTimer, formatDuration, slowestWorkflowPhases } from '../../../../src/operations/services/operations/workflow-timing.ts';

describe('workflow timing', () => {
	it('records passed, failed, and skipped phases', () => {
		const timer = createWorkflowTimer('2026-01-01T00:00:00.000Z');
	...
	it('sorts slowest phases and formats durations', () => {
		...
		expect(slowestWorkflowPhases(timing, 1)[0]?.id).toBe('long');
