...
import { NOTIFICATION_CONTENT_CAPABILITIES, isValidPersonalThemeDraft, normalizeNotificationPreferences } from '../../../src/accounts/account-contracts.ts';
import { defineRoute, validateRouteCapabilities } from '../../../src/platform/support/plugin.ts';
...
describe('account redesign contracts', () => {
	...
	it('rejects route collisions and incomplete capabilities', () => {
		...
		const route = defineRoute({ pattern: '/auth/test', resourcePath: 'pages/auth/test.astro', capability });
		expect(() => validateRouteCapabilities([route, route])).toThrow(/Duplicate TreeSeed route pattern/u);
	});
});
