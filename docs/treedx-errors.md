...
TreeDX SDK requests throw `TreeDxApiError` for API, network, timeout, and local validation failures.
...
Federated partial failures are normally returned in successful payloads when `includeErrors: true`. When a server rejects the whole request, the SDK preserves the response envelope in `TreeDxApiError.payload`.
